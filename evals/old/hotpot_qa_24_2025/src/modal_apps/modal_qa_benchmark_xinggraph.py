...
from qa.qa_benchmark_xinggraph import XingGraphConfig, QABenchmarkXingGraph
from modal_apps.modal_image import image

APP_NAME = "qa-benchmark-xinggraph"
VOLUME_NAME = "qa-benchmarks"
BENCHMARK_NAME = "xinggraph"
...
def run_xinggraph_benchmark(config_params: dict, dir_suffix: str):
    """Run the XingGraph QA benchmark on Modal."""
    print("Received benchmark request for XingGraph.")

    # Create benchmark folder structure
    qa_engine = config_params.get("qa_engine", "xinggraph_graph_completion")
    ...
    config = XingGraphConfig(**config_params)
...
# qa_engine: str = 'xinggraph_graph_completion', 'xinggraph_graph_completion_cot', 'xinggraph_graph_completion_context_extension'
...
async def main(
    ...
    """Trigger XingGraph QA benchmark runs on Modal."""
    print(f"🚀 Launching {runs} XingGraph QA benchmark run(s) on Modal with these parameters:")
    print(f"  - runs: {runs}")
    ...
    for params in config_params_list:
        run_xinggraph_benchmark.spawn(params, base_timestamp)
