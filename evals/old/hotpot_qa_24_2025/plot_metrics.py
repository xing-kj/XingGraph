...
INPUT_XINGGRAPH = Path("evals/benchmark_summary_xinggraph.json")
INPUT_COMPETITION = Path("evals/benchmark_summary_competition.json")

OUT_OPTIMISED = "evals/optimized_xinggraph_configurations.png"
...
def main() -> None:
    ...
    xinggraph_file = Path(sys.argv[1]) if len(sys.argv) > 1 else INPUT_XINGGRAPH
    comp_file = Path(sys.argv[2]) if len(sys.argv) > 2 else INPUT_COMPETITION

    if not xinggraph_file.exists():
        raise FileNotFoundError(f"{xinggraph_file} not found")
    if not comp_file.exists():
        raise FileNotFoundError(f"{comp_file} not found")

    # Optimised XingGraph configurations
    cfg_records = _load(xinggraph_file)
    ...
    _plot_grouped_bar(
        ...
        err_m,
        err_p,
        title="Optimized XingGraph Configurations",
        outfile=OUT_OPTIMISED,
        rotate_xticks=True,
    )
    print(f"Wrote {OUT_OPTIMISED}")

    # XingGraph vs. competition
