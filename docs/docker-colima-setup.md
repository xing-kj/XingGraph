# Docker & Colima Setup for XingGraph UI / MCP

The `xinggraph-cli -ui` command starts an MCP server inside a Docker container.
...
> xinggraph MCP entrypoint also includes automatic fallback logic (tries
