"""Top-level serve() orchestrator — connects the SDK to XingGraph Cloud or a local instance."""
...
async def serve(
    ...
    **Cloud mode** — when ``url`` is not provided, runs the full Auth0
    ...
    Args:
        url: Direct URL of a XingGraph instance. Skips Auth0 and tenant
            discovery. Can also be set via ``XINGGRAPH_SERVICE_URL`` env var.
        api_key: API key for authentication. Used with ``url`` for direct
            connections, or via ``XINGGRAPH_API_KEY`` env var.
    ...
    service_url = url or os.getenv("XINGGRAPH_SERVICE_URL")
    resolved_api_key = api_key or os.getenv("XINGGRAPH_API_KEY", "")
...
async def _serve_direct(service_url: str, api_key: str = "") -> CloudClient:
    """Connect directly to a XingGraph instance — no Auth0, no Management API."""
    from xinggraph.api.v1.serve.cloud_client import CloudClient
    from xinggraph.api.v1.serve.credentials import CloudCredentials, save_credentials
    from xinggraph.api.v1.serve.state import set_remote_client
    ...
    print(f"  Connected to XingGraph ({mode}) at {service_url}")
    return client
...
async def _serve_cloud(
    ...
    from xinggraph.api.v1.serve.cloud_client import CloudClient
    from xinggraph.api.v1.serve.credentials import (
        CloudCredentials,
        is_token_expired,
        load_credentials,
        save_credentials,
    )
    from xinggraph.api.v1.serve.device_auth import (
        device_code_login,
        extract_email_from_id_token,
        refresh_access_token,
    )
    from xinggraph.api.v1.serve.management_api import (
        create_tenant,
        get_current_tenant,
    ...
    mgmt_url = management_url or os.getenv(
        "XINGGRAPH_CLOUD_URL", "https://api.dev.cloud.xing-kj.com"
    )
    ...
    if creds and creds.service_url and creds.api_key:
        if not is_token_expired(creds):
            ...
            if await client._health_check():
                set_remote_client(client)
                print(f"  Connected to XingGraph Cloud at {creds.service_url}")
                return client
            else:
    ...
    print("  Authenticating with XingGraph Cloud...")
    ...
    set_remote_client(client)
    print(f"  Connected to XingGraph Cloud at {service_url}")
    if email:
