...
from xinggraph.modules.pipelines.methods import get_pipeline_run
from xinggraph.modules.users.models import User
from xinggraph.modules.users.methods import get_authenticated_user
from xinggraph.modules.users.get_user_db import get_user_db_context
from xinggraph.modules.graph.methods import get_formatted_graph_data
from xinggraph.modules.users.get_user_manager import get_user_manager_context
from xinggraph.infrastructure.databases.relational import get_relational_engine
from xinggraph.modules.users.authentication.default.default_jwt_strategy import DefaultJWTStrategy
from xinggraph.shared.data_models import KnowledgeGraph
from xinggraph.shared.graph_model_utils import graph_schema_to_graph_model
from xinggraph.modules.pipelines.models.PipelineRunInfo import (
    PipelineRunCompleted,
    PipelineRunInfo,
    PipelineRunErrored,
)
from xinggraph.modules.pipelines.queues.pipeline_run_info_queues import (
    get_from_queue,
    initialize_queue,
    remove_queue,
)
from xinggraph.infrastructure.llm.exceptions import LLMPaymentRequiredError
...
from xinggraph.api.DTO import ErrorResponse
...
class CognifyPayloadDTO(InDTO):
    ...
    chunks_per_batch: Optional[int] = Field(
        ...
        description=(
            ...
            "parallelism/throughput; leave null for the pipeline default. Higher values "
            "higher the parallelism/throughput"
        ),
    )
    ...
    chunker: Optional[str] = Field(
        ...
        examples=["automatic"],
        description=(
            "Chunking strategy: 'automatic'/'text' (paragraph-based), 'structured_doc' "
            "(splits on the 'Doc N/M: ... --- 内容开始 --- ... --- 内容结束 ---' wrapper "
            "blocks, one chunk per block), 'langchain', or 'csv'. When 'structured_doc' "
            "is selected and no custom_prompt is provided, the structured-doc extraction "
            "prompt is used automatically."
        ),
    )
...
def get_cognify_router() -> APIRouter:
    ...
    async def cognify(payload: CognifyPayloadDTO, user: User = Depends(get_authenticated_user)):
        ...
        This endpoint is the core of XingGraph's intelligence layer, responsible for converting
        ...
        - **chunk_size** (Optional[int]): Maximum tokens per chunk. If omitted, XingGraph chooses
        ...
        from xinggraph.api.v1.cognify import cognify as xinggraph_cognify
        from xinggraph.api.v1.ontologies.ontologies import OntologyService
        ...
        try:
            ...
            if payload.ontology_key:
                ...
                from xinggraph.modules.ontology.rdf_xml.RDFLibOntologyResolver import (
            ...
            from xinggraph.modules.chunking.registry import build_chunker

            chunker = build_chunker(payload.chunker)

            # structured_doc chunks are wrapped in "Doc N/M: ... --- 内容开始 --- --- 内容结束 ---"
            # markers; without an explicit custom_prompt, use the matching extraction prompt
            # so the LLM reads titles / boundaries before extracting entities.
            if not custom_prompt and payload.chunker and str(payload.chunker).lower() == "structured_doc":
            ...
            cognify_run = await xinggraph_cognify(
                datasets,
                user,
