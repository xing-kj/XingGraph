...
from xinggraph.api.DTO import ErrorResponse, InDTO, OutDTO
...
from xinggraph.modules.users.exceptions.exceptions import PermissionDeniedError, UserNotFoundError
...
def get_search_router() -> APIRouter:
    ...
    async def get_search_history(user: User = Depends(get_authenticated_user)):
        ...
        send_telemetry(
            ...
            user.id,
            additional_properties={"endpoint": "GET /v1/search", "xinggraph_version": xinggraph_version},
        )
    ...
    async def search(payload: SearchPayloadDTO, user: User = Depends(get_authenticated_user)):
        ...
        - **system_prompt** Optional[str]: System prompt to be used for Completion type searches in XingGraph
        ...
        - **only_context** bool: Set to true to only return context XingGraph will be sending to LLM in Completion type searches. This will be returned instead of LLM calls for completion type searches.
        ...
        from xinggraph.api.v1.search import search as xinggraph_search

        try:
            results = await xinggraph_search(
        ...
        except (DatabaseNotCreatedError, UserNotFoundError, XingGraphValidationError) as e:
            status_code = getattr(e, "status_code", status.HTTP_422_UNPROCESSABLE_CONTENT)
            return JSONResponse(
                status_code=status_code,
                content=ErrorResponse(
                    error="Search prerequisites not met, hint: Run `await xinggraph.add(...)` then `await xinggraph.cognify()` before searching.",
                    detail=str(e),
                    # Previous hint not matching "Error Response" structure defined in xinggraph.api.DTO, included in error.
                ).model_dump(),
            )
        except Exception as error:
            return JSONResponse(
