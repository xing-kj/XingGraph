...
from xinggraph import datasets
from xinggraph.api.DTO import InDTO, OutDTO
...
from xinggraph.modules.data.methods.create_authorized_dataset import create_authorized_dataset
from xinggraph.shared.logging_utils import get_logger
from xinggraph.api.v1.exceptions import DataNotFoundError
...
from xinggraph.modules.pipelines.models import PipelineRunStatus
...
def get_datasets_router() -> APIRouter:
    ...
    async def get_dataset_status(
        ...
        from xinggraph.api.v1.datasets.datasets import datasets as xinggraph_datasets

        try:
            # Verify user has permission to read dataset
            authorized_datasets = await get_authorized_existing_datasets(datasets, "read", user)

            datasets_statuses = await xinggraph_datasets.get_status(
    ...
    async def get_raw_data(
        ...
        if parsed_uri.scheme == "s3":
            ...
            from xinggraph.infrastructure.utils.run_async import run_async
        ...
        if parsed_uri.scheme in ("file", "") or (
            ...
            from xinggraph.infrastructure.files.utils.get_data_file_path import get_data_file_path
    ...
    async def get_dataset_schema(dataset_id: UUID, user: User = Depends(get_authenticated_user)):
        ...
        from xinggraph.modules.data.models import DatasetConfiguration
