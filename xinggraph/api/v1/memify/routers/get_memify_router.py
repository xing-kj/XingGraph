...
from xinggraph.modules.pipelines.models import PipelineRunErrored
...
def get_memify_router() -> APIRouter:
    ...
    async def memify(payload: MemifyPayloadDTO, user: User = Depends(get_authenticated_user)):
        ...
        ## Request Parameters
        - **extractionTasks** Optional[List[str]]: List of available XingGraph Tasks to execute for graph/data extraction.
        - **enrichmentTasks** Optional[List[str]]: List of available XingGraph Tasks to handle enrichment of provided graph/data from extraction tasks.
        ...
        send_telemetry(
            ...
            user.id,
            additional_properties={"endpoint": "POST /v1/memify", "xinggraph_version": xinggraph_version},
        )
        ...
        try:
            from xinggraph.modules.memify import memify as xinggraph_memify

            memify_run = await xinggraph_memify(
