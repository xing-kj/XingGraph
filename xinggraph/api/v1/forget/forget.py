...
async def forget(
    *,
    ...
    Usage patterns::
        ...
        await xinggraph.forget(data_id=data_id, dataset_id=dataset_id)

        # Forget an entire dataset (all data + graph nodes + vector entries)
        await xinggraph.forget(dataset="scientists")
        ...
        await xinggraph.forget(dataset="scientists", memory_only=True)

        # Forget only memory for a single file in a dataset
        await xinggraph.forget(dataset="scientists", data_id=data_id, memory_only=True)
    ...
    with new_span("xinggraph.api.forget") as span:
        span.set_attribute(XINGGRAPH_FORGET_TARGET, target)
        if dataset_ref:
            span.set_attribute(XINGGRAPH_DATASET_NAME, str(dataset_ref))
        ...
        if everything:
            ...
            span.set_attribute(XINGGRAPH_RESULT_COUNT, result.get("datasets_removed", 0))
            return result
...
async def _forget_everything(user: Any) -> dict:
    ...
    """
    from xinggraph.api.v1.datasets.datasets import datasets
    ...
    try:
        from xinggraph.infrastructure.databases.cache import get_cache_config
...
async def _forget_dataset_memory(dataset_ref: Union[str, UUID], user: Any) -> dict:
    ...
    from xinggraph.modules.data.models.DatasetData import DatasetData
    from xinggraph.modules.graph.methods.delete_dataset_nodes_and_edges import (
...
async def _forget_data_memory(data_id: UUID, dataset_ref: Union[str, UUID], user: Any) -> dict:
    ...
    from xinggraph.infrastructure.databases.relational import get_relational_engine
    from xinggraph.modules.data.models import Data
    from xinggraph.modules.graph.methods.delete_data_nodes_and_edges import (
...
async def _resolve_dataset_id(dataset_ref: Union[str, UUID], user: Any) -> UUID:
    ...
    from xinggraph.modules.data.methods import get_authorized_dataset_by_name
