...
from xinggraph.infrastructure.data.chunking.config import get_chunk_config
...
from xinggraph.infrastructure.databases.vector.embeddings.config import get_embedding_config
from xinggraph.infrastructure.databases.relational import get_relational_config, get_migration_config
from xinggraph.tasks.translation.config import get_translation_config
from xinggraph.api.v1.exceptions.exceptions import InvalidConfigAttributeError
...
def _coerce_bool(value, name: str) -> bool:
    ...
    the CLI (``xinggraph config set <key> <value>``), which always passes
    strings.
    """
...
class config:
    """
    Configuration namespace for XingGraph's runtime settings.
    ...
    Example:
        ...
        xinggraph.config.set_llm_api_key("your-api-key")
        xinggraph.config.set_llm_model("gpt-4o-mini")
        xinggraph.config.set_embedding_provider("fastembed")
        xinggraph.config.set_embedding_model("BAAI/bge-small-en-v1.5")
        xinggraph.config.set_embedding_dimensions(384)
        xinggraph.config.system_root_directory("/path/to/system")
        xinggraph.config.set_vector_db_provider("lancedb")
        ```
    """
    ...
    def system_root_directory(system_root_directory: str):
        ...
        if vector_config.vector_db_provider == "lancedb":
            vector_config.vector_db_url = os.path.join(databases_directory_path, "xinggraph.lancedb")
    ...
    def set_graph_database_subprocess_enabled(graph_database_subprocess_enabled):
        ...
        xinggraph process. Required for long-running services that otherwise
    ...
    def set_embedding_config(config_dict: dict):
        ...
        -------
            ```python
            xinggraph.config.set_embedding_config({
    ...
    def set_vector_db_subprocess_enabled(vector_db_subprocess_enabled):
        ...
        xinggraph process. Accepts a ``bool`` or a truthy/falsy string; the
    ...
    def set(key: str, value):
        ...
        This enables CLI commands like 'xinggraph config set llm_api_key <value>'.
