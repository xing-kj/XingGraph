...
async def improve(
    ...
    **kwargs: Unpack[ImproveKwargs],
    ...
    Returns:
        Pipeline run info (same as ``xinggraph.memify()``).

    Example::

        # Enrich graph + bridge session feedback and content
        await xinggraph.improve(dataset="docs", session_ids=["chat_1", "chat_2"])

        # Enrich graph only (no session bridging)
        await xinggraph.improve(dataset="docs")
    """
    ...
    with new_span("xinggraph.api.improve") as span:
        span.set_attribute(XINGGRAPH_DATASET_NAME, str(dataset))
        if session_ids:
            span.set_attribute(XINGGRAPH_SESSION_ID, ",".join(session_ids))
        ...
        if session_ids and len(session_ids) == 1:
            from xinggraph.infrastructure.locks import (
        ...
        try:
            ...
            if session_ids:
                ...
                # xinggraph.session.distill_session call.
                ...
                if build_truth_subspace:
                    try:
                        from xinggraph.modules.truth_subspace.build import (
            ...
            from xinggraph.modules.memify import memify
            ...
            span.set_attribute(XINGGRAPH_IMPROVE_STAGES, ",".join(stages_run))

            return result
        finally:
            if acquired_lock_for:
                from xinggraph.infrastructure.locks import release_improve_lock
...
async def _build_global_context_index(
    ...
    from xinggraph.memify_pipelines.global_context_index import global_context_index_pipeline
...
async def _bridge_sessions(
    ...
    from xinggraph.memify_pipelines.apply_feedback_weights import apply_feedback_weights_pipeline
...
async def _extract_agent_context(
    ...
    """
    from xinggraph.infrastructure.session.agent_context_extraction import (
...
async def _distill_sessions(
    ...
    """
    from xinggraph.modules.session_distillation import distill_session
