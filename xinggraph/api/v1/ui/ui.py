...
from xinggraph.version import get_xinggraph_version
...
def get_frontend_cache_dir() -> Path:
    ...
    when the xinggraph package version changes.
    """
    cache_dir = Path.home() / ".xinggraph" / "ui-cache"
...
def get_frontend_download_info() -> Tuple[str, str]:
    """
    Get the download URL and version for the actual xinggraph-frontend source.
    Downloads the real frontend from GitHub releases, matching the installed version.
    """
    version = get_xinggraph_version()
    ...
    download_url = f"https://github.com/xing-kj/xinggraph/archive/refs/tags/v{clean_version}.zip"
...
def download_frontend_assets(force: bool = False) -> bool:
    ...
    if not force and frontend_dir.exists() and version_file.exists():
        try:
            ...
            current_version = get_xinggraph_version()
            ...
            if cached_normalized == current_normalized:
                ...
                logger.info("Updating frontend cache to match current xinggraph version...")
    ...
    logger.info(f"Downloading xinggraph frontend assets for version {version}...")
    logger.info("This will be cached and reused until the xinggraph version changes.")

    try:
        # Create a temporary directory for download
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            archive_path = temp_path / "xinggraph-main.zip"

            # Download the actual xinggraph repository from releases
            logger.info(
                f"Downloading xinggraph v{version.replace('-local', '')} from GitHub releases..."
            )
            ...
            # Extract the archive and find the xinggraph-frontend directory
            if frontend_dir.exists():
                shutil.rmtree(frontend_dir)
            ...
            with zipfile.ZipFile(archive_path, "r") as zip_file:
                ...
                # Find the xinggraph-frontend directory in the extracted content
                # The archive structure will be: xinggraph-{version}/xinggraph-frontend/
                xinggraph_frontend_source = None
                for root, dirs, files in os.walk(extract_dir):
                    if "xinggraph-frontend" in dirs:
                        xinggraph_frontend_source = Path(root) / "xinggraph-frontend"
                        break

                if not xinggraph_frontend_source or not xinggraph_frontend_source.exists():
                    logger.error(
                        "Could not find xinggraph-frontend directory in downloaded release archive"
                    )
                    logger.error("This might indicate a version mismatch or missing release.")
                    return False

                # Copy the xinggraph-frontend to our cache
                shutil.copytree(xinggraph_frontend_source, frontend_dir)
                logger.debug(f"Frontend extracted to: {frontend_dir}")

            # Write version info for future cache validation
            version_file.write_text(version)
            logger.debug(f"Cached frontend for xinggraph version: {version}")

            logger.info(
                f"✓ XingGraph frontend v{version.replace('-local', '')} downloaded and cached successfully!"
            )
            return True
        ...
        if "404" in str(e):
            ...
            logger.error("Try using a stable release version of xinggraph.")
        else:
            logger.error(f"Failed to download from GitHub: {str(e)}")
        logger.error("You can still use xinggraph without the UI functionality.")
        return False
    except Exception as e:
...
def find_frontend_path() -> Optional[Path]:
    """
    Find the xinggraph-frontend directory.
    ...
    dev_search_paths = [
        current_file.parents[4] / "xinggraph-frontend",  # from xinggraph/api/v1/ui/ui.py to project root
        current_file.parents[3] / "xinggraph-frontend",  # fallback path
        current_file.parents[2] / "xinggraph-frontend",  # another fallback
    ]
...
def prompt_user_for_download() -> bool:
    ...
    try:
        print("\n" + "=" * 60)
        print("🎨 XingGraph UI Setup Required")
        print("=" * 60)
        print("The xinggraph frontend is not available on your system.")
        print("This is required to use the web interface.")
        print("\nWhat will happen:")
        print("• Download the actual xinggraph-frontend from GitHub")
        print("• Cache it in your home directory (~/.xinggraph/ui-cache/)")
        print("• Install dependencies with npm (requires Node.js)")
        print("• This is a one-time setup per xinggraph version")
...
def start_ui(
    ...
    """
    Start the xinggraph frontend UI server, optionally with the backend API server and MCP server.

    This function will:
    1. Optionally start the xinggraph backend API server
    2. Optionally start the xinggraph MCP server
    3. Find the xinggraph-frontend directory (development) or download it (pip install)
    ...
    Args:
        ...
        start_backend: If True, also start the xinggraph API backend server (default: False)
        backend_port: Port to run the backend server on (default: 8000)
        start_mcp: If True, also start the xinggraph MCP server (default: False)
    ...
    Example:
        ...
        >>> server = xinggraph.start_ui(dummy_callback)
        >>>
        >>> # Start both frontend and backend
        >>> server = xinggraph.start_ui(dummy_callback, start_backend=True)
        >>> # UI will be available at http://localhost:3000
        >>> # API will be available at http://localhost:8000
        >>>
        >>> # Start frontend with MCP server
        >>> server = xinggraph.start_ui(dummy_callback, start_mcp=True)
        >>> # UI will be available at http://localhost:3000
        >>> # MCP server will be available at http://127.0.0.1:8001/sse
        >>> # To stop all servers later:
        >>> server.terminate()
    """
    logger.info("Starting xinggraph UI...")
    ...
    if not all_ports_available:
        error_msg = f"Cannot start xinggraph UI: The following services have ports already in use: {', '.join(unavailable_services)}"
    ...
    if start_mcp:
        logger.info("Starting XingGraph MCP server with Docker...")
    ...
    if start_mcp:
        try:
            image = "xinggraph/xinggraph-mcp:main"
            ...
            container_name = f"xinggraph-mcp-{uuid.uuid4().hex[:8]}"
            ...
            logger.info(
                f"✓ XingGraph MCP server starting on http://127.0.0.1:{mcp_port}/sse ({mode_info})"
            )
        except Exception as e:
            logger.error(f"Failed to start MCP server with Docker: {str(e)}")
    # Start backend server if requested
    if start_backend:
        logger.info("Starting xinggraph backend API server...")
        try:
            import sys
    ...
    if not frontend_path:
        logger.info("Frontend not found locally. This is normal for pip-installed xinggraph.")
        ...
        if auto_download or prompt_user_for_download():
            ...
            logger.info("You can still use all other xinggraph features without the web interface.")
            return None
    ...
    try:
        ...
        logger.info("✓ XingGraph UI is starting up...")
