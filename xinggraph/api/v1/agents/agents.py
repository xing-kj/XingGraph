...
from xinggraph.modules.data.methods import get_authorized_dataset, get_datasets_by_name
from xinggraph.modules.agents.create_agent import create_agent
from xinggraph.modules.agents.list_agents import list_agents
from xinggraph.modules.agents.get_agent import get_agent
from xinggraph.modules.agents.delete_agent import delete_agent
from xinggraph.modules.agents.agent_mode import register_agent, unregister_agent
from xinggraph.modules.agents.operations import (
    RangeLiteral,
...
def _display_email(email: str) -> str:
    ...
    Internal agent emails are minted as ``{slug}+{parent_id}@xinggraph.agent``;
    this strips the ``+{parent_id}`` segment to produce ``{slug}@xinggraph.agent``.
    """
...
class agents:
    """
    Agent management namespace for XingGraph.
    ...
    Example:
        ...
        agent = await xinggraph.agents.create("my-agent", datasets=["my_project"])

        # List your agents
        all_agents = await xinggraph.agents.list()

        # Register an agent connection
        connection = await xinggraph.agents.register("session-1")
        ```
    """
