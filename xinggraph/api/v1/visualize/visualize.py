...
from xinggraph.shared.logging_utils import get_logger, setup_logging, ERROR
...
async def visualize_graph(
    ...
    async with set_database_global_context_variables(
        ...
        graph_data = await graph_engine.get_graph_data(
            dataset_id=dataset[0].id if dataset else None
        )

        search_events = None
        if include_session_events:
            from xinggraph.modules.visualization.session_events import collect_session_events

            search_events = await collect_session_events(user=user, session_ids=session_ids)

        graph = await xinggraph_network_visualization(
...
async def visualize_multi_user_graph(
    ...
    graph = await xinggraph_network_visualization(graph_data, destination_file_path)
