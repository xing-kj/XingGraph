"""SDK entry point: push a local dataset's knowledge graph to XingGraph Cloud.

Bridges ``xinggraph.export(format="cogx")`` and the remote remember endpoint:
...
Authentication reuses the ``serve`` stack: run ``xinggraph-cli serve`` (or
``await xinggraph.serve()``) once, and ``push`` picks up the saved credentials.
"""
...
@dataclass
class PushResult:
    """Outcome of a ``xinggraph.push()`` call."""
...
def _resolve_client(url: Optional[str], api_key: Optional[str]):
    ...
    ``serve()`` connection → ``XINGGRAPH_SERVICE_URL`` / ``XINGGRAPH_API_KEY``
    environment variables → saved credentials from ``xinggraph-cli serve``.
    """
    from xinggraph.api.v1.serve.cloud_client import CloudClient
    from xinggraph.api.v1.serve.credentials import load_credentials
    from xinggraph.api.v1.serve.state import get_remote_client

    if url:
        return CloudClient(url, api_key or os.getenv("XINGGRAPH_API_KEY", "")), True

    client = get_remote_client()
    if client is not None:
        return client, False

    env_url = os.getenv("XINGGRAPH_SERVICE_URL")
    if env_url:
        return CloudClient(env_url, api_key or os.getenv("XINGGRAPH_API_KEY", "")), True
    ...
    raise RuntimeError(
        "No XingGraph Cloud connection configured. Run `xinggraph-cli serve` to log in, "
        "pass url/api_key, or set XINGGRAPH_SERVICE_URL and XINGGRAPH_API_KEY."
    )
...
def _verify_migration_import(response: dict) -> None:
    ...
    if not any(isinstance(item, dict) and item.get("kind") == "migration_import" for item in items):
        raise RuntimeError(
            ...
            "older XingGraph version without migration support. The uploaded archive was NOT "
            "imported as a knowledge graph (it may have been ingested as a plain file). "
            "Upgrade the remote instance, or use xinggraph.sync()/remember() with raw data."
        )
...
async def push(
    ...
    *,
    ...
    """Upload a local dataset's knowledge graph to a XingGraph Cloud instance.
    ...
    Args:
        ...
        url: Remote instance URL; falls back to the active ``serve()``
            connection, ``XINGGRAPH_SERVICE_URL``, or saved credentials.
    ...
    """
    from xinggraph.api.v1.export.export import export
    from xinggraph.modules.migration.archive import ARCHIVE_SUFFIX, pack_archive
    from xinggraph.modules.migration.sources.base import IMPORT_MODES
    from xinggraph.modules.observability import XINGGRAPH_DATASET_NAME, new_span
    ...
    with new_span("xinggraph.api.push") as span:
        span.set_attribute(XINGGRAPH_DATASET_NAME, str(dataset))
        send_telemetry(
            "xinggraph.push",
            user or "sdk",
            ...
            with tempfile.TemporaryDirectory() as temporary_directory:
                ...
                if result.num_nodes == 0:
                    raise ValueError(
                        ...
                        "Run xinggraph.cognify() (or xinggraph.remember()) on the dataset first to "
