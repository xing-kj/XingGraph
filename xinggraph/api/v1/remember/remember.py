...
from xinggraph.tasks.ingestion.data_item import DataItem
from xinggraph.memory import (
    MemoryEntry,
    QAEntry,
    TraceEntry,
    FeedbackEntry,
...
from xinggraph.memory.entries import MEMORY_ENTRY_TYPES
from xinggraph.modules.migration.sources.base import MemorySource
from xinggraph.modules.pipelines.layers.resolve_authorized_user_datasets import (
...
async def _remember_entry(
    ...
    *,
    ...
    Routes to the remote HTTP client when ``xinggraph.serve(url=...)`` is
    active, otherwise runs the SessionManager call in-process.
    """
    from xinggraph.api.v1.serve.state import get_remote_client
...
async def _dispatch_session_entry(
    ...
    *,
    ...
    if isinstance(entry, SkillRunEntry):
        from xinggraph.modules.tools.skill_runs import remember_skill_run_entry
        ...
        if skill_improvement is not None:
            from xinggraph.modules.memify.skill_improvement import improve_skill_from_config
    ...
    try:
        ...
        from xinggraph.modules.data.methods.get_authorized_dataset import get_authorized_dataset
        from xinggraph.modules.session_lifecycle.metrics import ensure_and_touch_session
...
class RememberResult:
    ...
    Example::
        ...
        result = await xinggraph.remember("Einstein was born in Ulm.")
        print(result)
        ...
        # Background mode:
        result = await xinggraph.remember("data", run_in_background=True)
...
async def remember(
    ...
    **With session_id (session memory):** Stores the data in the
    ...
    if isinstance(data, MemorySource):
        from xinggraph.api.v1.serve.state import get_remote_client
        from xinggraph.modules.migration.import_source import import_memory_source

        if get_remote_client() is not None:
            raise ValueError(
                "remember() cannot import a MemorySource while connected to a remote "
                "XingGraph instance — the import would write to the local graph, not the "
                "remote one. Call xinggraph.disconnect() first to import locally, or use "
                "xinggraph.push() to upload the data to the remote instance."
            )
        ...
        with new_span("xinggraph.api.remember.import") as span:
            span.set_attribute(XINGGRAPH_DATASET_NAME, dataset_name)
            span.set_attribute(XINGGRAPH_OPERATION_MODE, data.mode)
            span.set_attribute("xinggraph.source_system", data.source_system)
            send_telemetry(
                "xinggraph.remember.import",
                kwargs.get("user", "sdk"),
    ...
    with new_span("xinggraph.api.remember") as span:
        span.set_attribute(XINGGRAPH_DATASET_NAME, dataset_name)
        span.set_attribute(XINGGRAPH_OPERATION_MODE, mode)
        span.set_attribute(XINGGRAPH_DATA_SIZE_BYTES, data_size)
        span.set_attribute(XINGGRAPH_DATA_ITEM_COUNT, item_count)
        if session_id:
            span.set_attribute(XINGGRAPH_SESSION_ID, session_id)
...
def _materialize_inline_skill(skills_text, skill_name):
    ...
    tmp = tempfile.TemporaryDirectory(prefix="xinggraph-skills-")
...
async def _remember_inner(
    ...
    **kwargs,
    ...
    client = get_remote_client()
    if client is not None:
        span.set_attribute(XINGGRAPH_OPERATION_MODE, "cloud")
    ...
    from xinggraph.modules.migrations.startup import run_migrations_and_block
    ...
    if content_type == "skills":
        ...
        from xinggraph.modules.engine.operations.setup import setup
        from xinggraph.modules.tools import add_skills
        ...
        if normalized_uploads:
            tmp_dir = tempfile.TemporaryDirectory(prefix="xinggraph-skills-", dir=_Path.cwd())
    ...
    from xinggraph.api.v1.add import add
    from xinggraph.api.v1.cognify import cognify
    ...
    if session_id:
        ...
        if self_improvement:
            from xinggraph.api.v1.improve import improve
    ...
    if run_in_background:
        ...
        from xinggraph.tasks.ingestion.utils import materialize_stream_for_background
