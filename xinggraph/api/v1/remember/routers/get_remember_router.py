...
from xinggraph.memory import QAEntry, TraceEntry, FeedbackEntry, SkillRunEntry
...
def resolve_chunker(chunker: Optional[str]):
    """Resolve a chunker name to its class. Falls back to the default TextChunker."""
    from xinggraph.modules.chunking.registry import build_chunker

    return build_chunker(chunker if chunker and str(chunker).lower() != "automatic" else None)


def maybe_apply_structured_doc_prompt(chunker: Optional[str], custom_prompt: Optional[str]) -> Optional[str]:
    """structured_doc chunks carry "Doc N/M ... --- 内容开始 --- --- 内容结束 ---" wrappers;
    without an explicit custom_prompt, use the matching extraction prompt so the
    LLM reads titles/headers/boundaries before extracting entities."""
    if not custom_prompt and chunker and str(chunker).lower() == "structured_doc":
        from xinggraph.infrastructure.llm.prompts import render_prompt

        return render_prompt("generate_graph_prompt_structured_doc.txt", {})
...
async def _import_cogx_archives(
    ...
    """Import uploaded COGX archive tarballs (produced by ``xinggraph.push()``)."""
    import tarfile
    import tempfile

    from xinggraph.modules.migration import COGXArchiveSource, import_memory_source
    from xinggraph.modules.migration.archive import unpack_archive
    from xinggraph.modules.migration.sources.base import IMPORT_MODES
...
def get_remember_router() -> APIRouter:
    ...
    async def remember(
        ...
        chunker: Optional[str] = Form(
            default=None,
            examples=["automatic"],
            description=(
                "Text chunking strategy: 'automatic'/'text', 'structured_doc', "
                "'langchain', or 'csv'. When 'structured_doc' is used without a "
                "custom_prompt, the structured-doc extraction prompt template is "
        ...
        from xinggraph.api.v1.remember import remember as xinggraph_remember
        ...
        try:
            ...
            result = await xinggraph_remember(
                data,
                ...
                run_in_background=run_in_background or False,
                custom_prompt=maybe_apply_structured_doc_prompt(chunker, custom_prompt or None),
