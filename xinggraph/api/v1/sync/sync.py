...
from xinggraph.tasks.ingestion.ingest_data import ingest_data
...
from xinggraph.modules.sync.methods import (
...
from xinggraph.shared.utils import create_secure_ssl_context
...
async def sync(
    ...
    """
    Sync local XingGraph data to XingGraph Cloud.

    This function handles synchronization of multiple datasets, knowledge graphs, and
    processed data to the XingGraph Cloud infrastructure. It uploads local data for
    ...
    Raises:
        ConnectionError: If XingGraph Cloud service is unreachable
        ...
        while retry_count < MAX_RETRY_COUNT:
            try:
                (
                    ...
                    bytes_uploaded,
                    dataset_sync_hashes,
                ) = await _sync_to_xinggraph_cloud(datasets, user, run_id)
                break
            except Exception as e:
                retry_count += 1
                logger.error(
...
async def _sync_to_xinggraph_cloud(
    datasets: List[Dataset], user: User, run_id: str
) -> tuple[int, int, int, int, dict]:
    """
    Sync local data to XingGraph Cloud using three-step idempotent process:
    ...
    """
    dataset_info = ", ".join([f"{d.name} ({d.id})" for d in datasets])
    logger.info(f"Starting sync to XingGraph Cloud: datasets {dataset_info}")
...
async def _get_cloud_base_url() -> str:
    """Get XingGraph Cloud API base URL.

    Canonical: XINGGRAPH_SERVICE_URL (shared with serve()/push()/MCP).
    XINGGRAPH_CLOUD_API_URL is accepted as a deprecated fallback.
    """
    return (
        os.getenv("XINGGRAPH_SERVICE_URL")
        or os.getenv("XINGGRAPH_CLOUD_API_URL")
        or "http://localhost:8001"
    )


async def _get_cloud_auth_token(user: User) -> str:
    """Get authentication token for XingGraph Cloud API.

    Canonical: XINGGRAPH_API_KEY (shared with serve()/push()/MCP).
    XINGGRAPH_CLOUD_AUTH_TOKEN is accepted as a deprecated fallback.
    """
    return os.getenv("XINGGRAPH_API_KEY") or os.getenv("XINGGRAPH_CLOUD_AUTH_TOKEN") or "your-auth-token"
