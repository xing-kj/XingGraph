...
from xinggraph.api.v1.recall.recall import RecallResponse
...
class RecallPayloadDTO(InDTO):
    ...
    # to opt into auto-routing (the new ``xinggraph.recall`` default).
    search_type: Optional[SearchType] = Field(
        ...
        description=(
            ...
            "Pass null to let xinggraph auto-route the query to the best strategy."
        ),
    )
...
def get_recall_router() -> APIRouter:
    ...
    async def get_recall_history(user: User = Depends(get_authenticated_user)):
        ...
        send_telemetry(
            ...
            user.id,
            additional_properties={"endpoint": "GET /v1/recall", "xinggraph_version": xinggraph_version},
        )
    ...
    async def recall(payload: RecallPayloadDTO, user: User = Depends(get_authenticated_user)):
        ...
        from xinggraph.api.v1.recall import recall as xinggraph_recall

        try:
            results = await xinggraph_recall(
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "hint": "Run `await xinggraph.remember(...)` or `await xinggraph.add(...)` then `await xinggraph.cognify()` before recalling.",
                },
            )
