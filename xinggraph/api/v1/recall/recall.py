...
from xinggraph.context_global_variables import set_session_user_context_variable
from xinggraph.exceptions import XingGraphValidationError
from xinggraph.infrastructure.databases.vector.embeddings.config import EmbeddingConfig
from xinggraph.infrastructure.llm.config import LLMConfig
...
from xinggraph.memory.entries import normalize_scope
...
from xinggraph.modules.recall.types.RecallResponse import (
    RecallResponse,
...
from xinggraph.modules.search.types import SearchResult, SearchType
...
async def _resolve_session_cache_user_id(session_id: str, caller_user_id: str | None) -> str | None:
    ...
    * Falls back to ``caller_user_id`` when nothing is in
    ...
    try:
        ...
        from xinggraph.modules.session_lifecycle.models import SessionRecord
        from xinggraph.modules.users.permissions.methods.get_specific_user_permission_datasets import (
...
async def _search_trace(
    ...
    for entry in entries:
        ...
        try:
            parts.append(json.dumps(mp, ensure_ascii=True))
        except Exception:
            parts.append(str(mp))
        mrv = entry.method_return_value
        try:
            parts.append(json.dumps(mrv, ensure_ascii=True))
        except Exception:
...
async def _fetch_session_context(
    ...
    """
    from xinggraph.infrastructure.session.get_session_manager import get_session_manager
    from xinggraph.infrastructure.session.session_context_builder import build_active_context_block
...
async def recall(
    ...
    *,
    ...
    with new_span("xinggraph.api.recall") as span:
        span.set_attribute(XINGGRAPH_SEARCH_QUERY, query_text[:500])
        span.set_attribute(XINGGRAPH_RECALL_SCOPE, span_scope)
        if session_id:
            span.set_attribute(XINGGRAPH_SESSION_ID, session_id)
        span.set_attribute("xinggraph.recall.top_k", top_k)
        ...
        if client is not None:
            ...
            span.set_attribute(XINGGRAPH_RECALL_SOURCE, "cloud")
            span.set_attribute(XINGGRAPH_RESULT_COUNT, len(results) if results else 0)
            return results
        ...
        async def _run_graph() -> list[RecallResponse]:
            ...
            from xinggraph.modules.recall.methods.normalize_search_payload import (
                normalize_search_payload,
            )
            from xinggraph.modules.search.methods.search import authorized_search
                ...
                try:
                    ...
                    raise XingGraphValidationError(
                        message=(
                            ...
                            "Initialize XingGraph before recalling by:\n"
                            "- running `await xinggraph.add(...)` followed by `await xinggraph.cognify()`."
                        ),
            ...
            if query_type is not None:
                ...
                from xinggraph.api.v1.recall.query_router import route_query
        ...
        if session_result_count:
            span.set_attribute(XINGGRAPH_SESSION_ENTRY_COUNT, session_result_count)

        # Choose a single-source label when only one source contributed,
        # else "multi".
        source_label = sources[0] if len(sources) == 1 else "multi"
        span.set_attribute(XINGGRAPH_RECALL_SOURCE, source_label)
        span.set_attribute(XINGGRAPH_RESULT_COUNT, len(merged))
