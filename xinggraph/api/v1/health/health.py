"""Health check system for xinggraph API."""
...
class HealthChecker:
    ...
    async def check_relational_db(self) -> ComponentHealth:
        ...
        try:
            from xinggraph.infrastructure.databases.relational.get_relational_engine import (
    ...
    async def check_vector_db(self) -> ComponentHealth:
        ...
        start_time = time.time()
        try:
            from xinggraph.infrastructure.databases.vector.get_vector_engine import (
    ...
    async def check_graph_db(self) -> ComponentHealth:
        ...
        start_time = time.time()
        try:
            from xinggraph.infrastructure.databases.graph.get_graph_engine import get_graph_engine
    ...
    async def check_file_storage(self) -> ComponentHealth:
        ...
        try:
            import os
            from xinggraph.infrastructure.files.storage.get_file_storage import get_file_storage
    ...
    async def check_llm_provider(self) -> ComponentHealth:
        ...
        start_time = time.time()
        try:
            from xinggraph.infrastructure.llm.config import get_llm_config

            config = get_llm_config()

            from xinggraph.infrastructure.llm.utils import test_llm_connection
    ...
    async def check_embedding_service(self) -> ComponentHealth:
        ...
        start_time = time.time()
        try:
            from xinggraph.infrastructure.llm.utils import test_embedding_connection
