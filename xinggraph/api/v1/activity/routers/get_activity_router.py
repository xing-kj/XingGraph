...
from xinggraph.modules.users.methods.get_authenticated_user import get_authenticated_user
...
def get_activity_router() -> APIRouter:
    ...
    async def get_pipeline_runs(
        ...
        from xinggraph.modules.pipelines.models import PipelineRun
        from xinggraph.modules.data.models.Dataset import Dataset
        from xinggraph.modules.users.models import User
    ...
    async def get_spans(user: User = Depends(get_authenticated_user)):
        """Return in-memory OTEL spans from the XingGraphSpanExporter buffer."""
        try:
            from xinggraph.modules.observability.tracing import get_exporter
            from xinggraph.modules.observability.trace_context import is_tracing_enabled
    ...
    async def get_tenant_users(user: User = Depends(get_authenticated_user)):
        ...
        try:
            from xinggraph.modules.users.tenants.methods import get_users_in_tenant
    ...
    async def get_agents(user: User = Depends(get_authenticated_user)):
        """Return registered agents (users with @xinggraph.agent emails)."""
        from xinggraph.infrastructure.databases.relational import get_relational_engine
        from xinggraph.modules.users.models import User
        from xinggraph.modules.users.models.UserApiKey import UserApiKey
        from xinggraph.modules.data.models.Data import Data
        from xinggraph.modules.search.models.Query import Query
        from sqlalchemy import select, func
        from datetime import datetime, timedelta, timezone

        db_engine = get_relational_engine()
        async with db_engine.get_async_session() as session:
            # Get all users (agents have @xinggraph.agent, but show all non-default)
        ...
        for u in all_users:
            email = u.email or ""
            is_agent = email.endswith("@xinggraph.agent")
            is_default = email == "default_user@example.com"

            # Parse agent type from email
            # Internal email format: "sanitized-name+{parent_user_id}@xinggraph.agent"
