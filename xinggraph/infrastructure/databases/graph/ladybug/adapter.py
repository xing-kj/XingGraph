...
from xinggraph.infrastructure.utils.run_sync import run_sync
from xinggraph.infrastructure.files.storage import get_file_storage
...
from xinggraph.infrastructure.databases.provenance.source_refs import (
...
if cache_config.shared_ladybug_lock:
    from xinggraph.infrastructure.databases.cache.get_cache_engine import get_cache_engine
...
class LadybugAdapter(GraphDBInterface):
    ...
    def create_subprocess(
        ...
        from xinggraph.infrastructure.databases.graph.kuzu.subprocess.proxy import (
    ...
    def _initialize_connection(self) -> None:
        ...
        # xinggraph_db_workers so the subprocess worker can use the same code
        # without importing xinggraph. Pass the instance's configured limits
        ...
        from xinggraph_db_workers._kuzu_helpers import install_json_extension_local
    ...
    async def push_to_s3(self) -> None:
        if os.getenv("STORAGE_BACKEND", "").lower() == "s3" and hasattr(self, "temp_graph_file"):
            from xinggraph.infrastructure.files.storage.S3FileStorage import S3FileStorage
    ...
    async def query(self, query: str, params: Optional[dict] = None) -> List[Tuple]:
        ...
        with new_span("xinggraph.db.graph.query") as otel_span:
            otel_span.set_attribute(XINGGRAPH_DB_SYSTEM, "ladybug")
            ...
            try:
                ...
                otel_span.set_attribute(XINGGRAPH_DB_ROW_COUNT, len(result))
                return result
            except Exception as e:
    ...
    async def get_id_filtered_graph_data(self, target_ids: list[str]):
        ...
        try:
            ...
            if not all(isinstance(x, str) for x in target_ids):
                raise XingGraphValidationError("target_ids must be a list of strings")
