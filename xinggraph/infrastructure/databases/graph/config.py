...
from xinggraph.root_dir import ensure_absolute_path
...
from xinggraph.infrastructure.databases.graph.kuzu.adapter import (
...
class GraphConfig(BaseSettings):
    ...
    def fill_derived(self):
        ...
        if not self.graph_filename:
            ...
            self.graph_filename = f"xinggraph_graph_{provider}"
            if provider == "ladybug":
                legacy_graph_path = os.path.join(graph_directory, "xinggraph_graph_kuzu")
                if os.path.exists(legacy_graph_path):
                    self.graph_filename = "xinggraph_graph_kuzu"
...
def get_graph_context_config():
    ...
    """
    from xinggraph.context_global_variables import graph_db_config
