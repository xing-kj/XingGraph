...
from xinggraph.modules.users.models import User, DatasetDatabase
...
class Neo4jAuraDevDatasetDatabaseHandler(DatasetDatabaseHandlerInterface):
    """
    Handler for a quick development PoC integration of XingGraph multi-user and permission mode with Neo4j Aura databases.
    This handler creates a new Neo4j Aura instance for each XingGraph dataset created.

    Improvements needed to be production ready:
    - Secret management for client credentials, currently secrets are encrypted and stored in the XingGraph relational database,
