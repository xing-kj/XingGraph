...
from xinggraph.modules.observability import OtelStatusCode as StatusCode
from xinggraph.infrastructure.engine import DataPoint
from xinggraph.modules.engine.utils.generate_timestamp_datapoint import date_to_int
from xinggraph.tasks.temporal_graph.models import Timestamp
from xinggraph.shared.logging_utils import get_logger, ERROR
from xinggraph.infrastructure.databases.graph.graph_db_interface import (
    GraphDBInterface,
)
from xinggraph.infrastructure.databases.exceptions import DatabaseCredentialsError
from xinggraph.modules.storage.utils import JSONEncoder
from datetime import datetime, timezone
from xinggraph.infrastructure.databases.provenance import (
    EdgeDeleteData,
    EdgeIdentity,
    NodeDeleteData,
...
from xinggraph.infrastructure.databases.provenance.source_ref_state import (
...
from xinggraph.modules.observability import new_span
from xinggraph.modules.observability.tracing import (
    XINGGRAPH_DB_SYSTEM,
    XINGGRAPH_DB_QUERY,
...
class Neo4jAdapter(GraphDBInterface):
    ...
    async def query(
        ...
        --------
            ...
            - List[Dict[str, Any]]: A list of dictionaries representing the result of the query
              execution.
        """
        with new_span("xinggraph.db.graph.query") as otel_span:
            otel_span.set_attribute(XINGGRAPH_DB_SYSTEM, "neo4j")
            otel_span.set_attribute(XINGGRAPH_DB_QUERY, redact_secrets(query[:500]))

            try:
                async with self.get_session() as session:
                    result = await session.run(query, parameters=params)
                    data = await result.data()
                    otel_span.set_attribute(XINGGRAPH_DB_ROW_COUNT, len(data))
                    return data
    ...
    async def add_node(self, node: DataPoint):
        ...
        # Native Cypher: no APOC needed.
        # MERGE with backtick-interpolated label, then SET properties.
        # For belongs_to_set: use coalesce + concat to preserve existing tags,
        # then use APOC-free dedup via a second pass (Python-side dedup handles
        # most cases; the MERGE itself is idempotent).
        label = type(node).__name__
        query = dedent(
            f"""MERGE (node: `{BASE_LABEL}`:`{label}`{{id: $node_id}})
    ...
    async def add_nodes(
        ...
        # Group nodes by label, run native Cypher per group (no APOC needed).
        # Each node's label is class-name based (safe for string interpolation).
        ...
        for node in nodes:
            ...
            lbl = props.get("type") or type(node).__name__
            by_label[lbl].append({"node_id": nid, "properties": props})

        results = []
        for label, group in by_label.items():
            query = f"""
            UNWIND $nodes AS node
            MERGE (n: `{BASE_LABEL}`:`{label}`{{id: node.node_id}})
            SET n += node.properties, n.updated_at = timestamp()
            {fold_clause}
            RETURN ID(n) AS internal_id, n.id AS nodeId
            """
            params = {"nodes": group, **provenance_params}
            results.append(self.query(query, params))
    ...
    async def get_edge_delete_data(
        ...
        from xinggraph.modules.graph.utils.prepare_edges_for_storage import get_edge_retrieval_text
    ...
    async def add_edges(
        ...
        # Convert incoming tuples to dicts first, then group by relationship name.
        from collections import defaultdict
        edges = [
            {
                "from_node": str(edge[0]),
        ...
        for edge in edges:
            by_rel[edge["relationship_name"]].append(edge)

        results = []
        for rel_name, group in by_rel.items():
            query = f"""
                UNWIND $edges AS edge
                MATCH (from_node: `{BASE_LABEL}`{{id: edge.from_node}})
                MATCH (to_node: `{BASE_LABEL}`{{id: edge.to_node}})
                MERGE (from_node)-[rel:`{rel_name}`]->(to_node)
                SET rel.source_node_id = edge.from_node,
                    rel.target_node_id = edge.to_node,
                    rel += edge.properties,
                    rel.updated_at = timestamp(),
                    rel.created_at = coalesce(rel.created_at, timestamp())
                {fold_clause}
                RETURN rel"""
            params = {"edges": group, **provenance_params}
            results.append(self.query(query, params))
    ...
    async def get_graph_data(self, dataset_id: Optional[UUID] = None):
        """
        ...
        try:
            ds_id_str = str(dataset_id) if dataset_id is not None else None

            # Retrieve all nodes so the visualization always shows the full
            # graph. Provenance filtering is applied to edges instead so the
            # renderer can color dataset-owned edges without hiding legacy
            # nodes or edges that lack source_dataset_ids.
            query = (
            ...
            nodes = []
            node_internal_ids: List[int] = []
            for record in result:
                node_internal_ids.append(record["id"])
            ...
            # Return the full edge set so no legacy edges are hidden. When a
            # dataset_id is given we also compute `has_provenance` per edge so
            # the frontend can distinguish dataset-owned edges from shared/
            # legacy ones.
            if dataset_id is not None:
                edge_query = f"""
                MATCH (n:`{BASE_LABEL}`)-[r]->(m:`{BASE_LABEL}`)
                RETURN ID(n) AS source, ID(m) AS target, TYPE(r) AS type, properties(r) AS properties,
                       CASE WHEN $dataset_id IN coalesce(r.source_dataset_ids, []) THEN true ELSE false END AS has_provenance
                """
                edge_params = {"dataset_id": ds_id_str}
            else:
                edge_query = f"""
                MATCH (n:`{BASE_LABEL}`)-[r]->(m:`{BASE_LABEL}`)
                RETURN ID(n) AS source, ID(m) AS target, TYPE(r) AS type, properties(r) AS properties,
                ...
                """
                edge_params = {}
            result = await self.query(edge_query, edge_params)

            edges = []
            for record in result:
                raw_props = record["properties"]
                has_prov = record.get("has_provenance", False)
                properties = _strip_provenance(raw_props)
                # Preserve the provenance flag for downstream visualization.
                properties["has_provenance"] = has_prov
                edges.append(
                    (
    ...
    async def get_neighborhood(
        ...
        try:
            ...
            node_dataset_where = ""
            node_params = {"ids": all_ids}
            if dataset_id is not None:
                node_dataset_where = "AND $dataset_id IN coalesce(n.source_dataset_ids, [])"
                node_params["dataset_id"] = str(dataset_id)
            ...
            RETURN n.id AS id, properties(n) AS properties
            """
            nodes_result = await self.query(nodes_query, node_params)
            nodes = []
            ...
            edge_params = {"ids": all_ids}
            edge_dataset_where = ""
            if dataset_id is not None:
                edge_dataset_where = "AND $dataset_id IN coalesce(r.source_dataset_ids, [])"
                edge_params["dataset_id"] = str(dataset_id)
            ...
            """
            edges_result = await self.query(edges_query, edge_params)
            edges = []
    ...
    async def get_id_filtered_graph_data(self, target_ids: list[str], dataset_id: Optional[UUID] = None):
        """
        ...
        try:
            ...
            edge_where = ""
            if dataset_id is not None:
                node_where = "WHERE $dataset_id IN coalesce(n.source_dataset_ids, [])"
                edge_where = "WHERE $dataset_id IN coalesce(r.source_dataset_ids, [])"
    ...
    async def get_nodeset_subgraph(
        self, node_type: Type[Any], node_name: List[str], node_name_filter_operator: str = "OR", dataset_id: Optional[UUID] = None
    ...
    async def get_filtered_graph_data(self, attribute_filters, dataset_id: Optional[UUID] = None):
        """
        ...
        if dataset_id is not None:
            where_clauses.append("$dataset_id IN coalesce(n.source_dataset_ids, [])")
        ...
        edge_where = " AND ".join(where_clauses)
        if dataset_id is not None:
            edge_where += " AND $dataset_id IN coalesce(r.source_dataset_ids, [])"

        query_edges = f"""
        MATCH (n)-[r]->(m)
        WHERE {edge_where.replace("n.", "m.").replace(" AND ", " AND m.")}
