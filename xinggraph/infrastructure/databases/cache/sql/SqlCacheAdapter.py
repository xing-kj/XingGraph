...
from xinggraph.infrastructure.databases.cache.models import SessionAgentTraceEntry, SessionQAEntry
from xinggraph.infrastructure.databases.exceptions.exceptions import (
...
_PURGE_LOCK_ID = int.from_bytes(sha256(b"xinggraph_cache_ttl_sweep").digest()[:8], "big", signed=True)
...
class SqlCacheAdapter(CacheDBInterface):
    ...
    def __init__(
        ...
        try:
            ...
            raise CacheConnectionError(
                ...
                "(CACHE_BACKEND=postgres requires xinggraph[postgres]): " + str(error)
            ) from error
        except Exception as error:
            raise CacheConnectionError(
    ...
    async def prune(self) -> None:
        """
        Empty the cache. Scoped to the four xinggraph cache tables only — a deliberate,
