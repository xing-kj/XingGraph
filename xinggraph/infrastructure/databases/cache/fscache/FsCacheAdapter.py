...
from xinggraph.infrastructure.files.storage.get_storage_config import get_storage_config
...
class FSCacheAdapter(CacheDBInterface):
    ...
    def __init__(self, session_ttl_seconds: int | None = 604800):
        ...
        self.cache_directory = os.path.join(data_root_directory, ".xinggraph_fs_cache", default_key)
    ...
    async def get_qa_entries_by_ids(
        ...
        return [
            ...
            for entry in self._load_entries(session_key)
    ...
    async def prune(self) -> None:
        """
        Remove all items from the cache. In XingGraph, prune means emptying the cache.
