...
class VectorConfig(BaseSettings):
    ...
    def fill_derived(self):
        ...
        if provider == "qdrant" and vector_dataset_database_handler in ("lancedb", "qdrant"):
            self.vector_dataset_database_handler = "qdrant"
        return self
    ...
    def validate_paths(self):
        ...
        if self.vector_db_url and Path(self.vector_db_url).exists():
            ...
            self.vector_db_url = os.path.join(databases_directory_path, "xinggraph.lancedb")
...
def get_vectordb_context_config():
    ...
    from xinggraph.context_global_variables import vector_db_config
