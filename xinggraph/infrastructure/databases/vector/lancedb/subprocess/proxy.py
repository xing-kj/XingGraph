...
and table/builder types the xinggraph adapter uses, but route every call through
a subprocess that only imports ``lancedb`` + ``pyarrow`` (no xinggraph).
"""
...
from xinggraph_db_workers.harness import (
    ReplayStep,
    Request,
    SubprocessSession,
...
from xinggraph_db_workers.lancedb_protocol import (
    OP_CONNECT,
    OP_CREATE_TABLE,
    OP_DROP_TABLE,
    OP_OPEN_TABLE,
...
from xinggraph_db_workers.lancedb_worker import worker_main
...
class RemoteLanceDBTable:
    ...
    xinggraph calls ``get_collection`` (which opens a table) on every operation.
    """
...
class RemoteLanceDBConnection:
    ...
    API used by xinggraph's LanceDBAdapter is exposed.
    """
