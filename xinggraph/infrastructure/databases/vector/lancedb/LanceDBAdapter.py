...
from xinggraph.modules.storage.utils import copy_model
...
class LanceDBAdapter(VectorDBInterface):
    ...
    async def search(
        ...
        with new_span("xinggraph.db.vector.search") as otel_span:
            otel_span.set_attribute(XINGGRAPH_DB_SYSTEM, "lancedb")
            otel_span.set_attribute(XINGGRAPH_VECTOR_COLLECTION, collection_name)
            ...
            otel_span.set_attribute(XINGGRAPH_VECTOR_RESULT_COUNT, len(results))
    ...
    async def remove_belongs_to_set_tags(
        ...
        for collection_name in collection_names:
            ...
            async with self.VECTOR_DB_LOCK:
                ...
                # Ids are UUID strings produced by xinggraph so no escaping
    ...
    def _make_lance_datapoint_cls(cls, payload_schema, vector_size: int):
        ...
        given (payload_schema, vector_size) pair. Memoized globally so xinggraph
