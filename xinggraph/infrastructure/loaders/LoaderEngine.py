...
from xinggraph.infrastructure.files.utils.guess_file_type import guess_file_type
...
class LoaderEngine:
    ...
    Follows xinggraph's adapter pattern similar to database engines,
    ...
    # xinggraph extra that provides the loader. Used to turn an opaque
    ...
    def _no_loader_message(self, file_path: str) -> str:
        ...
        user which extra to install (e.g. ``.pptx`` needs ``xinggraph[docling]``).
        """
        from pathlib import Path
        ...
        if extra:
            message += (
                ...
                f"Install it with `pip install xinggraph[{extra}]` (or `xinggraph[unstructured]`) and retry."
            )
