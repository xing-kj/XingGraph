...
from xinggraph.infrastructure.llm.retry_config import (
...
from xinggraph.infrastructure.llm.structured_output_framework.litellm_instructor.llm.types import (
...
from xinggraph.shared.rate_limiting import llm_rate_limiter_context_manager
...
def _enrich_llm_span(model: str, name: str) -> None:
    ...
    try:
        ...
        from xinggraph.context_global_variables import current_pipeline_stage
        from xinggraph.modules.observability.tracing import (
            XINGGRAPH_LLM_MODEL,
            XINGGRAPH_LLM_PROVIDER,
            XINGGRAPH_PIPELINE_STAGE,
        )

        current_span = otel_trace.get_current_span()
        if current_span and current_span.is_recording():
            current_span.set_attribute(XINGGRAPH_LLM_MODEL, model)
            current_span.set_attribute(XINGGRAPH_LLM_PROVIDER, name)
            stage = current_pipeline_stage.get()
            if stage:
                current_span.set_attribute(XINGGRAPH_PIPELINE_STAGE, stage)
    except Exception:
        pass
