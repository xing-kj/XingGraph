...
from xinggraph.modules.graph.utils.convert_node_to_data_point import get_all_subclasses
from xinggraph.modules.retrieval.base_retriever import BaseRetriever
from xinggraph.modules.retrieval.utils.brute_force_triplet_search import brute_force_triplet_search
from xinggraph.modules.retrieval.utils.global_context import (
    format_global_context_prelude,
    load_root_text,
    search_top_global_context_summaries,
)
from xinggraph.modules.retrieval.utils.used_graph_elements import (
    is_edge_list,
    extract_from_edges,
)
from xinggraph.modules.retrieval.utils.references import append_answer_grounded_evidence
from xinggraph.modules.retrieval.utils.completion import (
    generate_completion,
...
class GraphCompletionRetriever(BaseRetriever):
    ...
    async def get_context_from_objects(
        ...
        print("=" * 60, flush=True)
        print(">>> GRAPH CONTEXT (will be sent to LLM):", flush=True)
        print("=" * 60, flush=True)
        print(graph_context[:3000] if graph_context else "(empty)", flush=True)
        print("=" * 60, flush=True)
