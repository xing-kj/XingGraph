...
from xinggraph.modules.observability import new_span, XINGGRAPH_VECTOR_COLLECTION
...
class NodeEdgeVectorSearch:
    ...
    async def embed_and_retrieve_distances(
        ...
        with new_span("xinggraph.retrieval.vector_search") as span:
            span.set_attribute("xinggraph.vector.collection_count", len(collections))
            span.set_attribute(XINGGRAPH_VECTOR_COLLECTION, ", ".join(collections))
            span.set_attribute(
                "xinggraph.vector.mode", "batch" if query_batch is not None else "single"
            )
            if wide_search_limit is not None:
                span.set_attribute("xinggraph.vector.wide_search_limit", wide_search_limit)

            start_time = time.time()

            if query_batch is not None:
                self.query_list_length = len(query_batch)
                span.set_attribute("xinggraph.vector.batch_size", len(query_batch))
            ...
            span.set_attribute("xinggraph.vector.collections_with_results", collections_with_results)
            span.set_attribute("xinggraph.vector.duration_ms", round(elapsed_time * 1000, 1))
    ...
    async def _embed_query(self, query: str):
        ...
        with new_span("xinggraph.retrieval.embed_query") as span:
            span.set_attribute("xinggraph.vector.query_length", len(query))
            vector_engine = await self._get_vector_engine()
            query_embeddings = await vector_engine.embedding_engine.embed_text([query])
            self.query_vector = query_embeddings[0]
            span.set_attribute("xinggraph.vector.embedding_dimensions", len(self.query_vector))
