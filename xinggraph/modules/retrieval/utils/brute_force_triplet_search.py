...
from xinggraph.modules.graph.exceptions.exceptions import EntityNotFoundError
...
from xinggraph.modules.retrieval.utils.node_edge_vector_search import NodeEdgeVectorSearch
from xinggraph.modules.retrieval.utils.validate_queries import validate_queries
from xinggraph.shared.logging_utils import ERROR, get_logger
from xinggraph.exceptions import XingGraphValidationError

if TYPE_CHECKING:
    from xinggraph.infrastructure.databases.unified import UnifiedStoreEngine
...
async def get_memory_fragment(
    ...
    """Creates and initializes a XingGraphGraph memory fragment with optional property projections."""
    ...
    memory_fragment = XingGraphGraph()
...
async def _get_top_triplet_importances(
    memory_fragment: Optional[XingGraphGraph],
...
async def brute_force_triplet_search(
    ...
    properties_to_project: Optional[List[str]] = None,
    memory_fragment: Optional[XingGraphGraph] = None,
    node_type: Optional[Type] = None,
    node_name: Optional[List[str]] = None,
    node_name_filter_operator: str = "OR",
    ...
    Args:
        ...
        memory_fragment (Optional[XingGraphGraph]): Existing memory fragment to reuse.
    ...
    with new_span("xinggraph.retrieval.triplet_search") as otel_span:
        otel_span.set_attribute("xinggraph.retrieval.top_k", top_k)
        otel_span.set_attribute(
            "xinggraph.retrieval.mode", "batch" if query_batch is not None else "single"
        )
        ...
        otel_span.set_attribute("xinggraph.retrieval.collection_count", len(collections))
        otel_span.set_attribute(XINGGRAPH_VECTOR_COLLECTION, ", ".join(collections))
        ...
        try:
            ...
            if query_batch is not None:
                otel_span.set_attribute("xinggraph.retrieval.batch_size", len(query_batch))

            if not vector_search.has_results():
                otel_span.set_attribute(XINGGRAPH_VECTOR_RESULT_COUNT, 0)
                otel_span.set_attribute(XINGGRAPH_RESULT_SUMMARY, "No vector results found")
            ...
            otel_span.set_attribute(XINGGRAPH_VECTOR_RESULT_COUNT, result_count)
            otel_span.set_attribute(
                XINGGRAPH_RESULT_SUMMARY,
