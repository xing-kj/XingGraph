...
from xinggraph.infrastructure.llm.prompts import render_prompt, read_query_prompt
from xinggraph.modules.observability import new_span, XINGGRAPH_RESULT_SUMMARY
from xinggraph.shared.logging_utils import get_logger

logger = get_logger("generate_completion")
...
async def generate_completion(
    ...
    logger.debug("=" * 60)
    logger.debug(">>> SYSTEM: %s", system_prompt[:500] if system_prompt else "(empty)")
    logger.debug(">>> USER: %s", user_prompt[:3000] if user_prompt else "(empty)")
    logger.debug("=" * 60)
    ...
    with pipeline_stage("query"):
        with new_span("xinggraph.llm.completion") as span:
            span.set_attribute("xinggraph.llm.prompt_path", system_prompt_path)
            span.set_attribute("xinggraph.llm.context_length", len(context))
            span.set_attribute("xinggraph.llm.query_length", len(query))
            ...
            if isinstance(result, str):
                span.set_attribute("xinggraph.llm.response_length", len(result))
            span.set_attribute(XINGGRAPH_RESULT_SUMMARY, "LLM completion generated")
            return result
...
async def summarize_text(
    ...
    with new_span("xinggraph.llm.summarize") as span:
        span.set_attribute("xinggraph.llm.input_length", len(text))
        result = await LLMGateway.acreate_structured_output(
            text_input=text,
            system_prompt=system_prompt,
            response_model=str,
        ...
        if isinstance(result, str):
            span.set_attribute("xinggraph.llm.response_length", len(result))
        span.set_attribute(XINGGRAPH_RESULT_SUMMARY, "Text summarized")
        return result
