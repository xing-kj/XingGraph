...
from xinggraph.modules.engine.models import Skill, Tool
from xinggraph.modules.engine.models.SkillRun import ToolCall as SkillRunToolCall
...
from xinggraph.modules.tools.errors import (
    ToolInvocationError,
    ToolPermissionError,
    ToolScopeError,
)
from xinggraph.modules.tools.execute_tool import execute_tool
from xinggraph.modules.tools.registry import list_tools_for_dataset
from xinggraph.modules.tools.resolve_skills import resolve_skills
...
class AgentStep(BaseModel):
    """One LLM turn: either request a tool call, ask for clarification, or emit the final answer."""
    ...
    clarification_request: Optional[str] = Field(
        default=None,
        description=(
            "Populate when the query is ambiguous or you lack sufficient information to proceed. "
            "Ask a brief clarifying question and stop the loop."
        ),
    )
...
class AgenticRetriever(GraphCompletionRetriever):
    ...
    async def _record_skill_runs(
        ...
        *,
        ...
        from xinggraph.modules.engine.models.SkillRun import (
            CandidateSkill,
            SkillRun,
            UNSCORED_SKILL_RUN_SCORE,
        )
        from xinggraph.modules.engine.utils.generate_node_id import generate_node_id
        ...
        if user is not None and dataset is not None and dataset_id is not None:
            ctx = PipelineContext(
                ...
                data_item=SimpleNamespace(
                    id=uuid5(NAMESPACE_URL, f"xinggraph:skill-runs:{dataset_id}:{uuid4()}")
                ),
    ...
    async def _maybe_active_context_block(self, query: Optional[str]) -> tuple[str, list]:
        ...
        try:
            from xinggraph.infrastructure.databases.cache.config import CacheConfig
            from xinggraph.infrastructure.session.get_session_manager import get_session_manager
            from xinggraph.infrastructure.session.session_context_builder import (
    ...
    async def _run_tool_loop(
        ...
        for iteration in range(self.max_iter):
            ...
            if step.clarification_request and step.clarification_request.strip():
                return step.clarification_request
