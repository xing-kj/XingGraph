"""Ingest an RDF T-Box + A-Box into the xinggraph memory graph, preserving URIs.

This is the "complementary layer" bridge: point xinggraph at an RDF knowledge base
...
- The full external IRI is preserved verbatim on ``DataPoint.ontology_uri`` so
  ...
  ``xinggraph.modules.graph.rdf.export``).
...
returns xinggraph ``DataPoint``s plus explicit RDF custom edges.
...
from xinggraph.modules.engine.utils import generate_edge_name
...
def build_graph_from_rdf(rdf_graph: Graph) -> RDFIngestGraph:
    """Turn a parsed RDF graph into xinggraph nodes plus RDF object-property edges.
