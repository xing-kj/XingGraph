...
from xinggraph.modules.engine.models import NodeSet, Skill
...
from xinggraph.modules.tools.path_safety import trusted_is_dir, trusted_is_file, trusted_rglob
from xinggraph.shared.logging_utils import get_logger
from xinggraph.tasks.storage.add_data_points import add_data_points


logger = get_logger("xinggraph.tools.ingest_skills")

SKILL_SOURCE_ROOTS_ENV = "XINGGRAPH_SKILL_SOURCE_ROOTS"
...
def _skill_source_data_id(dataset_id: UUID, source: Path) -> UUID:
    return uuid5(NAMESPACE_URL, f"xinggraph:skills:{dataset_id}:{_normalize_skill_path(source)}")


def _scoped_skill_id(dataset_id: UUID, skill: Skill) -> UUID:
    return uuid5(
        NAMESPACE_URL,
        f"xinggraph:skill:{dataset_id}:{skill.source_dir}:{skill.name}",
    )
...
async def add_skills(
    ...
    *,
    ...
    from xinggraph.modules.tools.skill_parser import parse_skill_file, parse_skills_folder
