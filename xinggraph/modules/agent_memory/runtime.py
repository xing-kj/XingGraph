...
from xinggraph.modules.users.permissions.methods import get_all_user_permission_datasets
from xinggraph.shared.logging_utils import get_logger
from xinggraph.modules.agent_memory.sanitization import (
...
@dataclass(slots=True)
class AgentScope:
    """Authorized dataset scope resolved for XingGraph-backed memory retrieval."""
...
def validate_agent_memory_config(
    *,
    ...
    if not isinstance(with_memory, bool):
        raise XingGraphValidationError("with_memory must be a boolean.", log=False)
    if not isinstance(with_session_memory, bool):
        raise XingGraphValidationError("with_session_memory must be a boolean.", log=False)
    if not isinstance(save_session_traces, bool):
        raise XingGraphValidationError("save_session_traces must be a boolean.", log=False)
    if not isinstance(memory_only_context, bool):
        raise XingGraphValidationError("memory_only_context must be a boolean.", log=False)
    if not isinstance(session_trace_summary, bool):
        raise XingGraphValidationError("session_trace_summary must be a boolean.", log=False)
    ...
    if memory_query_fixed is not None and not isinstance(memory_query_fixed, str):
        raise XingGraphValidationError("memory_query_fixed must be a string when provided.", log=False)
    ...
    if memory_query_fixed is not None and memory_query_from_method is not None:
        raise XingGraphValidationError(
            "Only one of memory_query_fixed or memory_query_from_method can be provided to xinggraph.agent_memory.",
            log=False,
        )
    ...
    if not cache_config.caching and (
        ...
        raise XingGraphValidationError(
            (
                ...
                "or persist_session_trace_after with xinggraph.agent_memory."
            ),
            log=False,
        )
    if persist_session_trace_after is not None and (
    ...
    if user is not None and not hasattr(user, "id"):
        raise XingGraphValidationError("user must have an id attribute.", log=False)
...
async def resolve_agent_dataset_scope(config: AgentMemoryConfig, resolved_user: User) -> AgentScope:
    """Resolve the dataset scope for XingGraph search using a user with read and write access."""
    ...
    if not matching_datasets:
        raise XingGraphValidationError(
            (
                ...
                f"{requested_dataset_name!r} to use xinggraph.agent_memory."
            ),
            log=False,
        )
...
async def retrieve_memory_context(context: AgentMemoryContext) -> str:
    ...
    xinggraph_memory = await retrieve_xinggraph_memory_context(context)
    if xinggraph_memory:
        memory_parts.append(f"Relevant XingGraph Memory:\n{xinggraph_memory}")
...
async def retrieve_xinggraph_memory_context(context: AgentMemoryContext) -> str:
    """Fetch dataset-backed XingGraph search memory when enabled."""
    ...
    with new_span("xinggraph.agent_memory.retrieve") as span:
        span.set_attribute("xinggraph.agent_memory.query_length", len(query_text))
        try:
            from xinggraph.api.v1.search import SearchType, search
            ...
            span.set_attribute("xinggraph.agent_memory.retrieval_failed", True)
            return ""

        memory_context = truncate_text(normalize_search_results(results), MAX_MEMORY_CONTEXT_LENGTH)
        span.set_attribute("xinggraph.agent_memory.context_length", len(memory_context))
...
def normalize_search_results(results: Any) -> str:
    ...
    if isinstance(results, dict):
        ...
        return json.dumps(sanitize_value(results), ensure_ascii=True)
