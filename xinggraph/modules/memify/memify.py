...
from xinggraph.modules.retrieval.utils.brute_force_triplet_search import get_memory_fragment
...
from xinggraph.memify_pipelines.memify_default_tasks import (
...
async def memify(
    ...
    """
    Enrichment pipeline in XingGraph, can work with already built graphs. If no data is provided existing knowledge graph will be used as data,
    custom data can also be provided instead which can be processed with provided extraction and enrichment tasks.

    Provided tasks and data will be arranged to run the XingGraph pipeline and execute graph enrichment/creation.
    ...
    Args:
        extraction_tasks: List of XingGraph Tasks to execute for graph/data extraction.
        enrichment_tasks: List of XingGraph Tasks to handle enrichment of provided graph/data from extraction tasks.
    ...
    Example:
        ...
        await xinggraph.add("Your document content")
