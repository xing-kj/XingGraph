...
from xinggraph.modules.engine.models import NodeSet, Skill, SkillImprovementProposal, SkillRun
...
from xinggraph.modules.tools.resolve_skills import find_skill_by_id, find_skill_by_name
from xinggraph.shared.logging_utils import get_logger
from xinggraph.tasks.storage.add_data_points import add_data_points


logger = get_logger("xinggraph.skill_improvement")
...
def _storage_context(user, dataset, key: str) -> Optional[PipelineContext]:
    ...
    return PipelineContext(
        user=user,
        dataset=dataset,
        data_item=SimpleNamespace(id=uuid5(NAMESPACE_URL, f"xinggraph:skill-improvement:{key}")),
...
async def improve_skill(
    ...
    *,
    ...
    This is intentionally internal. Callers opt in through ``xinggraph.remember``
    ...
    async with set_database_global_context_variables(dataset_id, owner_id):
        ...
        try:
            from xinggraph.infrastructure.llm import get_llm_config
