"""OpenTelemetry-native tracing for XingGraph.

Provides XingGraphSpanExporter (in-memory span buffer), setup_tracing(),
get_tracer(), XingGraphTrace, semantic attribute constants, and redaction.
"""
...
# ---------------------------------------------------------------------------
XINGGRAPH_DB_SYSTEM = "xinggraph.db.system"
XINGGRAPH_DB_QUERY = "xinggraph.db.query"
XINGGRAPH_DB_ROW_COUNT = "xinggraph.db.row_count"
XINGGRAPH_LLM_MODEL = "xinggraph.llm.model"
XINGGRAPH_LLM_PROVIDER = "xinggraph.llm.provider"
XINGGRAPH_PIPELINE_STAGE = "xinggraph.pipeline.stage"
XINGGRAPH_SEARCH_TYPE = "xinggraph.search.type"
XINGGRAPH_SEARCH_QUERY = "xinggraph.search.query"
XINGGRAPH_PIPELINE_TASK_NAME = "xinggraph.pipeline.task_name"
XINGGRAPH_VECTOR_COLLECTION = "xinggraph.vector.collection"
XINGGRAPH_VECTOR_RESULT_COUNT = "xinggraph.vector.result_count"
XINGGRAPH_SPAN_CATEGORY = "xinggraph.span.category"
XINGGRAPH_RESULT_SUMMARY = "xinggraph.result.summary"
XINGGRAPH_RESULT_COUNT = "xinggraph.result.count"
XINGGRAPH_PIPELINE_NAME = "xinggraph.pipeline.name"

# V2 API attributes
XINGGRAPH_DATASET_NAME = "xinggraph.dataset.name"
XINGGRAPH_SESSION_ID = "xinggraph.session.id"
XINGGRAPH_SESSION_ENTRY_COUNT = "xinggraph.session.entry_count"
XINGGRAPH_DATA_SIZE_BYTES = "xinggraph.data.size_bytes"
XINGGRAPH_DATA_ITEM_COUNT = "xinggraph.data.item_count"
XINGGRAPH_OPERATION_MODE = "xinggraph.operation.mode"  # "session", "permanent", "cloud"
XINGGRAPH_RECALL_SCOPE = "xinggraph.recall.scope"  # "session", "graph", "auto"
XINGGRAPH_RECALL_SOURCE = "xinggraph.recall.source"  # where results came from
XINGGRAPH_FORGET_TARGET = "xinggraph.forget.target"  # "dataset", "data_item", "everything"
XINGGRAPH_IMPROVE_STAGES = "xinggraph.improve.stages"
XINGGRAPH_GRAPH_EDGES_SYNCED = "xinggraph.graph.edges_synced"  # compatibility; no longer emitted
...
def _check_otel_available() -> None:
    if not _OTEL_AVAILABLE:
        raise ImportError(
            ...
            "Install them with: pip install xinggraph[tracing]"
        )
...
class XingGraphSpanExporter(_ExporterBase):
...
# ---------------------------------------------------------------------------
# XingGraphTrace -- lightweight view over collected spans
# ---------------------------------------------------------------------------
class XingGraphTrace:
...
# Global state
# ---------------------------------------------------------------------------
_exporter: Optional["XingGraphSpanExporter"] = None
    ...
    try:
        ...
        try:
            ...
            warnings.warn(
                ...
                "Install with: pip install xinggraph[tracing]",
                stacklevel=2,
            )


def setup_tracing(console_output: bool = False) -> "trace.Tracer":
    """Set up OTEL tracing for XingGraph.
    ...
    attaches XingGraph's in-memory exporter to it instead of replacing it.
    ...
    _exporter = XingGraphSpanExporter()

    if _is_auto_instrumented():
        # An external tool already set up a provider — reuse it.
        _provider = trace.get_tracer_provider()
        # Attach our in-memory exporter so XingGraphTrace still works.
    ...
    _tracer = _provider.get_tracer("xinggraph", version)
    return _tracer


def get_tracer() -> Optional["trace.Tracer"]:
    """Return the xinggraph OTEL tracer, or None if not set up."""
    return _tracer
...
def get_exporter() -> Optional["XingGraphSpanExporter"]:
