...
def enable_tracing(console_output: bool = False) -> None:
    """Enable OpenTelemetry tracing for XingGraph.

    Sets up a TracerProvider with an in-memory XingGraphSpanExporter.
...
def is_tracing_enabled() -> bool:
    ...
    Checks the module-level flag, then the ``xinggraph_tracing_enabled`` config
    field, then falls back to the ``XINGGRAPH_TRACING_ENABLED`` env var directly
    ...
    enabled = config.xinggraph_tracing_enabled or os.environ.get(
        "XINGGRAPH_TRACING_ENABLED", ""
    ).lower() in ("true", "1", "yes")
...
def get_last_trace() -> Optional[XingGraphTrace]:
    ...
    if spans is None:
        return None
    return XingGraphTrace(spans)


def get_all_traces() -> list[XingGraphTrace]:
    """Return all traces currently held in the in-memory buffer."""
    exporter = get_exporter()
    if exporter is None:
        return []
    all_traces = exporter.get_all_traces()
    return [XingGraphTrace(spans) for spans in all_traces.values()]
