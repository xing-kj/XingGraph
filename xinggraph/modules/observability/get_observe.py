...
def _wrap_with_otel(inner_decorator):
    ...
    a ``xinggraph.span.category`` attribute on the span.
    """
    ...
    def otel_observe(*dec_args, **dec_kwargs):
        ...
        if not (len(dec_args) == 1 and callable(dec_args[0]) and not dec_kwargs):
            ...
            def outer(func):
                ...
                async def async_wrapper(*args, **kwargs):
                    ...
                    with tracer.start_as_current_span(f"xinggraph.observe.{func.__name__}") as span:
                        span.set_attribute(XINGGRAPH_SPAN_CATEGORY, category)
                        return await wrapped(*args, **kwargs)
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            from xinggraph.modules.observability.tracing import get_tracer

            tracer = get_tracer()
            if tracer is None:
                return await wrapped(*args, **kwargs)

            with tracer.start_as_current_span(f"xinggraph.observe.{func.__name__}"):
                return await wrapped(*args, **kwargs)
