...
var STORY_STAGE_ORDER=["document","chunk","wiki","entity","type","summary","context","schema","other"];
...
function recolorNodes(){
  nodes.forEach(function(n){
    ...
    // is_product endpoints stay gold in every color-by mode so product
    // relationships never disappear when the user re-colors the graph.
    if(n.product_linked===true){
      n.color="#FFB020";
      n._rgb=[255,176,32];
    }
...
function updateLegend(){
  ...
  if(colorByMode==="type"){
    ...
    // ontology-valid override and is_product endpoint override) so the legend
    // can never diverge from the canvas — the JS palette and the preprocessor
    // palette had drifted apart.
    colorSource=function(t){
      for(var i=0;i<nodes.length;i++){
        var n=nodes[i];
        if(n.type===t&&n.ontology_valid!==true&&n.product_linked!==true&&n.color)return n.color;
      }
  ...
  var ontologySection=
    ...
    '</div>';
  // Product section — only when the graph actually contains is_product
  // endpoints, so we don't advertise a cue that isn't present.
  var productSection="";
  if(nodes.some(function(n){return n.product_linked===true})){
    productSection=
      '<div class="legend-section">'+
        '<span class="legend-title">Product</span>'+
        '<span class="legend-item"><span class="legend-dot" style="background:#FFB020"></span>is_product endpoint</span>'+
      '</div>';
  }
...
function inspectorOverviewLine(n){
  ...
  if(stage==="document"){
    return "Top-level document. XingGraph extracted "+children+" child node(s) from it.";
  }
  if(stage==="chunk"){
    var doc=null;
  ...
  if(layers.edges){
    ...
    if(!skipEdges){
      ...
      links.forEach(function(l){
        ...
        else if(l.relation==="is_product"){isProductEdges.push(entry)}
        else if(l.has_provenance){normalProvEdges.push(entry)}
        else{normalLegacyEdges.push(entry)}
      });

      // Draw provenance edges (dataset-scoped, colored)
      if(normalProvEdges.length>0){
        var pAlpha=hoveredNode?0.02:hasSearch?0.02:0.25;
        ctx.beginPath();
        normalProvEdges.forEach(function(e){
          ctx.moveTo(e.l.source.x,e.l.source.y);
          ctx.lineTo(e.l.target.x,e.l.target.y);
        });
        ctx.strokeStyle=_light ? "rgba(0,100,220,"+Math.min(pAlpha*3,0.45)+")" : "rgba(100,180,255,"+pAlpha+")";
        ctx.lineWidth=(_light ? 1.0 : 0.7)/scale;
        ctx.stroke();
      }

      // Draw legacy edges (no provenance, subtle grey)
      if(normalLegacyEdges.length>0){
        var lAlpha=hoveredNode?0.02:hasSearch?0.02:0.06;
        ctx.beginPath();
        normalLegacyEdges.forEach(function(e){
          ctx.moveTo(e.l.source.x,e.l.source.y);
          ctx.lineTo(e.l.target.x,e.l.target.y);
        });
        ctx.strokeStyle=_light ? "rgba(80,80,80,"+Math.min(lAlpha*3,0.2)+")" : "rgba(219,216,216,"+lAlpha+")";
        ctx.lineWidth=(_light ? 0.5 : 0.4)/scale;
        ctx.stroke();
      }

      // Draw is_product edges (prominent orange)
      if(isProductEdges.length>0){
        var ipAlpha=hoveredNode?0.15:hasSearch?0.15:0.65;
        ctx.beginPath();
        isProductEdges.forEach(function(e){
          ctx.moveTo(e.l.source.x,e.l.source.y);
          ctx.lineTo(e.l.target.x,e.l.target.y);
        });
        ctx.strokeStyle=_light ? "rgba(210,75,0,"+ipAlpha+")" : "rgba(255,140,60,"+ipAlpha+")";
