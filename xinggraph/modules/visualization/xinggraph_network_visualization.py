"""XingGraph graph visualization — orchestrator.

Single entry point ``xinggraph_network_visualization(graph_data, ...)``:
...
Phase 1c refactor described in the design plan. It
preserves the public API for backward compatibility — every existing
caller of ``xinggraph_network_visualization`` or
...
from xinggraph.infrastructure.files.storage.LocalFileStorage import LocalFileStorage
from xinggraph.modules.visualization.preprocessor import preprocess
from xinggraph.modules.visualization.views import (
    inspector,
    memory_map,
    schema_view,
    semantic_map,
...
from xinggraph.modules.visualization.layouts import pipeline_layout, semantic_layout
from xinggraph.modules.visualization.embedding_join import fetch_node_embeddings, select_nodes
from xinggraph.modules.visualization.semantic_clusters import compute_clusters
...
async def xinggraph_network_visualization(
    graph_data,
    ...
    Args:
        ...
        search_events: Optional list of operation events shown on the Memory
            ...
            ``xinggraph.modules.visualization.session_events``; pass them
