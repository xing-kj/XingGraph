...
# Nodes joined by an is_product edge (product family <-> product model) get a
# warm gold fill so product relationships stand out against the purple/type
# palette and the pink ontology-valid marker.
_PRODUCT_LINKED_COLOR = "#FFB020"
    ...
    for key in ("summary", "description", "content", "title", "text"):
        ...
        if isinstance(value, str) and value.strip() and not looks_like_identifier(value):
            ...
            for word in words:
                ...
                length += len(word) + 1
            return " ".join(truncated) or normalized[:100]
...
def build_operation_layer(
    ...
    """
    from xinggraph.modules.visualization.operations_catalog import get_operations_catalog
...
class PreprocessedGraph:
    """Renderer-facing snapshot of a xinggraph graph.
...
def preprocess(graph_data, schema_data: Optional[Dict[str, Any]] = None) -> PreprocessedGraph:
    ...
    Mirrors the data shape the existing ``xinggraph_network_visualization``
    ...
    for edge in edges_data:
        ...
        has_provenance = bool(edge_info.get("has_provenance")) if edge_info else False
    ...
    # ── Nodes pass 4: is_product endpoint highlight ────────────────────────
    # Nodes joined by an is_product edge (product family <-> product model)
    # get a distinct fill so product relationships stand out in the graph.
    product_endpoint_ids = {
        node_id
        for link in links
        if link.get("relation") == "is_product"
        for node_id in (str(link.get("source")), str(link.get("target")))
    }
    if product_endpoint_ids:
        for node in nodes:
            if node["id"] in product_endpoint_ids:
                node["product_linked"] = True
                node["color"] = _PRODUCT_LINKED_COLOR
