...
``xinggraph.search`` routes to retrievers that produce wildly different
...
from xinggraph.modules.search.types.SearchType import SearchType
...
class SearchResultItem(BaseModel):
    ...
    # Retrieval debug surface: the exact context handed to the LLM and the
    # structured retrieval chain (populated by retrievers that record a trace,
    # e.g. WIKI_COMPLETION). Both are optional and omitted for plain retrievers.
    context: str | None = None
    trace: dict[str, Any] | None = None

    structured: Any | None = None

    # HITL: when set, this item is a clarification question from the retriever
    # rather than a normal answer. Frontends may render it with a distinct style.
