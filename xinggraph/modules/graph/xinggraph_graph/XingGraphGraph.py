...
from xinggraph.modules.graph.exceptions import (
    EntityNotFoundError,
    InvalidDimensionsError,
)
from xinggraph.infrastructure.databases.graph.graph_db_interface import GraphDBInterface
from xinggraph.modules.graph.xinggraph_graph.XingGraphGraphElements import Node, Edge
from xinggraph.modules.graph.xinggraph_graph.XingGraphAbstractGraph import XingGraphAbstractGraph
from xinggraph.base_config import get_base_config
import heapq

logger = get_logger("XingGraphGraph")


class XingGraphGraph(XingGraphAbstractGraph):
    """
    Concrete implementation of the AbstractGraph class for XingGraph.
    ...
    async def _get_nodeset_subgraph(self, adapter, node_type, node_name, node_name_filter_operator, dataset_id: Optional[UUID] = None):
    ...
    async def _get_full_or_id_filtered_graph(
        ...
        if relevant_ids_to_filter is None:
            ...
            nodes_data, edges_data = await adapter.get_graph_data(dataset_id=dataset_id)
            if not nodes_data or not edges_data:
        ...
        if getattr(adapter.__class__, "get_id_filtered_graph_data", None):
            ...
            nodes_data, edges_data = await get_graph_data_fn(target_ids=relevant_ids_to_filter, dataset_id=dataset_id)
        else:
            logger.info("Retrieving full graph from database.")
            nodes_data, edges_data = await get_graph_data_fn(dataset_id=dataset_id)
        ...
        try:
            if node_type is not None and node_name not in [None, [], ""]:
                ...
                nodes_data, edges_data = await self._get_full_or_id_filtered_graph(
                    adapter, relevant_ids_to_filter, dataset_id=dataset_id
                )
            else:
                nodes_data, edges_data = await self._get_filtered_graph(
                    adapter, memory_fragment_filter, dataset_id=dataset_id
                )
