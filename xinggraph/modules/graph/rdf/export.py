"""Export the xinggraph memory graph as RDF and query it with SPARQL.

xinggraph stores knowledge in a labeled property graph (Kuzu/Neo4j/etc.). This
...
un-grounded nodes get a minted xinggraph IRI under ``DEFAULT_BASE_IRI`` so the RDF
stays well-formed. RDF-ingested object-property edges that carry
``predicate_uri`` are emitted with that original RDF predicate IRI. XingGraph-native
edges without ``predicate_uri`` keep minted xinggraph predicate IRIs. Nothing is
...
# Base IRI for minted (non-ontology-grounded) xinggraph identifiers.
DEFAULT_BASE_IRI = "https://xinggraph.ai/graph/"
...
# xinggraph predicate. ``is_a`` links an individual to its class (rdf:type) or a
...
def graph_data_to_rdf(
    ...
    """
    g = Graph()
    g.bind("xinggraph", Namespace(base_iri))
