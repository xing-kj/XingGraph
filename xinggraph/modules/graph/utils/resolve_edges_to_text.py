...
from xinggraph.modules.retrieval.utils.stop_words import DEFAULT_STOP_WORDS
...
async def resolve_edges_to_text(retrieved_edges: List[Edge]) -> str:
    ...
    # Enrich with ontology reasoning chains if available
    reasoning_chains = []
    for node_info in nodes.values():
        node = node_info["node"]
        chain = node.attributes.get("ontology_reasoning_chain")
        if chain:
            reasoning_chains.append(f"{node_info['name']}: {chain}")
