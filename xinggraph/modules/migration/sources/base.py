...
produced by one) into a stream of COGX records. ``xinggraph.remember()`` accepts
...
from xinggraph.modules.migration.cogx import COGXRecord
...
class MemorySource(ABC):
    ...
    Args:
        mode: Import fidelity mode.
            - ``"re-derive"`` (default): ingest raw content (episodes,
              documents, memories) and run XingGraph's own extraction (cognify).
    ...
    # XingGraph-origin archives only (COGXArchiveSource sets it from
    ...
    # XingGraph-origin archives only (COGXArchiveSource sets it from the
