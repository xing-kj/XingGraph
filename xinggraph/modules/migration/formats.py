...
def write_graphml(nodes: List[Node], edges: List[Edge], destination: Path) -> None:
    ...
    lines.append('  <graph id="xinggraph" edgedefault="directed">')
...
def write_cypher(nodes: List[Node], edges: List[Edge], destination: Path) -> None:
    ...
    Every node gets the shared ``:XingGraphNode`` label (its sanitized ``type``
    becomes a secondary label), and an index on ``(:XingGraphNode).id`` is created
    up front so per-edge MATCH clauses are index lookups instead of full scans.
    """
    lines = [
        "// XingGraph graph export — load with cypher-shell or neo4j browser",
