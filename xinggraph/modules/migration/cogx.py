"""COGX — the XingGraph eXchange format for portable memory.
...
ingests COGX into XingGraph, and a single dumper exports XingGraph datasets into a
...
class COGXManifest(BaseModel):
    ...
    # XingGraph-origin archives only: the source store's stamped data-migration
...
def validate_cogx_version(version: str) -> None:
    ...
    if archive_major > current_major:
        raise ValueError(
            ...
            f"supports ({COGX_VERSION}). Upgrade xinggraph to import it."
        )
...
class COGXArchiveWriter:
    ...
    def __init__(self, directory: Union[str, Path], source_system: str = "xinggraph"):
