"""Export a XingGraph dataset's knowledge graph to portable formats.
...
from xinggraph.modules.migration.formats import write_cypher, write_graphml, write_json
from xinggraph.modules.migration.snapshot import GraphSnapshot, build_snapshot
...
async def _gather_social_layer(dataset_obj) -> Dict[str, Any]:
    ...
    xinggraph-to-xinggraph restore can recreate functional accounts — which makes
    ...
    from xinggraph.modules.users.models import ACL, Permission, User
...
def _write_cogx(
    ...
    with COGXArchiveWriter(destination, source_system="xinggraph") as writer:
        writer.add_note(f"Exported from XingGraph dataset {dataset_name!r}.")
...
async def export_dataset(
    ...
    ``permissions.json`` so a xinggraph-to-xinggraph restore recreates functional
    ...
    if format == "cogx":
        ...
        try:
            from xinggraph.infrastructure.databases.vector.embeddings.config import (
