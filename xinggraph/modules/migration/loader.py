"""Translate COGX records into XingGraph ingestion inputs.
...
- **data items** (``re-derive``): textual records become :class:`DataItem`s
  ...
  path so XingGraph's own extraction builds the graph.
...
from xinggraph.modules.migration.snapshot import rehydrate_node
...
def _register_entity(
    ...
    *,
    ...
    ``preserve_source_ids`` (xinggraph-origin archives): the record's external_id
    ...
    as xinggraph's own extraction does (``Entity.id_for`` via identity_fields),
...
async def stream_graph_from_source(source, stats: Dict[str, int], ctx=None) -> Dict[str, int]:
    ...
    # XingGraph-origin archives keep the source node UUIDs verbatim (exact graph
