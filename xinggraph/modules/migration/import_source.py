"""Orchestrate importing a MemorySource into XingGraph.

Called by ``xinggraph.remember()`` when it receives a :class:`MemorySource`.
Heavy XingGraph imports happen lazily inside the function to avoid import cycles
...
from xinggraph.modules.migration.loader import (
...
if TYPE_CHECKING:
    from xinggraph.api.v1.remember.remember import RememberResult
...
async def _ensure_user(user_payload: Dict[str, Any]):
    ...
    from xinggraph.modules.users.methods import create_user, get_user_by_email
...
async def _apply_social_grants(source: MemorySource, dataset_name: str, owner, importer) -> None:
    ...
    from xinggraph.modules.users.permissions.methods import give_permission_on_dataset
...
async def _restamp_to_source_revision(source: MemorySource, dataset_name: str, user) -> None:
    """Align the target's migration stamp with a xinggraph-origin archive.
...
async def import_memory_source(
    ...
    **kwargs,
    ...
    # After the rows land: xinggraph-origin archives may need the migration
...
async def _import_streaming(
    ...
    if has_graph_records:
        ...
        from xinggraph.modules.run_custom_pipeline import run_custom_pipeline
...
async def _import_buffered(
    ...
    **kwargs,
    ...
    translation = await translate_record_stream(
        source.records(),
        source.mode,
        # XingGraph-origin archives keep source node UUIDs verbatim; other
        # systems get class-namespaced ids (see _register_entity).
        preserve_source_ids=source.source_system == "xinggraph",
    )
    ...
    if translation.data_items and translation.cognify_data_items:
        from xinggraph.api.v1.remember.remember import remember
