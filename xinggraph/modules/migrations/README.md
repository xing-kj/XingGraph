...
ONE alembic-style revision chain for xinggraph's **data migrations** — each
...
has its own Alembic setup under `xinggraph/alembic/`). Each database pair records
...
`xinggraph_version` columns are **audit-only** (which release last migrated /
...
- explicitly: `await xinggraph.run_migrations()`
...
```
xinggraph-cli upgrade [revision]              # head (default) or a slug; partial-chain ok
xinggraph-cli downgrade <revision> [--dataset ID]   # 'base' or a slug; data-rewriting, confirmed
xinggraph-cli stamp <revision> [--dataset ID] # set the stored revision WITHOUT running anything
xinggraph-cli history                         # the chains, newest first
xinggraph-cli current                         # each database's stamped revision
```
...
- Unit: drive the store-helpers against the fakes in
  `xinggraph/tests/unit/modules/migrations/` — they mirror the REAL adapter
  contracts (IndexSchema payloads; no `create_data_points` on the vector fake,
  so model-reconstruction regressions fail loudly).
- Integration: `xinggraph/tests/migrations/test_migration_lockstep.py` — seed via real cognify,
...
- Cross-version: `xinggraph/tests/backwards_compatibility/phase{1,2}` against a
