...
2. graph + vector script migrations (revision chain, ``runner.py``), followed
   ...
   on every XingGraph version change (gated by a library-vs-recorded
   ``xinggraph_version`` mismatch), after the chain — both under the same per-
...
``xinggraph.run_migrations()``. Set ``ENABLE_AUTO_MIGRATIONS=false`` to
...
``xinggraph-cli upgrade`` remains the explicit path and ignores the flag.

``run_migrations`` is once-per-process: a xinggraph instance never needs
...
async def abort_write_if_migration_blocked(failed: list[str], datasets, user) -> None:
    ...
    if not backend_access_control_enabled():
        raise MigrationError(
            ...
            "`xinggraph-cli current`; it retries automatically on the next call."
        )
    ...
    if blocked:
        ...
        raise MigrationError(
            ...
            "Writing now would mix id schemes. Inspect with `xinggraph-cli current`; "
...
def _build_alembic_config(script_location: Optional[str] = None):
    ...
    defaults to xinggraph's packaged ``alembic`` dir, but can point elsewhere for
    custom/vendored deployments whose migration scripts live outside the package.
    Resolution order: explicit ``script_location`` arg → ``XINGGRAPH_ALEMBIC_PATH``
    env var → package default. (The alembic.ini is always xinggraph's packaged,
    ...
    if script_location is None:
        script_location = os.environ.get("XINGGRAPH_ALEMBIC_PATH") or os.path.join(
...
async def run_relational_migrations(target: str = "head", script_location: Optional[str] = None):
    ...
    (xinggraph.config.system_root_directory(...)) is honored. A subprocess would
...
async def _relational_schema_exists() -> bool:
    """True if the relational database already holds xinggraph's schema.

    We look for EITHER the ``users`` table OR Alembic's ``alembic_version``,
    because neither alone is reliable across xinggraph's history:
...
# Alembic revisions that CREATE the xinggraph data-migration bookkeeping (the
...
async def apply_all_migrations(
    ...
    """
    from xinggraph.modules.migrations.runner import migration_lock, run_database_migrations
    from xinggraph.infrastructure.databases.relational import get_relational_engine
...
async def revert_all_migrations(
    ...
    """
    from xinggraph.modules.migrations.runner import downgrade_database_migrations, migration_lock
...
async def run_migrations() -> list[str]:
    ...
    if not _auto_migrations_enabled():
        logger.info(
            ...
            "run `xinggraph-cli upgrade` to migrate explicitly."
        )
        return []
    ...
    async with _get_startup_lock():
        ...
        if failed:
            logger.warning(
                ...
                "(would duplicate entities until they migrate). Inspect with `xinggraph-cli "
