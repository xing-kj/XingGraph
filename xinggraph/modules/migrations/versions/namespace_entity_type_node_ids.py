...
(xing-kj/xinggraph#2515 / issue #2510).
...
--------------------
Every vector point xinggraph writes goes through ``index_data_points``, which
...
from xinggraph.infrastructure.databases.exceptions import UnsupportedProvenanceCapability
...
from xinggraph.infrastructure.databases.provenance.markers import stores_provenance_in_graph
from xinggraph.infrastructure.engine import DataPoint
from xinggraph.modules.migrations.migration import MigrationContext
from xinggraph.modules.migrations.versions._vector_rekey import (
...
def _frozen_normalize(value: str) -> str:
    ...
    as of xinggraph 1.2.0): lower-case, spaces to underscores, strip apostrophes."""
...
def _frozen_model_id(kind: str, name: str) -> str:
    ...
    (``DataPoint.id_for`` as of xinggraph 1.2.0): class-name-namespaced uuid5."""
...
async def _migrate_vector(vector_engine, id_map: dict, properties_by_id: dict) -> set:
    ...
    for node_type, old_ids in ids_by_kind.items():
        ...
        for index_field in index_fields:
            ...
            from xinggraph.infrastructure.databases.vector.exceptions import (
...
async def _migrate_ledger_edges(session, remap: dict, dataset_id) -> None:
    ...
    from xinggraph.modules.graph.models import Edge
...
async def _migrate_ledger_nodes(session, remap: dict, dataset_id) -> None:
    ...
    from xinggraph.modules.graph.models import Node
...
async def _migrate_ledger(id_map: dict, dataset_id) -> None:
    ...
    from xinggraph.modules.graph.legacy.GraphRelationshipLedger import GraphRelationshipLedger
...
def _is_hybrid_backend() -> bool:
    ...
    """
    from xinggraph.infrastructure.databases.graph.config import get_graph_context_config
    from xinggraph.infrastructure.databases.unified.get_unified_engine import _is_hybrid_provider
    from xinggraph.infrastructure.databases.vector.config import get_vectordb_context_config
...
def build_id_remap_reverse(nodes: list) -> dict:
    ...
    re-key and graph upsert collapse them, exactly as old xinggraph stored them.
    """
    id_map: dict = {}
    for node_id, properties in nodes:
        node_type = properties.get("type")
...
async def downgrade(context: MigrationContext) -> None:
    ...
    inverted. Only useful when rolling back to a pre-#2515 xinggraph release —
