...
pair, walked forward to head (plus, as an audit value, the current XingGraph
...
from xinggraph.infrastructure.databases.exceptions import EntityNotFoundError
...
from xinggraph.modules.migrations.models import GLOBAL_DATABASE_VERSION_ROW_ID, GlobalDatabaseVersion
from xinggraph.modules.migrations.registry import MIGRATIONS
from xinggraph.modules.migrations.versions.adapter_storage_migration import (
...
_GLOBAL_MIGRATION_LOCK_KEY = 0x636F676E6565_01  # "xinggraph" + 01
...
def _file_lock_path(db_engine, key: int) -> Optional[str]:
    ...
    return os.path.join(directory, f".xinggraph-migration-{key}.lock")
...
async def _migration_lock(db_engine, key: int):
    ...
    executor = concurrent.futures.ThreadPoolExecutor(
        max_workers=1, thread_name_prefix="xinggraph-migration-lock"
    )
    try:
...
async def _apply(
    ...
    for migration in pending:
        logger.info(
            "Applying migration '%s' (xinggraph %s).", migration.slug, migration.xinggraph_version
        )
...
async def _read_deployment_version() -> Optional[str]:
    """The deployment's recorded XingGraph version, or ``None`` if never recorded.
    ...
    try:
        async with db_engine.get_async_session() as session:
            record = await session.get(GlobalDatabaseVersion, GLOBAL_DATABASE_VERSION_ROW_ID)
            return record.xinggraph_version if record is not None else None
...
async def _record_deployment_version(current_version: str) -> GlobalDatabaseVersion:
    """Upsert the single ``global_database_version`` row's ``xinggraph_version``.

    Runs on every startup in BOTH access-control modes, so this row is the one
    place to read which XingGraph release last ran. The global revision column is
    ...
    async with db_engine.get_async_session() as session:
        record = await session.get(GlobalDatabaseVersion, GLOBAL_DATABASE_VERSION_ROW_ID)
        if record is not None:
            if record.xinggraph_version != current_version:
                record.xinggraph_version = current_version
                await session.commit()
            return record
    ...
    async with db_engine.get_async_session() as session:
        record = GlobalDatabaseVersion(
            ...
            xinggraph_version=current_version,
        )
        try:
            session.add(record)
            await session.commit()
...
async def run_database_migrations(target: str = "head") -> list[dict]:
    """Apply pending data migrations to every XingGraph database.
    ...
    # a XingGraph version change is detectable. It gates the vector adapter storage
