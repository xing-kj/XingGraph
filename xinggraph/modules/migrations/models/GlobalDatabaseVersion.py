...
from xinggraph.infrastructure.databases.relational import Base
...
class GlobalDatabaseVersion(Base):
    ...
    ``xinggraph_version`` is written on EVERY startup in BOTH access-control modes
    — the one place to read which XingGraph release last ran against this
    deployment (the per-dataset ``dataset_database.xinggraph_version`` records
    ...
    # XingGraph release that last started against this deployment (both modes).
    xinggraph_version = Column(String, nullable=True)
