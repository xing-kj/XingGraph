...
When XINGGRAPH_AGENT_MODE=true, the server tracks how many agent connections
...
orchestrator spins up a XingGraph server for one or more agents. Each
...
server launched with XINGGRAPH_AGENT_MODE=true will stay alive
...
def set_agent_mode(enabled: bool) -> None:
    os.environ["XINGGRAPH_AGENT_MODE"] = str(enabled).lower()


def is_agent_mode_enabled() -> bool:
    return os.getenv("XINGGRAPH_AGENT_MODE", "false").lower() == "true"
...
async def register_agent(user: User, request: RegisterAgentRequest) -> AgentConnection:
    ...
    from xinggraph.modules.agents.operations import register_agent_from_request
