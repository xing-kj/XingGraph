...
if get_llm_config().llm_provider.lower() == "gemini":
    ...
    class Edge(BaseModel):
        ...
        @field_validator("source_node_id", "target_node_id", mode="before")
        @classmethod
        def _coerce_endpoint(cls, v: Any) -> str:
            return str(v)
    ...
    class Node(BaseModel):
        ...
        @field_validator("id", "name", "type", "description", mode="before")
        @classmethod
        def _coerce_str(cls, v: Any) -> str:
            return str(v)
    ...
    class Edge(BaseModel):
        ...
        @field_validator(
            "source_node_id", "target_node_id", "relationship_name", "description", mode="before"
        )
        @classmethod
        def _coerce_endpoint(cls, v: Any) -> str:
...
class ChunkWiki(BaseModel):
    """Wiki summary for a document chunk, generated by LLM analysis."""

    wiki_id: str = Field(default_factory=str)
    chunk_id: str
    summary: str
...
class ChunkWikiLink(BaseModel):
    """Link between a document chunk and its wiki node."""
