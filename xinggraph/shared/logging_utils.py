...
def _get_xinggraph_version() -> str:
    """Get version without importing xinggraph (avoids triggering __init__.py)."""
    ...
    with suppress(FileNotFoundError, StopIteration):
        ...
        with open(_pyproject, encoding="utf-8") as f:
            ...
            return f"{_ver}-local"
    try:
        return _meta.version("xinggraph")
    except _meta.PackageNotFoundError:
        return "unknown"


xinggraph_version = _get_xinggraph_version()
...
def resolve_logs_dir() -> Path | None:
    ...
    Priority:
    1) BaseConfig.logs_root_directory (respects XINGGRAPH_LOGS_DIR)
    2) /tmp/xinggraph_logs (default, best-effort create)
    ...
    try:
        tmp_log_path = Path(os.path.join("/tmp", "xinggraph_logs"))
...
# Log rotation defaults — override via XINGGRAPH_LOG_MAX_BYTES / XINGGRAPH_LOG_BACKUP_COUNT
LOG_MAX_BYTES = int(os.getenv("XINGGRAPH_LOG_MAX_BYTES", 50 * 1024 * 1024))  # 50 MB
LOG_BACKUP_COUNT = int(os.getenv("XINGGRAPH_LOG_BACKUP_COUNT", 5))  # 5 backups → 300 MB cap
...
def log_database_configuration(logger) -> None:
    ...
    from xinggraph.infrastructure.databases.graph.config import get_graph_config
    from xinggraph.infrastructure.databases.relational.config import get_relational_config
    from xinggraph.infrastructure.databases.vector.config import get_vectordb_config
        ...
        if len(log_files) > max_files:
            ...
            for old_file in log_files[max_files:]:
                try:
                    ...
                    if os.getenv("XINGGRAPH_CLI_MODE") != "true":
            ...
            if os.getenv("XINGGRAPH_CLI_MODE") == "true" and deleted_count > 0:
...
def setup_logging(log_level=None, name=None) -> bool:
    ...
    # --- File logging (opt-out via XINGGRAPH_LOG_FILE=false) ---
    # Set XINGGRAPH_LOG_FILE=false to disable file logging entirely.
    log_file_enabled = os.getenv("XINGGRAPH_LOG_FILE", "true").lower() not in ("false", "0", "no")
    log_file_path = None
    ...
    if log_file_enabled:
        ...
        if not log_file_path and logs_dir is not None:
            # Create a new log file name with the xinggraph start time
...
def _log_deferred_info(logger) -> None:
    ...
    logger.warning(
        ...
        "Agents (@xinggraph.agent) auto-verified on registration. "
        "See https://docs.xinggraph.ai/"
    )
...
def get_timestamp_format() -> str:
    # NOTE: Some users have complained that XingGraph crashes when trying to get microsecond value
