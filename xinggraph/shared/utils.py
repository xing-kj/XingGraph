"""This module contains utility functions for the xinggraph."""
...
_DEFAULT_TELEMETRY_API_KEY_TRACKING_SALT = b"xinggraph.telemetry.api-key-tracking.v1"
...
_PERSISTENT_ID_DIR = pathlib.Path.home() / ".xinggraph"
...
def get_persistent_id() -> str:
    ...
    Stored in ~/.xinggraph/.persistent_id — a user-level directory that
    survives:
    - xinggraph.forget(everything=True) (data/DB deletion)
    - pip reinstalls / virtualenv recreation
    - git re-clones
    - XingGraph User recreation (new user_id after delete)
...
def send_telemetry(event_name: str, user_id: str | UUID, additional_properties: dict | None = None):
    ...
    - **persistent_id**: Stable machine-level ID (~/.xinggraph/.persistent_id).
      Survives data deletion, reinstalls, user recreation. Use this to
      correlate a single machine across all user_id changes.
    - **user_id**: Transient XingGraph User UUID from the database. Changes
