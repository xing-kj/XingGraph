...
from xinggraph.root_dir import get_absolute_path, ensure_absolute_path
...
class BaseConfig(BaseSettings):
    ...
    system_root_directory: str = get_absolute_path(".xinggraph_system")
    cache_root_directory: str = get_absolute_path(".xinggraph_cache")
    logs_root_directory: str = os.getenv("XINGGRAPH_LOGS_DIR", str(Path.home() / ".xinggraph" / "logs"))
    ...
    def validate_paths(self):
        ...
        if storage_backend == "s3" and not cache_root_env:
            ...
            if bucket_name:
                self.cache_root_directory = f"s3://{bucket_name}/xinggraph/cache"
    ...
    xinggraph_tracing_enabled: bool = os.getenv("XINGGRAPH_TRACING_ENABLED", "false").lower() in (
        "true",
        "1",
        "yes",
    )
    otel_service_name: str = os.getenv("OTEL_SERVICE_NAME", "xinggraph")
