...
async def cognify_session(
    ...
    For each window: adds its text to xinggraph with the
    ...
    Raises:
        XingGraphValidationError: If no valid, non-empty window was provided.
    ...
    if not valid_windows:
        ...
        raise XingGraphValidationError(message="Session window cannot be empty", log=False)
    ...
    try:
        for window in valid_windows:
            ...
            logger.debug("Session data added to xinggraph with node_set: user_sessions")
        ...
        raise XingGraphSystemError(message=f"Failed to cognify session data: {str(e)}", log=False)
