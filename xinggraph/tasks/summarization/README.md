...
The summarization module provides LLM-powered text and code summarization for XingGraph's knowledge graph pipeline. It transforms document chunks and source code into structured, searchable summaries.

> [!NOTE]
> This module runs **automatically** in `xinggraph.cognify()` (Task #4). Users typically don't call these functions directly unless building custom workflows.

**Pipeline Position:** `xinggraph.add()` → chunking → graph extraction → **summarize_text** → storage
...
await xinggraph.add(["document.pdf", "notes.txt"])
await xinggraph.cognify()  # summarize_text runs automatically

results = await xinggraph.search(query_text="key concepts", query_type=xinggraph.SearchType.SUMMARIES)
```
...
```python
from xinggraph.tasks.summarization import summarize_code
...
Default summarization model is `SummarizedContent` from `xinggraph.modules.cognify.config.get_cognify_config()`.
...
**Internal:** `xinggraph.infrastructure.llm.extraction`, `xinggraph.modules.chunking.models`, `xinggraph.infrastructure.engine.DataPoint`

**External:** `pydantic`, LLM provider (OpenAI/Anthropic/etc.)

## Related

- [xinggraph docs](https://docs.xinggraph.ai) | [Chunking](../documents/) | [Storage](../storage/) | [Tests](../../../tests/tasks/summarization/)
