...
The `web_scraper` module provides tools for scraping web content and storing structured data in xinggraph's graph database. It supports two scraping backends:
...
```python
from xinggraph.tasks.web_scraper import fetch_page_content
...
```python
from xinggraph.tasks.web_scraper import DefaultUrlCrawler
...
```python
from xinggraph.tasks.web_scraper import web_scraper_task
...
### Integration with xinggraph.add()
...
await xinggraph.add(data="https://example.com", soup_crawler_config=config)
...
| `headers` | `None` | Custom HTTP headers (defaults to XingGraph User-Agent) |
...
- [xinggraph docs](https://docs.xinggraph.ai) | [`xinggraph.tasks.ingestion`](../ingestion/) | [`DataPoint`](../../infrastructure/engine/models/DataPoint.py)
