...
XingGraph fetches user-supplied ``http(s)`` URLs server-side during ingestion
(``add()`` -> ``save_data_item_to_storage``) and web scraping. Without
validation, a caller can point XingGraph at internal-only addresses -- loopback,
...
class SSRFProtectionError(XingGraphValidationError):
...
async def validate_outbound_url(url: str) -> None:
    """Validate a user-supplied URL before XingGraph fetches it server-side.
