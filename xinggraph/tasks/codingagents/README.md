...
The coding agents module provides utilities for extracting developer coding rules and best practices from text and associating them with their original sources in XingGraph’s knowledge graph. It uses LLM-powered structured extraction to identify rules from conversations, documentation, or commit messages.

> [!NOTE]
> This module runs **automatically** in `xinggraph.memify()` (the enrichment pipeline), but is **not** enabled by default in the standard `xinggraph.cognify()` pipeline.
...
```python
import xinggraph
from xinggraph.tasks.codingagents.coding_rule_associations import get_existing_rules


await xinggraph.add(["agent.md"])# Add data (text or file paths)
await xinggraph.cognify() # Create Knowledge Graph
await xinggraph.memify()# Enrich Graph (Extract Rules automatically)
...
```python
from xinggraph.tasks.codingagents.coding_rule_associations import add_rule_associations
...
```python
from xinggraph.tasks.codingagents.coding_rule_associations import Rule, get_origin_edges
...
**Internal:** `xinggraph.infrastructure.databases.graph`, `xinggraph.infrastructure.databases.vector`, `xinggraph.infrastructure.llm`, `xinggraph.modules.engine.models`

**External:** `pydantic`, LLM provider

## Related

- [xinggraph docs](https://docs.xinggraph.ai) | [Ingestion](../ingestion/) | [Graph](../graph/)
