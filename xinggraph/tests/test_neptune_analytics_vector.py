...
async def main():
    ...
    xinggraph.config.set_vector_db_provider("neptune_analytics")
    xinggraph.config.set_vector_db_url(f"neptune-graph://{graph_id}")
    ...
    xinggraph_directory_path = str(
        pathlib.Path(
            os.path.join(pathlib.Path(__file__).parent, ".xinggraph_system/test_neptune")
        ).resolve()
    )
    ...
    await xinggraph.add([explanation_file_path_nlp], dataset_name)
...
async def vector_backend_api_test():
    ...
    # When URL is absent
    xinggraph.config.set_vector_db_url(None)
    with pytest.raises(OSError):
        await get_vector_engine_async()

    # Assert invalid graph ID.
    xinggraph.config.set_vector_db_url("invalid_url")
