...
```bash
uv run pytest xinggraph/tests/tasks/translation/ -v
```

### Run specific test file
```bash
uv run pytest xinggraph/tests/tasks/translation/detect_language_test.py -v
```

### Run tests directly (without pytest)
```bash
uv run python xinggraph/tests/tasks/translation/config_test.py
uv run python xinggraph/tests/tasks/translation/detect_language_test.py
uv run python xinggraph/tests/tasks/translation/providers_test.py
uv run python xinggraph/tests/tasks/translation/translate_content_test.py
uv run python xinggraph/tests/tasks/translation/integration_test.py
```

### Run all tests at once
```bash
for f in xinggraph/tests/tasks/translation/*_test.py; do uv run python "$f"; done
```

### Run with coverage
```bash
uv run pytest xinggraph/tests/tasks/translation/ --cov=xinggraph.tasks.translation --cov-report=html
```
...
**Note:** The translation feature uses the same LLM model configured for other xinggraph tasks (via `LLM_MODEL` and `LLM_PROVIDER` environment variables). This means any LLM provider supported by xinggraph (OpenAI, Azure, Anthropic, Ollama, etc.) can be used for translation.

## Usage Example

```python
import xinggraph
from xinggraph.tasks.translation import translate_text

# Configure translation (optional - defaults to LLM provider)
xinggraph.config.set_translation_config(
...
xinggraph.config.set_translation_provider("google")

# Use Azure Translator (requires AZURE_TRANSLATOR_KEY and AZURE_TRANSLATOR_REGION)
xinggraph.config.set_translation_provider("azure")
```
