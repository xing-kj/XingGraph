...
from xinggraph.infrastructure.databases.vector import get_vector_engine
...
async def main(mock_create_structured_output: AsyncMock):
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent, ".xinggraph_system/test_delete_two_documents_empty_store"
    )
    ...
    add_doc1 = await xinggraph.add(doc1_text)
    ...
    add_doc2 = await xinggraph.add(doc2_text)
