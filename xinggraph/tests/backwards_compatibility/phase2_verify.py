...
Phase 1 -  run with a legacy xinggraph version

Seeds the database with Lorem Ipsum data: add → cognify → search.

Phase 2 - run with current XingGraph branch
...
from xinggraph.infrastructure.session.session_persist_watermark import get_persisted_qa_count
from xinggraph.modules.data.methods import get_dataset_data, get_datasets_by_name
from xinggraph.modules.data.methods.get_dataset_databases import get_dataset_databases
from xinggraph.modules.data.models import DatasetData
...
async def _verify_vector_access(stage: str) -> None:
    ...
    results = await xinggraph.search(query_type=SearchType.CHUNKS, query_text=SEARCH_QUERY)
    if not results:
...
async def _verify_access(stage: str) -> None:
    ...
    for query_type in (
        ...
        await xinggraph.search(query_type=query_type, query_text=SEARCH_QUERY)
...
async def _verify_session_takeover(stage: str) -> None:
    ...
    await xinggraph.improve(DATASET, session_ids=[COMPAT_SESSION_ID])
    ...
    await xinggraph.remember(SESSION_FACT_3, session_id=COMPAT_SESSION_ID, self_improvement=False)
    await xinggraph.improve(DATASET, session_ids=[COMPAT_SESSION_ID])
...
async def _verify_delete(stage: str) -> None:
    ...
    for index, (data_id, dataset_id) in enumerate(pairs, start=1):
        ...
        await xinggraph.delete(data_id=data_id, dataset_id=dataset_id, mode="hard")
...
async def main():
    print(f"Running Phase 2 with xinggraph version: {xinggraph.__version__}")

    # ── Step 0: Run database migrations (relational + graph + vector) ─────────
    await xinggraph.run_migrations()
    ...
    await xinggraph.add(LOREM_IPSUM, dataset_name=DATASET)
