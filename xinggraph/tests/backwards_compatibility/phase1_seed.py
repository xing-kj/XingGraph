...
async def seed_session() -> None:
    ...
    await xinggraph.remember(SESSION_FACT_1, session_id=COMPAT_SESSION_ID, self_improvement=False)
    await xinggraph.remember(SESSION_FACT_2, session_id=COMPAT_SESSION_ID, self_improvement=False)
    await xinggraph.improve("lorem_ipsum", session_ids=[COMPAT_SESSION_ID])
    print(f"Seeded session '{COMPAT_SESSION_ID}' with 2 entries and bridged it into the graph.")


async def main():
    print(f"Running Phase 1 with xinggraph version: {xinggraph.__version__}")
    ...
    await xinggraph.add(LOREM_IPSUM, dataset_name="lorem_ipsum")

    print("Cognifying...")
    await xinggraph.cognify(datasets=["lorem_ipsum"])
