...
async def test_deduplication():
    ...
    await xinggraph.add([explanation_file_path], dataset_name, incremental_loading=False)
    await xinggraph.add([explanation_file_path2], dataset_name2, incremental_loading=False)
    ...
    await xinggraph.add([text], dataset_name2)
    ...
    await xinggraph.add([explanation_file_path], dataset_name)
    await xinggraph.add([explanation_file_path2], dataset_name2)
...
async def main():
    ...
    xinggraph_directory_path = str(
        pathlib.Path(
            os.path.join(pathlib.Path(__file__).parent, ".xinggraph_system/test_deduplication")
        ).resolve()
    )
