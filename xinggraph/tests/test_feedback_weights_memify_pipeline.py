...
from xinggraph.modules.users.methods import create_user, get_default_user, get_user_by_email
...
async def main():
    ...
    xinggraph_directory_path = str(
        (base_dir / ".xinggraph_system/test_feedback_weights_memify_pipeline").resolve()
    )
    ...
    await xinggraph.add(
        data=(
            "XingGraph builds knowledge graphs from text. TechCorp focuses on AI and machine learning."
        ),
    ...
    try:
        ...
        raise AssertionError(
            "Expected XingGraphValidationError for user without dataset write access."
        )
    except XingGraphValidationError as error:
