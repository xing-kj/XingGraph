...
from xinggraph.modules.engine.utils.generate_node_name import generate_node_name
...
async def main(mock_create_structured_output: AsyncMock):
    ...
    xinggraph_directory_path = os.path.join(
        ...
        ".xinggraph_system/test_delete_default_graph_with_legacy_graph_1",
    )
    ...
    add_john_result = await xinggraph.add(johns_text)
    ...
    add_marie_result = await xinggraph.add(maries_text)
    maries_data_id = add_marie_result.data_ingestion_info[0]["data_id"]

    cognify_result: dict = await xinggraph.cognify()
