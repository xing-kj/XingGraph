"""
XingGraph Load Tests using Locust

Usage examples:
    # Note: ENV variables must be set before usage.

    # Multi-dataset test through the web UI
    locust -f locust_performance_analysis.py --host http://your-xinggraph-instance.com MultiDatasetXingGraphTest

    # Single-dataset test through the web UI
    locust -f locust_performance_analysis.py --host http://your-xinggraph-instance.com SingleDatasetXingGraphTest

    # Both simultaneously
    locust -f locust_performance_analysis.py --host http://your-xinggraph-instance.com
...
Environment variables:
    XINGGRAPH_API_KEY          - API key for authentication (required)
"""
...
API_KEY = os.environ.get("XINGGRAPH_API_KEY", "")
SEARCH_TYPE = os.environ.get("XINGGRAPH_SEARCH_TYPE", "GRAPH_COMPLETION")
...
def add_custom_arguments(parser):
    parser.add_argument(
        ...
        type=str,
        default="",
        help="API key for XingGraph (overrides XINGGRAPH_API_KEY env var)",
    )


@events.test_start.add_listener
def on_test_start(environment, **kwargs):
    api_key = environment.parsed_options.xinggraph_api_key or API_KEY
    if not api_key:
        environment.runner.quit()
        raise SystemExit(
            "No API key provided. Set XINGGRAPH_API_KEY env var or pass --xinggraph-api-key."
        )
...
class MultiDatasetFlow(AddCognifySearchFlow):
    ...
    def on_start(self):
        ...
        self.api_key = self.user.environment.parsed_options.xinggraph_api_key or API_KEY


class MultiDatasetXingGraphTest(HttpUser):
    """
...
class SingleDatasetXingGraphTest(HttpUser):
    """
...
if __name__ == "__main__":
    ...
    def wait_for_server(url: str, timeout: float = 240.0) -> None:
        ...
        while time.time() < deadline:
            try:
                ...
                pass
            time.sleep(0.5)
        raise SystemExit(f"XingGraph server at {url} did not become ready in {timeout}s")
    ...
    server_proc = subprocess.Popen(
        [sys.executable, "-m", "uvicorn", "xinggraph.api.client:app", "--host", host, "--port", port],
    ...
    try:
        ...
        env = {**os.environ, "XINGGRAPH_API_KEY": api_key}
        ...
        cmd = [
            ...
            "--run-time",
            "5min",
            "MultiDatasetXingGraphTest",  # Run only the multi-dataset scenario
            *sys.argv[1:],
        ]
