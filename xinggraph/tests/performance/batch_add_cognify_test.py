...
Starts the XingGraph server, creates an API key, adds 200 files to a dataset,
calls cognify, and logs timing for every operation.

Usage:
    python -m xinggraph.tests.performance.batch_add_cognify_test
"""
...
def main() -> None:
    ...
    log(f"Starting XingGraph server on {base_url}")
    server_proc = subprocess.Popen(
        [VENV_PYTHON, "-m", "uvicorn", "xinggraph.api.client:app", "--host", host, "--port", port],
