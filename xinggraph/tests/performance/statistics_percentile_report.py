"""
Percentile report runner for bench_xinggraph.py.

Executes bench_xinggraph.py N times sequentially, collects timing results,
...
BENCH_SCRIPT = (Path(__file__).parent / "statistics_percentile" / "bench_xinggraph.py").resolve()
RESULTS_DIR = Path(__file__).parent / "results"
XINGGRAPH_DIR = Path(__file__).resolve().parents[3]
METRICS = [
    "add_time_s",
    "cognify_time_s",
    "total_ingest_time_s",
...
LABELS = {
    "add_time_s": "xinggraph.add()",
    "cognify_time_s": "xinggraph.cognify()",
...
def run_single(run_num: int, total: int, extra_args: list[str]) -> dict:
    ...
    # BENCH_CMD lets another SDK (e.g. the Rust xinggraph-cli bench) drive this
    ...
    t0 = time.time()
    result = subprocess.run(cmd, text=True, cwd=str(XINGGRAPH_DIR))
    wall = time.time() - t0
...
<title>XingGraph Benchmark Report</title>
...
<body>
<h1>XingGraph Benchmark Report</h1>
...
new Chart(lineCtx, {{
  ...
  data: {{
    ...
    datasets: [
      ...
      {{ label: 'xinggraph.add()', data: {run_add}, borderColor: '#3fb950', backgroundColor: 'rgba(63,185,80,0.1)', fill: true, tension: 0.3 }},
      {{ label: 'xinggraph.cognify()', data: {run_cognify}, borderColor: '#d29922', backgroundColor: 'rgba(210,153,34,0.1)', fill: true, tension: 0.3 }},
    ]
  }},
  options: {{
    responsive: true,
...
def main():
    parser = argparse.ArgumentParser(
        description="Run bench_xinggraph.py N times and produce a percentile report."
    )
    parser.add_argument(
    ...
    parser.add_argument("--memories", type=Path, default=None, help="Forward to bench_xinggraph.py")
    parser.add_argument("--llm-model", default=None, help="Forward to bench_xinggraph.py")
    parser.add_argument("--llm-provider", default=None, help="Forward to bench_xinggraph.py")
    parser.add_argument("--embedding-model", default=None, help="Forward to bench_xinggraph.py")
    parser.add_argument("--embedding-provider", default=None, help="Forward to bench_xinggraph.py")
    parser.add_argument(
        "--embedding-dims", type=int, default=None, help="Forward to bench_xinggraph.py"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mock-memories", type=Path, default=None, help="Forward to bench_xinggraph.py"
    )
    ...
    print(f"Starting {args.runs} sequential run(s) of bench_xinggraph.py...{mode}")
