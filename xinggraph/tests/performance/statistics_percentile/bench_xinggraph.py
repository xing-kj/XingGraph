"""
Standalone XingGraph ingest benchmark.

Inserts text memories into XingGraph (add + cognify) and measures wall-clock
time for each phase.

Usage:
    python bench_xinggraph.py                     # default settings
    python bench_xinggraph.py --memories data.json # custom memories file
    python bench_xinggraph.py --llm-model gpt-4o  # override LLM model
...
os.environ.setdefault("XINGGRAPH_SKIP_CONNECTION_TEST", "true")
...
def _install_mocks(mock_data: dict[str, dict]) -> None:
    """Mock the LLM (structured-output replay) and embeddings (xinggraph MOCK_EMBEDDING)."""
    import importlib

    from xinggraph.infrastructure.llm.LLMGateway import LLMGateway
    from xinggraph.shared.data_models import KnowledgeGraph, SummarizedContent

    emb_mod = importlib.import_module(
        "xinggraph.infrastructure.databases.vector.embeddings.get_embedding_engine"
    )
    vec_mod = importlib.import_module("xinggraph.infrastructure.databases.vector.create_vector_engine")
    ...
    # Mock embeddings via xinggraph's built-in MOCK_EMBEDDING switch instead of
...
async def run_benchmark(
    ...
    *,
    ...
    for module_name in filter(None, os.environ.get("XINGGRAPH_REGISTER_ADAPTERS", "").split(",")):
    ...
    xinggraph.config.set_llm_api_key(config["api_key"])
    xinggraph.config.set_llm_provider(llm_provider)
    xinggraph.config.set_llm_model(llm_model)
    xinggraph.config.set_embedding_provider(config["embedding_provider"])
    xinggraph.config.set_embedding_model(embedding_model)
    xinggraph.config.set_embedding_dimensions(embedding_dims)
    xinggraph.config.set_embedding_api_key(config["embedding_api_key"])
    ...
    # ── Phase 1: xinggraph.add() ────────────────────────────────────────────
    print(f"\nPhase 1: Adding {n} memories via xinggraph.add()...")
    text_list = [memory_to_text(mem) for mem in memories]

    try:
        t_add_start = time.time()
        await xinggraph.add(text_list, dataset_name=DATASET_NAME)
    ...
    # ── Phase 2: xinggraph.cognify() ────────────────────────────────────────
    print("\nPhase 2: Running xinggraph.cognify() (knowledge graph build)...")
    try:
        t_cognify_start = time.time()
        await xinggraph.cognify(data_per_batch=n, chunks_per_batch=10000)
    ...
    # ── Phase 3: xinggraph.search() ─────────────────────────────────────────
    print("\nPhase 3: Running search queries...")
    try:
        t_q_start = time.time()
        await xinggraph.search(query_text="What is in the document", only_context=True)
    ...
    print(f"  xinggraph.add() time : {t_add:.2f}s  ({t_add / n:.2f}s per memory)  [{status['add']}]")
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark XingGraph memory ingestion (add + cognify).",
    )
    parser.add_argument(
        "--memories",
        type=Path,
