...
async def main():
    ...
    xinggraph_directory_path = str(
        pathlib.Path(
            os.path.join(
                pathlib.Path(__file__).parent,
                ".xinggraph_system/test_edge_centered_payload",
            )
        ).resolve()
    )
    ...
    await xinggraph.add(data=text_data, dataset_name=dataset_name)
    ...
    try:
        ...
        await xinggraph.cognify(datasets=[dataset_name], config=config)
        ...
        search_results_phase2 = await xinggraph.search(
