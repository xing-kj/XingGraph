...
Run (add ENABLE_BACKEND_ACCESS_CONTROL=False for the global-mode variant):
    ...
    python xinggraph/tests/migrations/test_migration_lockstep.py <seed|downgrade|migrate|verify|recognify|delete>
"""
...
from xinggraph.modules.data.models import Data, Dataset
from xinggraph.modules.engine.models import Entity, EntityType
from xinggraph.modules.graph.models import Edge, Node
from xinggraph.modules.migrations.versions.namespace_entity_type_node_ids import build_id_remap
from xinggraph.modules.migrations.runner import run_database_migrations
...
async def verify(stage: str):
    async with _store_context():
        ...
        from xinggraph.modules.graph.models.EdgeType import EdgeType
    ...
    chunks = await xinggraph.search(query_type=xinggraph.SearchType.CHUNKS, query_text="lorem ipsum")
    check(f"{stage}: CHUNKS search returns results", bool(chunks), f"{len(chunks)} results")
    await xinggraph.search(query_type=xinggraph.SearchType.GRAPH_COMPLETION, query_text="what is lorem?")
...
async def main(phase: str):
    ...
    if phase == "seed":
        ...
        await xinggraph.add(TEXT, dataset_name=DATASET)
        await xinggraph.cognify(datasets=[DATASET])
        ...
        from xinggraph.modules.migrations.runner import downgrade_database_migrations
        ...
        for data_row in data_rows:
            await xinggraph.delete(data_id=data_row.id, dataset_id=dataset_id, mode="hard")
        async with _store_context():
            graph_engine = await get_graph_engine()
            nodes, _ = await graph_engine.get_graph_data()
