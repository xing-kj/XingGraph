...
async def main():
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent, ".xinggraph_system/test_delete_default_graph_non_mocked"
    )
    ...
    add_result = await xinggraph.add(john_document_text)
    johns_data_id = add_result.data_ingestion_info[0]["data_id"]

    add_result = await xinggraph.add(
