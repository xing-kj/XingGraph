...
async def test_search_result_quality():
    ...
    async with migration_engine.engine.connect() as conn:
        ...
        for row in result:
            ...
            # Use XingGraph search to get invoice IDs for the same Customer but by providing Customer name
            search_results = await xinggraph.search(
                query_type=SearchType.GRAPH_COMPLETION,
            ...
            print(f"XingGraph search result: {search_results}")
...
async def test_migration_sqlite():
    ...
    xinggraph.config.set_migration_db_config(
        {
    ...
    xinggraph.config.set_migration_db_config(
        {
            ...
            "migration_db_username": "xinggraph",
            "migration_db_password": "xinggraph",
