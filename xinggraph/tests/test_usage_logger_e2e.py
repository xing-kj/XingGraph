...
import xinggraph
from xinggraph.api.client import app
...
from xinggraph.modules.users.methods import get_default_user, get_authenticated_user
...
async def test_data_setup():
    ...
    await xinggraph.add(test_text, dataset_name)
...
async def test_mcp_tool_logging(e2e_config, cache_engine):
    ...
    mcp_src_path = repo_root / "xinggraph-mcp" / "src"
    ...
    if mcp_server_module.xinggraph_client is None:
        xinggraph_client_path = mcp_src_path / "xinggraph_client.py"
        if xinggraph_client_path.exists():
            spec_client = importlib.util.spec_from_file_location(
                "xinggraph_client", xinggraph_client_path
            )
            xinggraph_client_module = importlib.util.module_from_spec(spec_client)
            spec_client.loader.exec_module(xinggraph_client_module)
            XingGraphClient = xinggraph_client_module.XingGraphClient
            mcp_server_module.xinggraph_client = XingGraphClient()
        else:
            pytest.skip(f"XingGraphClient not found at {xinggraph_client_path}")
