"""Unit tests for xinggraph.cli.user_resolution."""
...
from xinggraph.cli.user_resolution import resolve_cli_user, scoped_session_id
...
class TestResolveCliUser:
    def test_none_returns_default_user(self):
        ...
        with patch("xinggraph.modules.users.methods.get_default_user", mock_get_default):
    ...
    def test_valid_uuid_existing_user(self):
        ...
        with patch("xinggraph.modules.users.methods.get_user", mock_get_user):
    ...
    def test_valid_uuid_unknown_user_warns_and_falls_back(self):
        ...
        with patch("xinggraph.modules.users.methods.get_user", mock_get_user):
            with patch("xinggraph.modules.users.methods.get_default_user", mock_get_default):
                with patch("xinggraph.cli.echo.warning") as mock_warn:
                    result = asyncio.run(resolve_cli_user(uid))
