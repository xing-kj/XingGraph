"""Unit tests for xinggraph.cli.api_client."""
...
class TestXingGraphApiClientInit:
    """Test client construction and URL helpers."""

    def test_base_url_trailing_slash_stripped(self):
        client = XingGraphApiClient("http://localhost:8000/")
    ...
    def test_custom_headers_stored(self):
        client = XingGraphApiClient("http://x", headers={"X-User-Id": "abc"})
        assert client._extra_headers == {"X-User-Id": "abc"}

    def test_default_timeout(self):
        client = XingGraphApiClient("http://x")
        assert client.timeout == 120.0

    def test_custom_timeout(self):
        client = XingGraphApiClient("http://x", timeout=30.0)
        assert client.timeout == 30.0


class TestXingGraphApiClientSharedConnection:
    ...
    def test_client_reused(self):
        ...
        with patch("xinggraph.cli.api_client._import_httpx") as mock_httpx:
            mock_client = MagicMock()
            mock_httpx.return_value.Client.return_value = mock_client

            api = XingGraphApiClient("http://localhost:8000")
            c1 = api._get_client()
            c2 = api._get_client()
            assert c1 is c2
    ...
    def test_context_manager(self):
        with patch("xinggraph.cli.api_client._import_httpx") as mock_httpx:
            ...
            with XingGraphApiClient("http://localhost:8000") as api:
                api._get_client()
            mock_client.close.assert_called_once()
...
class TestAddFileDetection:
    ...
    def test_text_items_sent_as_bytes(self):
        with patch("xinggraph.cli.api_client._import_httpx"):
            client = XingGraphApiClient("http://x")
