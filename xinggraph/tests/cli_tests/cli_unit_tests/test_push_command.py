...
from xinggraph.cli.commands.push_command import PushCommand
...
class TestPushCommand:
    ...
    def test_command_properties(self):
        ...
        assert "XingGraph Cloud" in command.help_string
    ...
    @patch("xinggraph.cli.user_resolution.resolve_cli_user", new_callable=AsyncMock)
    @patch("xinggraph.api.v1.push.push._resolve_client")
    @patch("xinggraph.push", new_callable=AsyncMock, create=True)
    @patch("xinggraph.cli.commands.push_command.asyncio.run", side_effect=_mock_run)
    def test_execute_resolves_user_and_passes_it(
        self, mock_asyncio_run, mock_push, mock_resolve_client, mock_resolve_user
    ):
        """--user-id is resolved via resolve_cli_user and passed to xinggraph.push (M4)."""
        user = _mock_user()
    ...
    @patch("xinggraph.cli.commands.push_command.fmt")
    @patch("xinggraph.cli.user_resolution.resolve_cli_user", new_callable=AsyncMock)
    @patch("xinggraph.api.v1.push.push._resolve_client")
    @patch("xinggraph.push", new_callable=AsyncMock, create=True)
    @patch("xinggraph.cli.commands.push_command.asyncio.run", side_effect=_mock_run)
    ...
    @patch("xinggraph.cli.commands.push_command.asyncio.run")
