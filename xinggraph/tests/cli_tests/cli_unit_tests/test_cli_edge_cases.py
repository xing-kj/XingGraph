...
import xinggraph
from xinggraph.cli.commands.add_command import AddCommand
from xinggraph.cli.commands.search_command import SearchCommand
from xinggraph.cli.commands.cognify_command import CognifyCommand
from xinggraph.cli.commands.delete_command import DeleteCommand
from xinggraph.cli.commands.config_command import ConfigCommand
from xinggraph.cli.exceptions import CliCommandException
from xinggraph.modules.data.methods.get_deletion_counts import DeletionCountsPreview
...
_RESOLVE_USER_PATCH = "xinggraph.cli.user_resolution.resolve_cli_user"
...
class TestAddCommandEdgeCases:
    ...
    @patch("xinggraph.cli.commands.add_command.asyncio.run", side_effect=_mock_run)
    def test_add_empty_data_list(self, mock_asyncio_run, _mock_resolve):
        mock_xinggraph = MagicMock()
        mock_xinggraph.add = AsyncMock()

        with patch.dict(sys.modules, {"xinggraph": mock_xinggraph}):
            command = AddCommand()
        ...
        mock_xinggraph.add.assert_awaited_once_with(data=[], dataset_name="test_dataset", user=ANY)

    @patch("xinggraph.cli.commands.add_command.asyncio.run")
...
class TestSearchCommandEdgeCases:
    ...
    @patch(_RESOLVE_USER_PATCH, new_callable=lambda: AsyncMock(return_value=_mock_user()))
    @patch("xinggraph.cli.commands.search_command.asyncio.run", side_effect=_mock_run)
    def test_search_empty_results(self, mock_asyncio_run, _mock_resolve):
        """Test search command with empty results"""
        # Mock the xinggraph module and SearchType
        mock_xinggraph = MagicMock()
        mock_xinggraph.search = AsyncMock(return_value=[])
        ...
        mock_asyncio_run.assert_called_once()
        assert asyncio.iscoroutine(mock_asyncio_run.call_args[0][0])
        mock_xinggraph.search.assert_awaited_once_with(
            query_text="nonexistent query",
            query_type=ANY,
            user=ANY,
            datasets=None,
        ...
        called_enum = mock_xinggraph.search.await_args.kwargs["query_type"]
    ...
    def test_search_very_large_top_k(self, mock_asyncio_run, _mock_resolve):
        ...
        # Mock the xinggraph module and SearchType
        mock_xinggraph = MagicMock()
        mock_xinggraph.search = AsyncMock(return_value=["result1"])
        mock_search_type = MagicMock()
    ...
    def test_search_invalid_search_type_enum(self, mock_import):
        ...
        def mock_import_func(name, fromlist=None, *args, **kwargs):
            if name == "xinggraph.modules.search.types":
                module = MagicMock()
    ...
    def test_search_results_with_none_values(self, mock_asyncio_run, _mock_resolve):
        ...
        # Mock the xinggraph module and SearchType
        mock_xinggraph = MagicMock()
        mock_xinggraph.search = AsyncMock(return_value=[None, "valid result", None])
        mock_search_type = MagicMock()
        mock_search_type.__getitem__.return_value = "CHUNKS"
...
class TestCognifyCommandEdgeCases:
    ...
    @patch(_RESOLVE_USER_PATCH, new_callable=lambda: AsyncMock(return_value=_mock_user()))
    @patch("xinggraph.cli.commands.cognify_command.asyncio.run", side_effect=_mock_run)
    def test_cognify_invalid_chunk_size(self, mock_asyncio_run, _mock_resolve):
        """Test cognify command with invalid chunk size"""
        # Mock the xinggraph module
        mock_xinggraph = MagicMock()
        mock_xinggraph.cognify = AsyncMock()
        ...
        mock_xinggraph.cognify.assert_awaited_once_with(
            datasets=None,
            user=ANY,
            chunk_size=-100,
            config=None,
    ...
    def test_cognify_nonexistent_ontology_file(self, mock_asyncio_run):
        ...
        mock_xinggraph.cognify.assert_not_awaited()

    @patch("xinggraph.cli.commands.cognify_command.asyncio.run")
    def test_cognify_langchain_chunker_import_error(self, mock_asyncio_run):
        ...
        def mock_import_func(name, fromlist=None, *args, **kwargs):
            if name == "xinggraph":
                return mock_xinggraph
            elif (
                name == "xinggraph.modules.chunking.LangchainChunker"
                and fromlist
                and "LangchainChunker" in fromlist
            ):
                raise ImportError("LangchainChunker not available")
            elif (
                name == "xinggraph.modules.chunking.TextChunker"
                and fromlist
        ...
        with (
            ...
            patch.dict(sys.modules, {"xinggraph": mock_xinggraph}),
        ):
            command = CognifyCommand()
            args = argparse.Namespace(
                datasets=None,
...
class TestDeleteCommandEdgeCases:
    ...
    @patch(_RESOLVE_USER_PATCH, new_callable=lambda: AsyncMock(return_value=_mock_user()))
    @patch("xinggraph.cli.commands.delete_command.asyncio.run", side_effect=_mock_run)
    @patch("xinggraph.cli.commands.delete_command.xinggraph_datasets")
    def test_delete_all_with_user_id(self, datasets_mock, async_run_mock, _mock_resolve):
        ...
        xinggraph_directory_path = os.path.join(
            os.path.dirname(__file__), ".xinggraph_system/test_cli_commands"
        )

        xinggraph.config.data_root_directory(data_directory_path)
        xinggraph.config.system_root_directory(xinggraph_directory_path)

        asyncio.run(xinggraph.prune.prune_data())
        asyncio.run(xinggraph.prune.prune_system(metadata=True))
    ...
    @patch("xinggraph.cli.commands.delete_command.get_deletion_counts")
    @patch("xinggraph.cli.commands.delete_command.fmt.confirm")
    ...
    @patch("xinggraph.cli.commands.delete_command.asyncio.run")
...
class TestConfigCommandEdgeCases:
    ...
    def test_config_get_nonexistent_key(self, mock_import):
        ...
        mock_xinggraph = MagicMock()
        mock_xinggraph.config.get = MagicMock(side_effect=KeyError("Key not found"))
        mock_import.return_value = mock_xinggraph
        ...
        command.execute(args)
        mock_xinggraph.config.get.assert_called_once_with("nonexistent_key")
    ...
    def test_config_set_complex_json_value(self, mock_import):
        ...
        # Mock the xinggraph module
        mock_xinggraph = MagicMock()
        mock_xinggraph.config.set = MagicMock()
        mock_import.return_value = mock_xinggraph
        ...
        command.execute(args)
        mock_xinggraph.config.set.assert_called_once_with(
    ...
    def test_config_set_invalid_json_value(self, mock_import):
        ...
        command.execute(args)
        mock_xinggraph.config.set.assert_called_once_with("test_key", invalid_json)

    @patch("xinggraph.cli.commands.config_command.fmt.confirm")
    ...
    def test_config_unset_method_not_found(self, mock_import):
        ...
        mock_xinggraph = MagicMock()
        mock_xinggraph.config = MagicMock()
        ...
        command.execute(args)
        mock_xinggraph.config.unset.assert_not_called()
