...
class TestCliIntegration:
    ...
    def test_cli_help(self):
        ...
        assert "xinggraph" in result.stdout.lower()
        assert "available commands" in result.stdout.lower()

    def test_cli_version(self):
        """Test that CLI version works"""
        result = subprocess.run(
            ["xinggraph-cli", "--version"],
            capture_output=True,
            text=True,
            cwd=Path(__file__).parent.parent.parent,  # Go to project root
        )
    ...
    def test_invalid_command(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "xinggraph.cli._xinggraph", "invalid_command"],
            capture_output=True,
            text=True,
            cwd=Path(__file__).parent.parent.parent,  # Go to project root
        )
    ...
    def test_config_subcommands(self):
        ...
        for subcommand in subcommands:
            result = subprocess.run(
                ["xinggraph-cli", "config", subcommand, "--help"],
                capture_output=True,
                text=True,
                cwd=Path(__file__).parent.parent.parent,  # Go to project root
            )
...
class TestCliArgumentParsing:
    ...
    def test_config_set_command(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "xinggraph.cli._xinggraph", "config", "set", "test_key", "test_value"],
            capture_output=True,
            text=True,
            cwd=Path(__file__).parent.parent.parent,  # Go to project root
        )
...
class TestCliErrorHandling:
    ...
    def test_debug_mode_flag(self):
        ...
        result = subprocess.run(
            ["xinggraph-cli", "--debug", "search", "test query"],
            capture_output=True,
            text=True,
            cwd=Path(__file__).parent.parent.parent,  # Go to project root
        )
    ...
    def test_invalid_chunker(self):
        ...
        result = subprocess.run(
            ["xinggraph-cli", "cognify", "--chunker", "InvalidChunker"],
            capture_output=True,
            text=True,
            cwd=Path(__file__).parent.parent.parent,  # Go to project root
        )
