"""
Release E2E test: 10 parallel users exercising a live XingGraph HTTP server.

Boots ``uvicorn xinggraph.api.client:app`` with ``ENABLE_BACKEND_ACCESS_CONTROL=True``
...
Usage:
    python xinggraph/tests/test_release_multi_user_e2e.py
    pytest xinggraph/tests/test_release_multi_user_e2e.py
"""
...
def wait_for_server(url: str, timeout: float = 300.0) -> None:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(1)
    raise RuntimeError(f"XingGraph server at {url} did not become ready within {timeout}s")


def start_server() -> subprocess.Popen:
    """Start uvicorn with access control on and fresh storage roots.

    A repo .env (loaded by xinggraph with override=True) wins over these env vars;
    CI has no .env, so the job env fully controls the configuration there.
    """
    storage_root = tempfile.mkdtemp(prefix=f"xinggraph_release_e2e_{RUN_ID}_")
    env = {
        **os.environ,
...
def main() -> None:
    log(f"starting XingGraph server on {BASE_URL} (run id {RUN_ID})")
    server = start_server()
    try:
