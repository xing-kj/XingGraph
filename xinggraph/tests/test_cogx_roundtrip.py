...
This is the XingGraph-to-XingGraph instance-migration path (the local half of
push/pull): ``xinggraph.export(format="cogx")`` on the source machine,
``xinggraph.remember(COGXArchiveSource(...))`` on the target. The test covers
...
restored graph carries the exact source node ids and edge count (xinggraph
...
import xinggraph
from xinggraph.modules.migration.sources.cogx_archive import COGXArchiveSource
...
async def _wipe_store():
    """Reset to an empty store, as on a machine that never ran XingGraph."""
...
async def main():
    ...
    xinggraph_directory_path = str(
        pathlib.Path(
            os.path.join(pathlib.Path(__file__).parent, ".xinggraph_system/test_cogx_roundtrip")
        ).resolve()
    )
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        await xinggraph.add([TEXT], dataset_name)
        await xinggraph.cognify([dataset_name])

        source_results = await xinggraph.search(
        ...
        from xinggraph.modules.users.methods import get_user_by_email
        ...
        export_result = await xinggraph.export(
        ...
        await xinggraph.export(dataset_name, format="cogx", destination=plain_archive_dir)
        ...
        from xinggraph.modules.migration.cogx import read_manifest
        from xinggraph.modules.migrations.migration import head_revision
        from xinggraph.modules.migrations.registry import MIGRATIONS
        ...
        import xinggraph.modules.migrations.startup as migrations_startup

        migrations_startup._startup_migrations_done = False

        import_result = await xinggraph.remember(
        ...
        restored_results = await xinggraph.search(
            query_type=SearchType.GRAPH_COMPLETION, query_text=QUERY, datasets=[dataset_name]
        )
        ...
        chunk_results = await xinggraph.search(
        ...
        plain_result = await xinggraph.remember(
