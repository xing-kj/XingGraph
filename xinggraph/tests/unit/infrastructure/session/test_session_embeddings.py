...
from xinggraph.infrastructure.session.session_turn import select_session_history
...
class TestSelectHybridQaEntries:
    ...
    async def test_session_history_hydrates_vector_hits_by_id(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "xinggraph.infrastructure.session.session_turn.search_session_qa_ids", search
        )
...
class TestSessionQaVectorHelpers:
    ...
    async def test_index_session_qa_uses_session_scope_tag(self, monkeypatch):
        ...
        index_module = importlib.import_module("xinggraph.tasks.storage.index_data_points")
    ...
    async def test_search_session_qa_ids_uses_vector_engine_scope(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "xinggraph.infrastructure.databases.vector.get_vector_engine_async",
            _get_vector_engine,
        )
