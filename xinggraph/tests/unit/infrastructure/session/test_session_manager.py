...
from xinggraph.infrastructure.databases.exceptions import SessionParameterValidationError
from xinggraph.infrastructure.session.feedback_models import (
...
from xinggraph.infrastructure.session.session_turn import compose_session_prompt
...
class TestSessionManager:
    ...
    def session_vector_mocks(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "xinggraph.infrastructure.session.session_manager.index_session_qa",
            index_mock,
        )
        monkeypatch.setattr(
            "xinggraph.infrastructure.session.session_manager.delete_session_qa_vector",
            delete_qa_mock,
        )
        monkeypatch.setattr(
            "xinggraph.infrastructure.session.session_manager.delete_session_qa_vectors",
            delete_session_mock,
        )
        monkeypatch.setattr(
            "xinggraph.infrastructure.session.session_turn.search_session_qa_ids",
    ...
    async def test_add_agent_trace_step_runs_extraction_when_enabled(
        ...
        import xinggraph.infrastructure.session.agent_context_extraction as ace
    ...
    async def test_add_agent_trace_step_returns_trace_id_and_feedback(self, sm, mock_cache):
        ...
        with (
            patch(
                "xinggraph.infrastructure.session.session_agent_trace.read_query_prompt",
                return_value="summarize this",
            ),
            patch(
                "xinggraph.infrastructure.session.session_agent_trace.LLMGateway.acreate_structured_output",
    ...
    async def test_generate_completion_with_session_no_user_id_calls_generate_completion_only(
        ...
        with (
            patch(
                "xinggraph.infrastructure.session.session_manager.session_user"
            ) as mock_session_user,
            patch(
                "xinggraph.infrastructure.session.session_manager.generate_completion",
                new_callable=AsyncMock,
    ...
    async def test_generate_completion_with_session_cache_disabled_calls_generate_completion_only(
        ...
        with (
            ...
            patch("xinggraph.infrastructure.session.session_manager.CacheConfig") as mock_config_cls,
            patch(
                "xinggraph.infrastructure.session.session_manager.generate_completion",
                new_callable=AsyncMock,
                return_value="Generated answer",
    ...
    async def test_generate_completion_with_session_available_calls_add_qa(self, sm, mock_cache):
        ...
        with (
            ...
            patch("xinggraph.infrastructure.session.session_manager.CacheConfig") as mock_config_cls,
            patch(
                "xinggraph.infrastructure.session.session_turn.generate_session_completion_with_optional_summary",
                new_callable=AsyncMock,
    ...
    async def test_generate_completion_with_session_feedback_only_records_qa(self, sm, mock_cache):
        ...
        with (
            ...
            patch("xinggraph.infrastructure.session.session_manager.CacheConfig") as mock_config_cls,
            patch(
                "xinggraph.infrastructure.session.session_turn.analyze_turn_for_session_context",
                new_callable=AsyncMock,
    ...
    async def test_generate_completion_with_session_feedback_and_followup_persists_and_adds_qa(
        ...
        """When query_to_answer is present: answer it and add QA for the original message."""
        with (
            patch(
                "xinggraph.infrastructure.session.session_manager.session_user"
            ) as mock_session_user,
    ...
    @pytest.mark.asyncio
    async def test_generate_completion_with_session_query_to_answer_no_last_qa_id_adds_qa(
        self, sm, mock_cache
    ):
        """When query_to_answer is present with empty session: add QA and no feedback write."""
        mock_cache.get_latest_qa_entries.return_value = []
        with (
            patch(
                "xinggraph.infrastructure.session.session_manager.session_user"
    ...
    async def test_generate_completion_with_session_empty_analysis_answers_original_query(
        ...
        with (
            ...
            patch(
                "xinggraph.infrastructure.session.session_turn.build_active_context_block_safe",
                new_callable=AsyncMock,
                return_value=("", []),
            ),
        ):
