...
from xinggraph.infrastructure.databases.dataset_database_handler.supported_dataset_database_handlers import (
...
async def test_create_dataset_creates_neo4j_database(monkeypatch):
    ...
    assert created_databases == ["xinggraph12345678123456781234567812345678"]
    assert initialized_databases == created_databases
    assert database_info == {
        "graph_database_provider": "neo4j",
        "graph_database_url": "bolt://localhost:7687",
        "graph_database_name": "xinggraph12345678123456781234567812345678",
        "graph_database_key": "",
...
async def test_create_neo4j_database_uses_system_database(monkeypatch):
    ...
    await Neo4jDatasetDatabaseHandler._create_neo4j_database(
        "xinggraph12345678123456781234567812345678"
    )

    assert calls == [
        (
            "system",
            "CREATE DATABASE xinggraph12345678123456781234567812345678 IF NOT EXISTS",
            {},
        ),
        ("wait", "xinggraph12345678123456781234567812345678", 30),
    ]
    assert closed == [True]
...
async def test_delete_dataset_evicts_and_drops_database(monkeypatch):
    ...
    monkeypatch.setattr(
        "xinggraph.infrastructure.databases.graph.neo4j_driver."
    ...
    assert dropped_databases == ["xinggraph12345678123456781234567812345678"]
