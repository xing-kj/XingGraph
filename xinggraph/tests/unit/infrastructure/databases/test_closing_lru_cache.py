...
from xinggraph.infrastructure.databases.utils.closing_lru_cache import (
    ClosingLRUCache,
    _start_close,
    closing_lru_cache,
)
...
def test_eviction_async_close_failure_is_logged_under_running_loop(caplog):
    ...
    with caplog.at_level(
        logging.WARNING,
        logger="xinggraph.infrastructure.databases.utils.closing_lru_cache",
    ):
        asyncio.run(run())
...
def test_name_bound_pin_predicate_resolves_parameter():
    ...
    with patch(
        "xinggraph.infrastructure.databases.dataset_queue.dataset_queue",
...
def test_wait_for_pending_close_is_bounded(monkeypatch):
    ...
    from xinggraph.infrastructure.databases.utils import closing_lru_cache as cache_module
...
_CACHE_LOGGER = "xinggraph.infrastructure.databases.utils.closing_lru_cache"
