...
try:
    ...
    from xinggraph.infrastructure.databases.vector.lancedb.LanceDBAdapter import (
        LanceDBAdapter,
    )
...
async def test_close_during_pending_get_connection_blocks_resurrection(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "xinggraph.infrastructure.databases.vector.lancedb.LanceDBAdapter.lancedb.connect_async",
        slow_connect_async,
    )
...
async def test_prune_removes_local_db_directory(tmp_path):
    ...
    from xinggraph.infrastructure.databases.vector.lancedb.LanceDBAdapter import IndexSchema
