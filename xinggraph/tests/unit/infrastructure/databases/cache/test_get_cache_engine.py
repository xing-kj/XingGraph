...
from xinggraph.infrastructure.databases.exceptions import CacheConnectionError
...
factory_mod = importlib.import_module("xinggraph.infrastructure.databases.cache.get_cache_engine")

RELATIONAL_CONFIG_MOD = "xinggraph.infrastructure.databases.relational.config"
REDIS_ADAPTER_MOD = "xinggraph.infrastructure.databases.cache.redis.RedisAdapter"
...
def test_postgres_backend_with_cache_db_url_returns_sql_adapter():
    ...
    explicit_url = "postgresql+asyncpg://xinggraph:xinggraph@localhost:5432/xinggraph_db"
...
def test_default_sqlite_backend_falls_back_to_fs_when_db_path_on_s3():
    ...
    fake_relational = types.SimpleNamespace(
        db_path="s3://bucket/xinggraph/system/databases", db_provider="sqlite"
    )
...
def test_postgres_backend_falls_back_to_relational_postgres_settings():
    ...
    assert isinstance(engine, SqlCacheAdapter)
    assert engine.db_uri == "postgresql+asyncpg://xinggraph:xinggraph@localhost:5432/xinggraph_db"
