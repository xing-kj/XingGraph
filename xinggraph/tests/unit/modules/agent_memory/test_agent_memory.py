...
from xinggraph.modules.agent_memory.runtime import (
    AgentMemoryConfig,
    AgentMemoryContext,
...
def _patch_decorator_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("xinggraph.modules.agent_memory.decorator.resolve_agent_user", resolve_user)
    monkeypatch.setattr(
        "xinggraph.modules.agent_memory.decorator.resolve_agent_dataset_scope",
        resolve_scope,
    )
    monkeypatch.setattr(
        "xinggraph.modules.agent_memory.decorator.retrieve_memory_context",
        retrieve_memory,
    )
    monkeypatch.setattr(
        "xinggraph.modules.agent_memory.decorator.persist_trace",
        persist_trace_mock,
    )
...
def _patch_session_manager(monkeypatch, session_manager):
    session_manager_module = importlib.import_module(
        "xinggraph.infrastructure.session.get_session_manager"
    )
...
def test_agent_memory_rejects_sync_functions():
    with pytest.raises(XingGraphValidationError):

        @xinggraph.agent_memory()
        def sync_fn():
            return "nope"
...
def test_agent_memory_rejects_invalid_configuration(kwargs):
    with pytest.raises(XingGraphValidationError):
        xinggraph.agent_memory(**kwargs)


def test_agent_memory_rejects_missing_task_query_from_method_param():
    with pytest.raises(XingGraphValidationError):

        @xinggraph.agent_memory(memory_query_from_method="question")
...
def test_agent_memory_requires_caching_for_session_backed_features(monkeypatch, kwargs):
    monkeypatch.setattr(
        "xinggraph.infrastructure.databases.cache.config.get_cache_config",
        lambda: SimpleNamespace(caching=False),
    )

    with pytest.raises(XingGraphValidationError, match="Caching must be enabled"):
        xinggraph.agent_memory(**kwargs)
...
async def test_agent_memory_sets_and_clears_context(monkeypatch):
    ...
    @xinggraph.agent_memory(with_memory=True, save_session_traces=True)
    async def sample_agent(question: str) -> str:
        ...
        assert context.method_params["question"] == "What does XingGraph do?"
        return "answer"

    assert await sample_agent("What does XingGraph do?") == "answer"
...
async def test_agent_memory_with_session_memory_resolves_user_but_not_dataset_scope(monkeypatch):
    ...
    @xinggraph.agent_memory(with_memory=False, with_session_memory=True, save_session_traces=False)
    async def sample_agent() -> str:
        context = get_current_agent_memory_context()
        assert context is not None
        assert context.user == resolved_user
...
async def test_retrieve_memory_context_passes_explicit_scope(monkeypatch):
    ...
    monkeypatch.setattr("xinggraph.api.v1.search.search", search_mock)
    ...
    assert await retrieve_memory_context(context) == "Relevant XingGraph Memory:\nRelevant memory"
    ...
    assert search_mock.await_args.kwargs["query_type"] == xinggraph.SearchType.GRAPH_SUMMARY_COMPLETION
...
@pytest.mark.asyncio
async def test_retrieve_memory_context_combines_session_and_xinggraph_memory(monkeypatch):
    search_mock = AsyncMock(return_value=["Relevant memory"])
...
async def test_resolve_agent_dataset_scope_defaults_to_main_dataset(monkeypatch):
    ...
    monkeypatch.setattr(
        "xinggraph.modules.agent_memory.runtime.get_default_user",
        AsyncMock(return_value=user),
    )
    monkeypatch.setattr(
        "xinggraph.modules.agent_memory.runtime.get_all_user_permission_datasets",
        permissions,
    )
...
async def test_resolve_agent_dataset_scope_validates_permissions(
    ...
    with pytest.raises(XingGraphValidationError, match=match):
...
async def test_persist_trace_uses_session_manager_with_structured_payload(monkeypatch):
    ...
    context = _make_context(
        ...
        method_params={"question": "What does XingGraph do?"},
    )
    context.scope = None
    context.memory_query = "What does XingGraph do?"
    context.memory_context = "memory context"
    context.method_return_value = {"answer": "XingGraph helps"}
    ...
    session_manager.add_agent_trace_step.assert_awaited_once_with(
        ...
        memory_query="What does XingGraph do?",
        memory_context="memory context",
        method_params={"question": "What does XingGraph do?"},
        method_return_value={"answer": "XingGraph helps"},
        error_message="",
    )
...
async def test_persist_trace_skips_all_work_when_session_trace_persistence_disabled(monkeypatch):
    ...
    monkeypatch.setattr(
        "xinggraph.memify_pipelines.persist_agent_trace_feedbacks_in_knowledge_graph.persist_agent_trace_feedbacks_in_knowledge_graph_pipeline",
        persist_memify_mock,
    )
...
async def test_agent_memory_persists_error_trace_and_reraises(monkeypatch):
    ...
    monkeypatch.setattr(
        "xinggraph.modules.agent_memory.decorator.resolve_agent_user",
    ...
    @xinggraph.agent_memory(with_memory=False, save_session_traces=True)
...
def test_llmgateway_inject_agent_memory_includes_created_context_text():
    from xinggraph.infrastructure.llm.LLMGateway import _inject_agent_memory
...
async def test_llmgateway_injects_agent_memory(monkeypatch):
    llm_gateway_module = importlib.import_module("xinggraph.infrastructure.llm.LLMGateway")
    ...
    monkeypatch.setattr(
        "xinggraph.infrastructure.llm.structured_output_framework.litellm_instructor.llm.get_llm_client.get_llm_client",
        lambda: llm_client,
    )
