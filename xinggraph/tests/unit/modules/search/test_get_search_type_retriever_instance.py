...
async def test_feeling_lucky_delegates_to_select_search_type(monkeypatch):
    import xinggraph.modules.search.methods.get_search_type_retriever_instance as mod
...
async def test_registered_community_retriever_is_used(monkeypatch):
    ...
    """
    import xinggraph.modules.search.methods.get_search_type_retriever_instance as mod
    from xinggraph.modules.retrieval import registered_community_retrievers as registry
...
async def test_hybrid_completion_get_retriever_output_smoke():
    retriever_output_module = importlib.import_module(
        "xinggraph.modules.search.methods.get_retriever_output"
    )
    ...
    with (
        ...
        patch(
            "xinggraph.modules.retrieval.hybrid_retriever.get_unified_engine",
            new_callable=AsyncMock,
            return_value=unified,
        ),
        patch(
            "xinggraph.modules.retrieval.hybrid.chunks.BM25ChunksRetriever",
            return_value=bm25_retriever,
        ),
        patch(
            "xinggraph.modules.retrieval.hybrid_retriever.generate_completion",
            new_callable=AsyncMock,
            return_value="answer",
        ),
    ):
