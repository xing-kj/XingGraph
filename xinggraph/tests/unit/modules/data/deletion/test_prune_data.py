...
from xinggraph.infrastructure.databases.cache.get_cache_engine import (
    close_cache_engine,
    create_cache_engine,
    get_cache_engine,
)
from xinggraph.modules.data.deletion.prune_data import prune_data
...
async def test_prune_data_closes_cached_fs_cache(tmp_path: pathlib.Path):
    ...
    try:
        ...
        xinggraph.config.data_root_directory(str(tmp_path))
        cache_config.cache_backend = "fs"
        cache_config.caching = True

        cache_engine = get_cache_engine()
        assert cache_engine is not None
        assert (tmp_path / ".xinggraph_fs_cache" / "sessions_db").exists()
        ...
        await close_cache_engine()
        xinggraph.config.data_root_directory(previous_data_root_directory)
