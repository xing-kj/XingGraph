...
from xinggraph.modules.visualization import xinggraph_network_visualization as orch
...
def _render(tmp_path, monkeypatch, embeddings):
    ...
    return asyncio.run(xinggraph_network_visualization(_graph(), str(tmp_path / "out.html")))
...
def test_semantic_failure_never_breaks_render(tmp_path, monkeypatch):
    ...
    html = asyncio.run(xinggraph_network_visualization(_graph(), str(tmp_path / "out.html")))
