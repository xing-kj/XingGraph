...
from xinggraph.modules.graph.exceptions import EntityNotFoundError
...
@pytest.fixture
def setup_graph():
    """Fixture to initialize a XingGraphGraph instance."""
    return XingGraphGraph()
...
async def test_project_graph_from_db_missing_nodes_are_skipped(setup_graph, mock_adapter, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="XingGraphGraph"):
        await graph.project_graph_from_db(
            adapter=mock_adapter,
            node_properties_to_project=["name"],
            edge_properties_to_project=["relationship_name"],
