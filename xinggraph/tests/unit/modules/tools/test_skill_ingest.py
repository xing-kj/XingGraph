...
class TestSkillContract(unittest.TestCase):
    def setUp(self) -> None:
        ...
        patcher = patch(
            "xinggraph.context_global_variables.backend_access_control_enabled",
            return_value=False,
        )
        patcher.start()
    ...
    def test_add_skills_persists_one_dataset_scoped_skill(self):
        ...
        from xinggraph.modules.tools.ingest_skills import add_skills
        ...
        try:
            with patch(
                "xinggraph.modules.tools.ingest_skills.add_data_points",
    ...
    def test_agentic_completion_uses_agentic_retriever_only_when_explicit(self):
        ...
        from xinggraph.modules.retrieval.graph_completion_cot_retriever import (
            GraphCompletionCotRetriever,
        )
        from xinggraph.modules.search.exceptions import UnsupportedSearchTypeError
    ...
    def test_agentic_retriever_skips_memory_retrieval_when_graph_has_no_edges(self):
        ...
        async def run_retrieval():
            with (
                ...
                patch(
                    "xinggraph.modules.retrieval.agentic_retriever.resolve_skills",
                    new_callable=AsyncMock,
                    return_value=[],
                ),
                patch(
                    "xinggraph.modules.retrieval.agentic_retriever.list_tools_for_dataset",
                    new_callable=AsyncMock,
                    return_value=[],
                ),
                patch.object(
    ...
    def test_load_skill_loads_only_active_context_skills(self):
        from xinggraph.modules.engine.models import Skill
        from xinggraph.modules.tools.builtin.load_skill import handler
        from xinggraph.modules.tools.context import active_skills_var, opened_skills_var
        from xinggraph.modules.tools.errors import ToolInvocationError
    ...
    def test_agentic_skill_run_write_is_dataset_scoped(self):
        from xinggraph.modules.engine.models import Skill
        from xinggraph.modules.engine.models.SkillRun import UNSCORED_SKILL_RUN_SCORE
        from xinggraph.modules.retrieval.agentic_retriever import AgenticRetriever
        ...
        add_data_points_module = importlib.import_module("xinggraph.tasks.storage.add_data_points")
    ...
    def test_remember_skill_run_entry_persists_with_dataset_context(self):
        from xinggraph.memory import SkillRunEntry
        from xinggraph.modules.engine.models import Skill
        ...
        async def run_entry():
            with (
                patch("xinggraph.modules.tools.skill_runs.setup", new_callable=AsyncMock),
                patch(
                    "xinggraph.modules.tools.skill_runs.resolve_authorized_user_datasets",
                    new_callable=AsyncMock,
                    return_value=(user, [dataset]),
                ),
                patch(
                    "xinggraph.modules.tools.skill_runs.resolve_skills",
                    new_callable=AsyncMock,
                    return_value=[skill],
                ),
                patch(
                    "xinggraph.modules.tools.skill_runs.add_data_points",
                    new_callable=AsyncMock,
                ) as mock_add,
            ):
    ...
    def test_skill_improvement_proposal_does_not_mutate_skill(self):
        from xinggraph.modules.engine.models import Skill, SkillRun
        from xinggraph.modules.memify.skill_improvement import (
        ...
        async def create_proposal():
            with (
                patch(
                    "xinggraph.modules.memify.skill_improvement.find_skill_by_name",
                    new_callable=AsyncMock,
                    return_value=skill,
                ),
                patch(
                    "xinggraph.modules.memify.skill_improvement._find_recent_failure_runs",
                    new_callable=AsyncMock,
                    return_value=[run],
                ),
                patch(
                    "xinggraph.modules.memify.skill_improvement._generate_proposal",
                    new_callable=AsyncMock,
                ...
                patch(
                    "xinggraph.modules.memify.skill_improvement.add_data_points",
                    new_callable=AsyncMock,
                ) as mock_add,
            ):
    ...
    def test_skill_improvement_apply_requires_existing_proposal_id(self):
        from xinggraph.modules.memify.skill_improvement import improve_skill
    ...
    def test_skill_improvement_apply_updates_only_target_skill(self):
        from xinggraph.modules.engine.models import Skill, SkillImprovementProposal
        from xinggraph.modules.memify.skill_improvement import improve_skill
        ...
        async def apply_proposal():
            with (
                patch(
                    "xinggraph.modules.memify.skill_improvement._find_proposal",
                    new_callable=AsyncMock,
                    return_value=proposal,
                ),
                patch(
                    "xinggraph.modules.memify.skill_improvement.find_skill_by_id",
                    new_callable=AsyncMock,
                    return_value=skill,
                ),
                patch(
    ...
    def test_get_proposal_returns_proposal_preview(self):
        from xinggraph.modules.engine.models import SkillImprovementProposal
        from xinggraph.modules.memify.skill_improvement import get_proposal
    ...
    def test_materialize_inline_skill_writes_slug_dir(self):
        from xinggraph.api.v1.remember.remember import _materialize_inline_skill
