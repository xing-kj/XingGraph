...
from xinggraph.modules.migrations.migration import (
    Migration,
    head_revision,
...
from xinggraph.modules.migrations.registry import MIGRATIONS
...
def _chain(with_downs: bool = False) -> list[Migration]:
    ...
    m1 = Migration(slug="m1", xinggraph_version="1.0.0", up=_noop, down_revision=None, down=down)
    m2 = Migration(slug="m2", xinggraph_version="1.1.0", up=_noop, down_revision="m1", down=down)
    m3 = Migration(slug="m3", xinggraph_version="1.2.0", up=_noop, down_revision="m2", down=down)
    return [m3, m1, m2]
...
def test_pending_unknown_revision_runs_nothing_but_warns(caplog):
    ...
    with caplog.at_level("WARNING", logger="xinggraph.modules.migrations.migration"):
...
def test_branched_chain_raises():
    a = Migration(slug="a", xinggraph_version="1.0.0", up=_noop, down_revision=None)
    b = Migration(slug="b", xinggraph_version="1.0.0", up=_noop, down_revision=None)
...
def test_disconnected_chain_raises():
    a = Migration(slug="a", xinggraph_version="1.0.0", up=_noop, down_revision=None)
    orphan = Migration(
        slug="orphan", xinggraph_version="1.0.0", up=_noop, down_revision="missing-parent"
    )
    with pytest.raises(ValueError):
...
def test_frozen_id_derivations_are_pinned():
    ...
    new scheme instead."""
    from xinggraph.modules.migrations.versions.namespace_entity_type_node_ids import (
        _frozen_bare_id,
        _frozen_model_id,
    )
...
def test_frozen_derivations_currently_match_live_models():
    ...
    from xinggraph.infrastructure.engine.utils.generate_node_id import generate_node_id
...
def test_apply_runs_pending_in_order_and_stamps_per_step(monkeypatch):
    import xinggraph.modules.migrations.runner as runner
    ...
    m1 = Migration(slug="t1", xinggraph_version="1.0.0", up=make_up("t1"), down_revision=None)
    m2 = Migration(slug="t2", xinggraph_version="1.1.0", up=make_up("t2"), down_revision="t1")
...
def test_revert_span_runs_downs_newest_first_and_stamps_per_step():
    ...
    from xinggraph.modules.migrations.migration import migrations_to_downgrade
    from xinggraph.modules.migrations.runner import _revert_span
    ...
    m1 = Migration(
        slug="t1", xinggraph_version="1.0.0", up=_noop, down_revision=None, down=make_down("t1")
    )
    m2 = Migration(
        slug="t2", xinggraph_version="1.1.0", up=_noop, down_revision="t1", down=make_down("t2")
    )
...
def test_revert_span_failure_mid_span_keeps_bookkeeping_consistent():
    ...
    m1 = Migration(slug="t1", xinggraph_version="1.0.0", up=_noop, down_revision=None, down=boom_down)
    m2 = Migration(slug="t2", xinggraph_version="1.1.0", up=_noop, down_revision="t1", down=ok_down)
...
def test_downgrade_span_is_synchronous_validation():
    ...
    from xinggraph.modules.migrations.runner import _downgrade_span
...
def test_adapter_storage_sync_runs_on_version_change_even_at_chain_head(monkeypatch, tmp_path):
    """The vector adapter storage sync must run on a XingGraph VERSION change even
    ...
    from xinggraph.modules.migrations.models import (
    ...
    async def scenario():
        ...
        monkeypatch.setattr(runner, "get_xinggraph_version", lambda: "9.9.9-new")
