...
from xinggraph.infrastructure.session.session_manager import SessionTurnPreparation
from xinggraph.modules.graph.models.EdgeType import EdgeType
from xinggraph.modules.retrieval.exceptions.exceptions import NoDataError, QueryValidationError
...
async def test_global_context_is_omitted_by_default():
    ...
    with patch(
        "xinggraph.modules.retrieval.hybrid_retriever.load_root_text",
        new_callable=AsyncMock,
    ) as load_root_text:
        context = await retriever.get_context_from_objects(
            query="q", retrieved_objects={"chunks": [], "entities": []}
...
async def test_global_context_is_prepended_when_enabled():
    ...
    with (
        ...
        patch(
            "xinggraph.modules.retrieval.hybrid_retriever.search_top_global_context_summaries",
            new_callable=AsyncMock,
...
async def test_session_path_calls_session_manager_with_used_node_ids():
    ...
    with (
        ...
        patch(
            "xinggraph.modules.retrieval.hybrid_retriever.get_session_manager",
