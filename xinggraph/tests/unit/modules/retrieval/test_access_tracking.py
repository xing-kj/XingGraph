...
from xinggraph.modules.retrieval.utils.access_tracking import (
...
async def test_update_node_access_timestamps_handles_hybrid_retrieval_dict(monkeypatch):
    ...
    with (
        patch(
            "xinggraph.modules.retrieval.utils.access_tracking.get_graph_engine",
            new_callable=AsyncMock,
            return_value=graph_engine,
        ),
        patch(
            "xinggraph.modules.retrieval.utils.access_tracking._find_origin_documents_via_projection",
            new_callable=AsyncMock,
            return_value=["doc-1"],
        ) as find_origin_documents,
        patch(
            "xinggraph.modules.retrieval.utils.access_tracking._update_sql_records",
            new_callable=AsyncMock,
