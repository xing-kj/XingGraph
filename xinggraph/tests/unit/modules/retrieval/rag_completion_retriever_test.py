...
async def test_get_context_success(mock_vector_engine):
    ...
    with patch(
        "xinggraph.modules.retrieval.completion_retriever.get_vector_engine_async",
...
async def test_get_completion_without_session(mock_vector_engine):
    ...
    with (
        ...
        patch(
            "xinggraph.modules.retrieval.completion_retriever.generate_completion",
            return_value="Generated answer",
        ),
        patch("xinggraph.modules.retrieval.completion_retriever.CacheConfig") as mock_cache_config,
    ):
...
async def test_get_completion_with_session(mock_vector_engine):
    ...
    with (
        ...
        patch(
            "xinggraph.modules.retrieval.completion_retriever.get_session_manager",
        ) as mock_get_sm,
        patch("xinggraph.modules.retrieval.completion_retriever.CacheConfig") as mock_cache_config,
        patch("xinggraph.modules.retrieval.completion_retriever.session_user") as mock_session_user,
    ):
        mock_config = MagicMock()
