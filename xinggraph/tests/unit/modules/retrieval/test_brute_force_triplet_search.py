...
from xinggraph.modules.retrieval.utils.brute_force_triplet_search import (
...
from xinggraph.infrastructure.databases.vector.exceptions.exceptions import CollectionNotFoundError
...
async def test_brute_force_triplet_search_wide_search_limit_global_search():
    ...
    with patch(
        "xinggraph.modules.retrieval.utils.node_edge_vector_search.get_vector_engine_async",
...
async def test_brute_force_triplet_search_extracts_node_ids_global_search():
    ...
    with (
        ...
        patch(
            "xinggraph.modules.retrieval.utils.brute_force_triplet_search.get_memory_fragment",
...
async def test_get_memory_fragment_projects_feedback_weight_only_when_feedback_influence_enabled():
    ...
    mock_fragment = MagicMock(spec=XingGraphGraph)
    mock_fragment.project_graph_from_db = AsyncMock()

    with (
        patch(
            "xinggraph.modules.retrieval.utils.brute_force_triplet_search.get_graph_engine",
            return_value=mock_graph_engine,
        ),
        patch(
            "xinggraph.modules.retrieval.utils.brute_force_triplet_search.XingGraphGraph",
            return_value=mock_fragment,
        ),
    ):
...
async def test_brute_force_triplet_search_invalid_feedback_influence_raises():
    ...
    with pytest.raises(
        XingGraphValidationError, match="feedback_influence must be in range \\[0, 1\\]"
    ):
...
async def test_get_memory_fragment_returns_empty_graph_on_error():
    ...
    with patch(
        ...
        assert isinstance(fragment, XingGraphGraph)
...
@pytest.mark.asyncio
async def test_xinggraph_graph_mapping_batch_shapes():
    """Test that XingGraphGraph mapping methods accept list-of-lists with query_list_length set."""
    from xinggraph.modules.graph.xinggraph_graph.XingGraphGraphElements import Node, Edge
