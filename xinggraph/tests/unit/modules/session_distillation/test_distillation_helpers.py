...
import xinggraph.modules.session_distillation.distill as distill_module
from xinggraph.modules.session_distillation.distill import render_lesson_document
from xinggraph.modules.session_distillation.models import (
    BATCH_CHAR_BUDGET,
...
class TestRenderLessonDocument:
    ...
    async def test_publish_tags_lessons_with_global_and_session_node_sets(self):
        ...
        with (
            patch("xinggraph.api.v1.add.add", add),
            patch("xinggraph.api.v1.cognify.cognify", cognify),
        ):
...
class TestDistillSessionBoundary:
    ...
    async def test_requires_dataset(self):
        ...
        with pytest.raises(XingGraphValidationError, match="dataset is required"):
    ...
    async def test_rejects_missing_or_unwritable_dataset(self, monkeypatch):
        ...
        with pytest.raises(XingGraphValidationError, match="not found or not writable"):
