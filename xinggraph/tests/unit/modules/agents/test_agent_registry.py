...
async def test_agent_memory_registers_and_deactivates_connection(monkeypatch):
    ...
    monkeypatch.setattr(
        "xinggraph.modules.agents.registry._persist_agent_connection",
        noop_persist,
    )
    monkeypatch.setattr(
        "xinggraph.modules.agents.registry._deactivate_persisted_connection",
        noop_deactivate,
    )
    ...
    monkeypatch.setattr("xinggraph.modules.agent_memory.decorator.persist_trace", persist_trace)

    captured_connections = []

    @xinggraph.agent_memory(with_memory=True, with_session_memory=True, save_session_traces=True)
