...
from xinggraph.infrastructure.session.session_persist_watermark import SessionPersistWindow
from xinggraph.tasks.memify.cognify_session import cognify_session
from xinggraph.exceptions import XingGraphValidationError, XingGraphSystemError

# Get the actual module object (not the function) for patching
cognify_session_module = sys.modules["xinggraph.tasks.memify.cognify_session"]
...
async def test_cognify_session_add_failure():
    """Test cognification handles xinggraph.add failure without advancing the watermark."""
    mock_sm = _mock_session_manager()
    ...
    with (
        ...
        with pytest.raises(XingGraphSystemError) as exc_info:
...
async def test_cognify_session_re_raises_validation_error():
    """Test that XingGraphValidationError is re-raised as-is."""
    with pytest.raises(XingGraphValidationError):
...
async def test_cognify_session_with_special_characters():
    ...
    window = _window("Session: test™ © Question: What's special? Answer: XingGraph is special!")
    mock_sm = _mock_session_manager()
