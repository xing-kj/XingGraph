...
extract_feedback_qas_module = sys.modules["xinggraph.tasks.memify.extract_feedback_qas"]
...
async def test_extract_feedback_qas_rejects_non_list_session_ids(mock_user):
    ...
    with (
        ...
        with pytest.raises(XingGraphValidationError, match="session_ids must be provided"):
