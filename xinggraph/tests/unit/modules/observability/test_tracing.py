"""Unit tests for the xinggraph tracing / observability module."""

import os
import pytest

from xinggraph.modules.observability.tracing import (
    XingGraphSpanExporter,
    XingGraphTrace,
    redact_secrets,
    setup_tracing,
    shutdown_tracing,
...
from xinggraph.modules.observability.trace_context import (
    enable_tracing,
    disable_tracing,
    is_tracing_enabled,
    get_last_trace,
...
# ---------------------------------------------------------------------------
# XingGraphSpanExporter tests
# ---------------------------------------------------------------------------


class TestXingGraphSpanExporter:
    def test_export_and_retrieve(self):
        ...
        exporter = XingGraphSpanExporter()
...
# ---------------------------------------------------------------------------
# XingGraphTrace tests
# ---------------------------------------------------------------------------


class TestXingGraphTrace:
    def test_summary_empty(self):
        trace = XingGraphTrace([])
    ...
    def test_summary_with_spans(self):
        spans = [
            ...
            },
            {
                ...
                "duration_ms": 100.0,
                "status": "OK",
                "attributes": {"xinggraph.db.system": "ladybug"},
            },
        ]

        trace = XingGraphTrace(spans)
        summary = trace.summary()
        assert summary["operation"] == "xinggraph.search"
        assert summary["total_duration_ms"] == 1000.0
        assert summary["span_count"] == 2
        assert "xinggraph.graph.query" in summary["breakdown"]
        assert summary["breakdown"]["xinggraph.graph.query"]["count"] == 1
...
class TestTracingLifecycle:
    def setup_method(self):
        ...
        disable_tracing()
        os.environ.pop("XINGGRAPH_TRACING_ENABLED", None)
    ...
    def test_env_var_enables(self):
        os.environ["XINGGRAPH_TRACING_ENABLED"] = "true"
        assert is_tracing_enabled()
