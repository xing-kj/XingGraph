"""Unit tests for the agents SDK namespace (``xinggraph.agents``) and the
``AgentsCommand`` CLI command.

These tests run WITHOUT a live LLM, network, or relational database. The
backend ``xinggraph.modules.agents.*`` functions and permission helpers are
...
agents_sdk = pytest.importorskip(
    "xinggraph.api.v1.agents.agents",
...
def _make_user(user_id=None):
    """A minimal stand-in for a xinggraph User object."""
...
def _make_agent_info(parent_id, slug="support", api_key_label="support"):
    ...
    agent_id = uuid4()
    email = f"{slug}+{parent_id}@xinggraph.agent"
...
async def test_create_list_get_delete_round_trip(monkeypatch):
    ...
    assert created["agent_email"] == "support-bot@xinggraph.agent"
...
async def test_register_list_connections_unregister(monkeypatch):
    from xinggraph.modules.agents.models import (
        AgentConnection,
        AgentsListResponse,
    )
...
async def test_list_connections_drops_unscoped_connections(monkeypatch):
    ...
    from xinggraph.modules.agents.models import AgentConnection, AgentsListResponse
...
cli_module = pytest.importorskip(
    "xinggraph.cli.commands.agents_command",
...
def test_agents_command_execute_list(monkeypatch):
    ...
    monkeypatch.setattr("xinggraph.cli.user_resolution.resolve_cli_user", fake_resolve_cli_user)

    listed_rows = [
        {
            "agent_id": str(uuid4()),
            "agent_email": "bot@xinggraph.agent",
            "api_key_label": "bot",
        }
    ]
    ...
    # The top-level ``xinggraph.agents`` export may be wired in a separate edit.
    # Ensure the CLI can resolve it regardless, then patch ``list`` on the
    # SDK class object that the CLI calls through.
    if not hasattr(xinggraph, "agents"):
        monkeypatch.setattr(xinggraph, "agents", agents, raising=False)
    monkeypatch.setattr(xinggraph.agents, "list", staticmethod(fake_list))

    captured = []
    monkeypatch.setattr("xinggraph.cli.echo.echo", lambda *a, **k: captured.append(a))

    # execute() runs the list action without raising
    command.execute(args)
    assert any("bot@xinggraph.agent" in str(c) for c in captured)
...
def test_agents_command_execute_no_action_raises(monkeypatch):
    ...
    monkeypatch.setattr("xinggraph.cli.echo.error", lambda *a, **k: None)
...
async def test_resolve_cli_user_strict_unknown_uuid_raises(monkeypatch):
    ...
    from xinggraph.cli import user_resolution
    ...
    monkeypatch.setattr("xinggraph.modules.users.methods.get_user", fake_get_user, raising=False)
    monkeypatch.setattr(
        "xinggraph.modules.users.methods.get_default_user", fake_get_default_user, raising=False
    )
...
async def test_resolve_cli_user_non_strict_falls_back(monkeypatch):
    ...
    monkeypatch.setattr("xinggraph.cli.echo.warning", lambda *a, **k: None)
