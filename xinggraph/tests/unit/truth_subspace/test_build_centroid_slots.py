...
async def _run_build(monkeypatch, session_ids=None):
    ...
    with (
        patch(
            "xinggraph.modules.truth_subspace.build.get_authorized_existing_datasets",
            new=AsyncMock(return_value=[dataset]),
        ),
        patch(
            "xinggraph.modules.truth_subspace.build.get_vector_engine_async",
            new=AsyncMock(return_value=vector_engine),
        ),
        patch(
            "xinggraph.modules.truth_subspace.build.get_graph_engine",
            new=AsyncMock(return_value=graph_engine),
        ),
        patch(
            "xinggraph.modules.truth_subspace.build.get_embedding_engine",
