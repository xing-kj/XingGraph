...
from xinggraph.modules.data.processing.document_types.Document import Document
from xinggraph.modules.engine.models import Triplet
from xinggraph.modules.graph.utils import ensure_default_edge_properties
from xinggraph.modules.pipelines.models import PipelineContext
from xinggraph.tasks.storage.add_data_points import (
    add_data_points,
...
adp_module = sys.modules["xinggraph.tasks.storage.add_data_points"]
...
async def test_add_data_points_skips_provenance_when_data_item_has_no_id(
    ...
    """memify hands the XingGraphGraph itself as data_item (no `.id`/`.data_id`):
    ...
    class _GraphLike:  # stand-in for XingGraphGraph: no id, no data_id
        pass
