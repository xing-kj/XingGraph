...
from xinggraph.modules.graph.xinggraph_graph.XingGraphGraphElements import Edge, Node
from xinggraph.modules.pipelines.models import PipelineContext
from xinggraph.tasks.memify.global_context_index.constants import SUMMARIZED_IN
from xinggraph.tasks.memify.global_context_index.load import (
    extract_context_index_input,
)
from xinggraph.tasks.memify.global_context_index.models import (
    GlobalContextIndexInput,
    GlobalContextIndexUpdateData,
    SummaryNode,
)
from xinggraph.tasks.memify.global_context_index.persist import (
    ensure_global_context_storage_context,
)
from xinggraph.tasks.memify.global_context_index.update import (
    update_global_context_index,
)
from xinggraph.tasks.summarization.models import GlobalContextSummary

update_global_context_index_module = import_module(
    "xinggraph.tasks.memify.global_context_index.update"
)
load_module = import_module("xinggraph.tasks.memify.global_context_index.load")
summary_generation_module = import_module("xinggraph.tasks.memify.global_context_index.summarize")
persist_module = import_module("xinggraph.tasks.memify.global_context_index.persist")
...
def _add_summarized_in_edge(graph: XingGraphGraph, child: Node, parent: Node) -> None:
    graph.add_edge(
        Edge(
            child,
            parent,
...
def test_ensure_global_context_storage_context_adds_stable_data_item_id():
    ...
    assert ctx.data_item.id == uuid5(
        NAMESPACE_URL,
        f"xinggraph:global-context-index:{dataset_id}",
    )


def test_global_context_index_package_exports_pipeline_tasks():
    package_module = import_module("xinggraph.tasks.memify.global_context_index")
    update_module = import_module("xinggraph.tasks.memify.global_context_index.update")
...
async def test_update_global_context_index_first_build_creates_one_bucket_per_summary(monkeypatch):
    ...
    monkeypatch.setattr(
        "xinggraph.tasks.memify.global_context_index.summarize.generate_bucket_summary",
        summarize_bucket,
    )
    monkeypatch.setattr(
        "xinggraph.tasks.memify.global_context_index.summarize.generate_global_context_summary",
        summarize_root,
    )
    monkeypatch.setattr(
        "xinggraph.tasks.memify.global_context_index.persist.add_data_points",
...
async def test_persist_context_index_edges_stamps_dataset_ref_on_graph_provenance(monkeypatch):
    ...
    from xinggraph.tasks.memify.global_context_index.models import BucketAssignment
    from xinggraph.tasks.memify.global_context_index.persist import persist_context_index_edges
    ...
    sentinel_data_id = uuid5(NAMESPACE_URL, f"xinggraph:global-context-index:{dataset_id}")
