...
from xinggraph.infrastructure.session.session_persist_watermark import (
...
from xinggraph.tasks.memify.extract_user_sessions import extract_user_sessions
...
async def test_cognify_session_advances_watermark_on_success(user, manager, monkeypatch):
    ...
    monkeypatch.setattr(xinggraph, "add", fake_add)
    monkeypatch.setattr(xinggraph, "cognify", fake_cognify)
...
async def test_cognify_session_keeps_watermark_on_failure(user, manager, monkeypatch):
    ...
    monkeypatch.setattr(xinggraph, "add", fake_add)
    monkeypatch.setattr(xinggraph, "cognify", failing_cognify)
    ...
    with pytest.raises(XingGraphSystemError):
        await cognify_session(window, dataset_id=uuid.uuid4(), user=user)
