...
@pytest.mark.asyncio
@pytest.mark.parametrize("benchmark", benchmark_options)
@patch.object(CorpusBuilderExecutor, "run_xinggraph", new_callable=AsyncMock)
async def test_corpus_builder_build_corpus(mock_run_xinggraph, benchmark):
    limit = 2
    if benchmark in ("HotPotQA", "TwoWikiMultiHop"):
        with patch.object(HotpotQAAdapter, "_get_raw_corpus", return_value=MOCK_HOTPOT_CORPUS):
            corpus_builder = CorpusBuilderExecutor(benchmark, "Default")
...
@pytest.mark.asyncio
@patch.object(CorpusBuilderExecutor, "run_xinggraph", new_callable=AsyncMock)
async def test_corpus_builder_prepares_adapter_before_loading(mock_run_xinggraph):
