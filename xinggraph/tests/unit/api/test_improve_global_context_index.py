...
async def test_global_context_index_pipeline_calls_memify(monkeypatch):
    from xinggraph.memify_pipelines import global_context_index as pipeline_module
...
async def test_improve_global_context_index_opt_in(monkeypatch, build_global_context_index):
    import xinggraph.shared.utils as shared_utils

    improve_module = import_module("xinggraph.api.v1.improve.improve")
    serve_state = import_module("xinggraph.api.v1.serve.state")
    memify_module = import_module("xinggraph.modules.memify")
    pipeline_module = import_module("xinggraph.memify_pipelines.global_context_index")
...
def test_improve_payload_global_context_index_defaults_to_false():
    router_module = import_module("xinggraph.api.v1.improve.routers.get_improve_router")
