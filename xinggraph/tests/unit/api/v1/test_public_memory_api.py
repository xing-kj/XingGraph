...
# Patching by the dotted string "xinggraph.api.v1.serve.state…" fails on
# Python 3.10's unittest.mock because `xinggraph.api.v1.serve` is shadowed by
# the `serve()` function re-exported in xinggraph/api/v1/__init__.py — mock's
# pre-3.11 dotted-path walk hits the function and raises AttributeError.
from xinggraph.api.v1.serve import state as serve_state
...
def _search_methods_module():
    return importlib.import_module("xinggraph.modules.search.methods.search")


@pytest.mark.asyncio
async def test_xinggraph_remember_public_api_completes():
    import xinggraph
    ...
    with (
        patch("xinggraph.shared.utils.send_telemetry"),
        patch.object(serve_state, "get_remote_client", return_value=None),
        patch("xinggraph.modules.engine.operations.setup.setup", AsyncMock()),
        patch("xinggraph.modules.users.methods.get_default_user", AsyncMock(return_value=mock_user)),
        patch.object(
            remember_mod,
            "resolve_authorized_user_datasets",
            AsyncMock(return_value=(mock_user, [mock_dataset])),
        ),
        patch("xinggraph.api.v1.add.add", AsyncMock()) as add,
        patch("xinggraph.api.v1.cognify.cognify", AsyncMock(return_value={})) as cognify,
    ):
    ...
    assert isinstance(result, xinggraph.RememberResult)
    assert result.status == "completed"
    assert result.dataset_name == "public_api"


@pytest.mark.asyncio
async def test_xinggraph_recall_public_api_resolves_default_user_for_graph_search():
    import xinggraph
