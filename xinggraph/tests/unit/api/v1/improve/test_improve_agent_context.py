...
@pytest.fixture
def improve_mod():
    return importlib.import_module("xinggraph.api.v1.improve.improve")
...
def _patch_extractor(monkeypatch, behavior):
    ace_mod = importlib.import_module("xinggraph.infrastructure.session.agent_context_extraction")
    calls = []
