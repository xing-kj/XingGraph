"""Tests for the memory_only option in xinggraph.forget().
...
forget_module = importlib.import_module("xinggraph.api.v1.forget.forget")
serve_state_module = importlib.import_module("xinggraph.api.v1.serve.state")
delete_dataset_nodes_and_edges_module = importlib.import_module(
    "xinggraph.modules.graph.methods.delete_dataset_nodes_and_edges"
)
delete_data_nodes_and_edges_module = importlib.import_module(
    "xinggraph.modules.graph.methods.delete_data_nodes_and_edges"
)
reset_dataset_pipeline_run_status_module = importlib.import_module(
    "xinggraph.modules.pipelines.layers.reset_dataset_pipeline_run_status"
)
...
async def test_forget_memory_only_without_dataset_raises(monkeypatch):
    ...
    with (
        patch("xinggraph.shared.utils.send_telemetry", lambda *a, **kw: None),
        patch.object(serve_state_module, "get_remote_client", return_value=None),
        patch("xinggraph.low_level.setup", AsyncMock()),
        patch("xinggraph.modules.users.methods.get_default_user", AsyncMock(return_value=USER)),
        patch.object(
...
async def test_forget_telemetry_target_labels(monkeypatch):
    ...
    with (
        patch("xinggraph.shared.utils.send_telemetry", fake_telemetry),
        patch.object(serve_state_module, "get_remote_client", return_value=None),
        patch("xinggraph.low_level.setup", AsyncMock()),
        patch("xinggraph.modules.users.methods.get_default_user", AsyncMock(return_value=USER)),
        patch.object(
