...
from xinggraph.api.v1.session import SessionQAEntry
from xinggraph.modules.recall.types.RecallResponse import ResponseQAEntry
...
_mod_sm = importlib.import_module("xinggraph.infrastructure.session.get_session_manager")
_pkg_improve = importlib.import_module("xinggraph.api.v1.improve")
_mod_query_router = importlib.import_module("xinggraph.api.v1.recall.query_router")
_mod_search_methods = importlib.import_module("xinggraph.modules.search.methods.search")


@contextmanager
def _patch_remember_startup():
    with (
        patch("xinggraph.modules.migrations.startup.run_migrations_and_block", new=AsyncMock()),
        patch("xinggraph.modules.engine.operations.setup.setup", new=AsyncMock()),
    ):
        yield


def _get_remember_module():
    return importlib.import_module("xinggraph.api.v1.remember.remember")


def _get_improve_module():
    return importlib.import_module("xinggraph.api.v1.improve")


def _get_query_router_module():
    return importlib.import_module("xinggraph.api.v1.recall.query_router")
...
async def test_remember_passes_session_ids_to_improve():
    ...
    with (
        ...
        patch("xinggraph.api.v1.add.add", AsyncMock()),
        patch("xinggraph.api.v1.cognify.cognify", AsyncMock(return_value={"status": "ok"})),
        patch.object(_pkg_improve, "improve", mock_improve),
        patch(
            "xinggraph.modules.users.methods.get_default_user",
...
class TestRememberResult:
    ...
    async def test_remember_returns_remember_result(self):
        ...
        with (
            ...
            patch("xinggraph.api.v1.add.add", AsyncMock()),
            patch(
                "xinggraph.api.v1.cognify.cognify",
...
class TestRememberResultSessions:
    ...
    async def test_remember_permanent_carries_session_ids(self):
        ...
        with (
            _patch_remember_startup(),
            patch("xinggraph.api.v1.add.add", AsyncMock()),
            patch("xinggraph.api.v1.cognify.cognify", AsyncMock(return_value={})),
            patch.object(_pkg_improve, "improve", AsyncMock()),
            patch(
                "xinggraph.modules.users.methods.get_default_user",
                AsyncMock(return_value=mock_user),
...
class TestSearchSession:
    ...
    async def test_word_boundary_matching(self):
        ...
        from xinggraph.api.v1.recall.recall import _search_session
...
def _get_recall_module():
    ...
    return importlib.import_module("xinggraph.api.v1.recall.recall")


class TestRecallSessionMode:
    @pytest.fixture(autouse=True)
    def _disable_telemetry(self, monkeypatch):
        monkeypatch.setattr("xinggraph.shared.utils.send_telemetry", lambda *args, **kwargs: None)
