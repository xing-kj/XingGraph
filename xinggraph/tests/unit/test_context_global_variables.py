...
async def test_dataset_database_configs_persist_after_exit(monkeypatch):
    ...
    monkeypatch.setattr(
        "xinggraph.context_global_variables.backend_access_control_enabled", lambda: True
    )
    monkeypatch.setattr("xinggraph.context_global_variables.get_user", fake_get_user)
    monkeypatch.setattr(
        "xinggraph.context_global_variables.get_or_create_dataset_database",
        fake_get_or_create_dataset_database,
    )
    monkeypatch.setattr(
        "xinggraph.context_global_variables.resolve_dataset_database_connection_info",
        fake_resolve_connection_info,
    )
    monkeypatch.setattr(
        "xinggraph.infrastructure.databases.dataset_queue.dataset_queue", FakeDatasetQueue
    )
