...
async def test_persist_agent_trace_feedbacks_pipeline_wires_memify_tasks():
    ...
    with (
        patch(
            "xinggraph.memify_pipelines.persist_agent_trace_feedbacks_in_knowledge_graph.set_session_user_context_variable",
            new=AsyncMock(),
        ) as set_user_ctx,
        patch(
            "xinggraph.memify_pipelines.persist_agent_trace_feedbacks_in_knowledge_graph.get_authorized_existing_datasets",
            new=AsyncMock(return_value=[authorized_dataset]),
        ) as get_authorized_dataset,
        patch(
            "xinggraph.memify_pipelines.persist_agent_trace_feedbacks_in_knowledge_graph.set_database_global_context_variables",
            new=AsyncMock(),
        ) as set_db_ctx,
        patch(
            "xinggraph.memify_pipelines.persist_agent_trace_feedbacks_in_knowledge_graph.memify",
