...
def test_run_command_polls_until_completed(monkeypatch):
    ...
    result = module.run_command(client, "xinggraph-api", ["echo", "hi"], poll_interval=0)
    ...
    assert fake.exec_calls[0]["sandbox_name"] == "xinggraph-api"
...
def test_run_command_returns_on_failed_and_timeout_statuses(monkeypatch, terminal_status):
    ...
    result = module.run_command(client, "xinggraph-api", ["false"], poll_interval=0)
...
def test_run_command_raises_when_exec_never_terminal(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="terminal status"):
        module.run_command(client, "xinggraph-api", ["sleep", "inf"], poll_interval=0, max_wait=0)
...
def test_wait_for_sandbox_running_success(monkeypatch):
    ...
    sandbox = module.wait_for_sandbox_running(client, "xinggraph-api", poll_interval=0)
...
def test_wait_for_sandbox_running_raises_on_failed(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="failed"):
        module.wait_for_sandbox_running(client, "xinggraph-api", poll_interval=0)
...
def _deploy_with_fakes(monkeypatch):
    """Run deploy_xinggraph() against fully scripted fakes; returns (module, client)."""
    module = _import_module(monkeypatch)
    monkeypatch.setenv("ISLO_API_KEY", "test-key")
    ...
    sandbox = module.deploy_xinggraph()
...
def test_deploy_happy_path(monkeypatch):
    ...
    assert any("pip install 'xinggraph[api]'" in " ".join(command) for command in commands)
...
def test_deploy_aborts_when_install_fails(monkeypatch, install_status, install_exit_code):
    ...
    with pytest.raises(RuntimeError, match="Failed to install xinggraph"):
        module.deploy_xinggraph()
...
def test_deploy_aborts_when_server_start_fails(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="Failed to start xinggraph API server"):
        module.deploy_xinggraph()
...
def test_deploy_reports_existing_sandbox_conflict(monkeypatch):
    ...
    assert "delete_sandbox('xinggraph-api')" in str(excinfo.value)
