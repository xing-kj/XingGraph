"""Unit tests for xinggraph.push() and the COGX archive transport helpers.
...
from xinggraph.modules.migration.archive import (
    ARCHIVE_SUFFIX,
    find_archive_root,
    pack_archive,
    unpack_archive,
)
from xinggraph.modules.migration.cogx import (
    COGXArchiveWriter,
    COGXEntity,
    COGXFact,
    read_archive,
...
def _write_sample_archive(directory):
    with COGXArchiveWriter(directory, source_system="xinggraph") as writer:
        writer.write(COGXEntity(external_system="xinggraph", external_id="e1", name="Alice"))
        writer.write(COGXEntity(external_system="xinggraph", external_id="e2", name="Bob"))
        writer.write(
            COGXFact(
...
class TestArchiveTransport:
    def test_pack_unpack_round_trip(self, tmp_path):
        ...
        assert manifest.source_system == "xinggraph"
...
class TestResolveClient:
    def _clear_sources(self, monkeypatch):
        from xinggraph.api.v1.serve import credentials as credentials_module
        from xinggraph.api.v1.serve import state

        monkeypatch.setattr(state, "_remote_client", None)
        monkeypatch.setattr(credentials_module, "load_credentials", lambda: None)
        monkeypatch.delenv("XINGGRAPH_SERVICE_URL", raising=False)
        monkeypatch.delenv("XINGGRAPH_API_KEY", raising=False)

    def test_explicit_url_wins(self, monkeypatch):
        from xinggraph.api.v1.push.push import _resolve_client
    ...
    def test_saved_credentials_used(self, monkeypatch):
        from xinggraph.api.v1.push.push import _resolve_client
        from xinggraph.api.v1.serve.credentials import CloudCredentials
    ...
    def test_env_fallback(self, monkeypatch):
        ...
        self._clear_sources(monkeypatch)
        monkeypatch.setenv("XINGGRAPH_SERVICE_URL", "https://env.example")
        monkeypatch.setenv("XINGGRAPH_API_KEY", "env-key")
        client, created = _resolve_client(None, None)
        assert created is True
    ...
    def test_env_beats_saved_credentials(self, monkeypatch):
        """Precedence matches serve(): XINGGRAPH_SERVICE_URL wins over saved creds."""
        from xinggraph.api.v1.push.push import _resolve_client
        from xinggraph.api.v1.serve import credentials as credentials_module
        from xinggraph.api.v1.serve.credentials import CloudCredentials
    ...
    def test_no_connection_raises(self, monkeypatch):
        ...
        self._clear_sources(monkeypatch)
        with pytest.raises(RuntimeError, match="xinggraph-cli serve"):
...
def _fake_export(dataset_name="main_dataset", num_nodes=2, num_edges=1):
    from xinggraph.modules.migration.export import ExportResult
...
class TestPush:
    def _patch(self, monkeypatch, fake_export, fake_client):
        push_module = importlib.import_module("xinggraph.api.v1.push.push")
        ...
        export_module = importlib.import_module("xinggraph.api.v1.export.export")
    ...
    def test_push_invalid_mode(self):
        from xinggraph.api.v1.push.push import push
    ...
    def test_push_result_is_typed_dataclass(self, monkeypatch):
        ...
        from xinggraph.api.v1.push.push import PushResult
