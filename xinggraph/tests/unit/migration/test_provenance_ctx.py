...
``add_data_points`` (``_provenance_ctx`` in ``xinggraph/modules/migration/loader.py``).
...
from xinggraph.modules.migration.loader import _provenance_ctx
from xinggraph.modules.pipelines.models.PipelineContext import PipelineContext
...
def test_every_pipeline_context_field_survives_adaptation():
    ...
    for context_field in dataclasses.fields(PipelineContext):
        assert hasattr(adapted, context_field.name), (
            ...
            "ctx adaptation (_provenance_ctx in xinggraph/modules/migration/loader.py). "
