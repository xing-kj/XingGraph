...
These exercise the xinggraph -> COGX -> xinggraph path on synthetic (nodes, edges)
...
from xinggraph.modules.migration import loader
from xinggraph.modules.migration.archive import pack_archive, unpack_archive
from xinggraph.modules.migration.cogx import COGX_VERSION, read_archive, read_manifest
from xinggraph.modules.migration.export import _write_cogx
from xinggraph.modules.migration.loader import translate_records
...
def sample_graph():
    """A small xinggraph graph in get_graph_data() shape.
...
# contract for archives produced by older xinggraph versions / existing backups.
FROZEN_MANIFEST = json.dumps(
    {
        "cogx_version": "0.1",
        "source_system": "xinggraph",
        "counts": {"entity": 2, "fact": 1, "raw_node": 1},
    }
)
FROZEN_ENTITIES = (
    '{"kind": "entity", "external_system": "xinggraph", "external_id": "ent-alice", "name": "Alice", "description": "A person"}\n'
    '{"kind": "entity", "external_system": "xinggraph", "external_id": "ent-berlin", "name": "Berlin", "description": "A city"}\n'
)
FROZEN_FACTS = (
    '{"kind": "fact", "external_system": "xinggraph", "external_id": "fact-1", '
...
class TestArchiveTransportRoundTrip:
    ...
    ``COGXArchiveSource`` — the path ``xinggraph.push()`` and restore actually use.
    ...
    def test_export_pack_unpack_reimport_preserves_graph(self, tmp_path):
        ...
        assert source.source_system == "xinggraph"
...
class TestExportDatasetSelfLoops:
    def test_self_loops_filtered_before_emitters_and_counts(self, tmp_path, monkeypatch):
        ...
        import xinggraph.context_global_variables as global_variables
        import xinggraph.infrastructure.databases.graph as graph_module
        import xinggraph.modules.data.methods as data_methods

        from xinggraph.modules.migration.export import export_dataset
