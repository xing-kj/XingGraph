...
from xinggraph.api.v1.remember.remember import RememberResult, remember
from xinggraph.modules.migration.cogx import COGXDocument, COGXEntity, COGXFact, COGXRecord
...
# same-named functions (e.g. ``xinggraph.api.v1.remember.remember``).
add_module = importlib.import_module("xinggraph.api.v1.add")
migrations_startup_module = importlib.import_module("xinggraph.modules.migrations.startup")
pipeline_module = importlib.import_module("xinggraph.modules.run_custom_pipeline")
remember_module = importlib.import_module("xinggraph.api.v1.remember.remember")
serve_state = importlib.import_module("xinggraph.api.v1.serve.state")
shared_utils = importlib.import_module("xinggraph.shared.utils")
storage_module = importlib.import_module("xinggraph.tasks.storage.add_data_points")
...
class TestImportMemorySourceModes:
    ...
    def test_preserve_streaming_flushes_in_bounded_batches(self, monkeypatch):
        sinks = install_sinks(monkeypatch)
        loader_module = importlib.import_module("xinggraph.modules.migration.loader")
...
class TestNodeIdScheme:
    """Imported entities must carry xinggraph's CURRENT class-namespaced ids
    ...
    def test_imported_entity_ids_use_xinggraph_identity_scheme(self, monkeypatch):
    ...
    def _xinggraph_archive_records(self):
        a1, a2, b = str(uuid4()), str(uuid4()), str(uuid4())
        records = [
            COGXEntity(external_system="xinggraph", external_id=a1, name="Alice"),
            COGXEntity(external_system="xinggraph", external_id=a2, name="Alice"),
            COGXEntity(external_system="xinggraph", external_id=b, name="Bob"),
            COGXFact(
                external_system="xinggraph",
                external_id="f1",
                subject_ref=a1,
                predicate="knows",
                object_ref=b,
    ...
    def test_xinggraph_archives_preserve_source_uuids(self, monkeypatch, replayable):
        """XingGraph-to-xinggraph transfers are exact copies: entity nodes keep the
        ...
        sinks = install_sinks(monkeypatch)
        records, alice_ids = self._xinggraph_archive_records()
        source = FakeSource(records, mode="preserve")
        source.source_system = "xinggraph"
...
class TestSocialLayerRestore:
    ...
    def test_import_runs_as_archived_owner_and_applies_grants(self, monkeypatch):
        import xinggraph.modules.migration.import_source as import_source_module
    ...
    def test_social_layer_requires_superuser_importer(self, monkeypatch):
        ...
        from xinggraph.modules.users.exceptions.exceptions import PermissionDeniedError
...
class TestSourceRevisionRestamp:
    """The import aligns the target's migration stamp with a xinggraph-origin
    ...
    def test_stamps_backward_when_archive_is_behind(self):
        from xinggraph.modules.migration.import_source import _revision_to_stamp
    ...
    def test_source_without_revision_skips_restamp_without_db_access(self):
        ...
        from xinggraph.modules.migration.import_source import _restamp_to_source_revision
...
class TestRememberDispatch:
    ...
    def test_remote_client_guard_raises_with_push_hint(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match=r"xinggraph\.push\(\)"):
