"""Tests for startup migrations (xinggraph/modules/migrations/startup.py + runner).

Originally written against xinggraph/run_migrations.py; the implementation moved
to xinggraph.modules.migrations.{startup,runner} (run_migrations.py is now a thin
shim). The original intents are preserved:

  * Alembic runs IN-PROCESS so programmatic xinggraph config is honored
...
def _reset_startup_flag():
    startup = importlib.import_module("xinggraph.modules.migrations.startup")
...
class TestRunRelationalMigrations(unittest.TestCase):
    """Alembic must run IN-PROCESS so programmatic xinggraph config is honored.

    A subprocess (the previous implementation) inherits only environment
    variables, so tests/examples that set roots via
    ``xinggraph.config.system_root_directory(...)`` had their migrations applied
    ...
    def test_runs_alembic_in_process_without_logger_clobbering(self):
        shim = importlib.import_module("xinggraph.run_migrations")
        startup = importlib.import_module("xinggraph.modules.migrations.startup")
...
class TestMissingBookkeepingTables(unittest.TestCase):
    ...
    def _assert_skips(self, error):
        ...
        runner = importlib.import_module("xinggraph.modules.migrations.runner")
...
class TestStartupMigrationsBootstrap(unittest.TestCase):
    ...
    def test_fresh_db_creates_schema_and_stamps_head(self):
        ...
        with (
            ...
            patch(
                "xinggraph.infrastructure.databases.relational.get_relational_engine",
                return_value=db_engine,
            ),
            patch(
                "xinggraph.modules.migrations.runner.run_database_migrations",
