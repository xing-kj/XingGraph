...
Scenario:
    ...
    - Three concurrent ``xinggraph.add`` calls on three different datasets.
    - Three concurrent ``xinggraph.cognify`` calls on the same three datasets.
    - Three concurrent ``xinggraph.search`` calls on the same three datasets.
...
# Skip xinggraph's LLM-endpoint connectivity probe during ``setup()``. This
...
import xinggraph.shared.lru_cache as _lru_cache_module
from xinggraph.infrastructure.databases.dataset_queue import queue as _queue_module
from xinggraph.infrastructure.databases.dataset_queue import dataset_queue
...
from xinggraph.modules.users.methods.get_default_user import get_default_user

_lru_cache_module.DATABASE_MAX_LRU_CACHE_SIZE = 1
_queue_module.DATABASE_MAX_LRU_CACHE_SIZE = 1

logger = get_logger("xinggraph.tests.dataset_queue_e2e")
...
async def main() -> None:
    ...
    system_directory_path = str((here / ".xinggraph_system/dataset_queue_e2e").resolve())
    ...
    for i in range(3):
        text.append(
            ...
            f"XingGraph will ingest this and queue-serialise the add pipeline."
        )

    dataset_names = [f"queue_e2e_ds_{i}" for i in range(3)]

    # ====================================================================
    # PHASE 1: concurrent xinggraph.add on 3 different datasets
    # ====================================================================
    events: List[Event] = []
    ...
    add_tasks = [
        asyncio.create_task(
            xinggraph.add(text[i], dataset_name=dataset_names[i]),
    ...
    print("\n=== Phase 1: xinggraph.add timeline ===")
    ...
    # ====================================================================
    # PHASE 2: concurrent xinggraph.cognify on the 3 datasets
    # ====================================================================
    events.clear()
    ...
    # queue-focused Phase 4 always runs. Set ``XINGGRAPH_E2E_RUN_LLM_PHASES=1``
    # to include them.
    run_llm_phases = os.getenv("XINGGRAPH_E2E_RUN_LLM_PHASES", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
    ...
    if run_llm_phases:
        cognify_tasks = [
            asyncio.create_task(
                xinggraph.cognify(datasets=[dataset_names[i]]),
        ...
        print("\n=== Phase 2: xinggraph.cognify timeline ===")
        print(_summarise(events))
        _assert_serialized(events, phase="cognify", min_expected_enters=3)
    else:
        print("\nPhase 2 (cognify) skipped — set XINGGRAPH_E2E_RUN_LLM_PHASES=1 to run.")

    # ====================================================================
    # PHASE 3: concurrent xinggraph.search across the 3 datasets
    # ====================================================================
    events.clear()
    ...
    if run_llm_phases:
        ...
        print("\n=== Phase 3: xinggraph.search timeline ===")
        print(_summarise(events))
        ...
        print("Phase 3 (search) skipped — set XINGGRAPH_E2E_RUN_LLM_PHASES=1 to run.")
    ...
    cap2_add_tasks = [
        asyncio.create_task(
            xinggraph.add(cap2_text[i], dataset_name=cap2_dataset_names[i]),
    ...
    print("\n=== Phase 4: xinggraph.add (max_concurrent=2, 4 datasets) timeline ===")
