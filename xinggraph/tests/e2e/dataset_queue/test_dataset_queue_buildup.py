...
from xinggraph.modules.graph.methods.get_formatted_graph_data import (
...
GET_DATASET_QUEUE_SETTINGS = (
    "xinggraph.infrastructure.databases.dataset_queue.queue.get_dataset_queue_settings"
)
...
async def test_max_concurrent_one_does_not_accumulate_slots(mock_llm):
    ...
    system_dir = os.path.join(
        pathlib.Path(__file__).parent, ".xinggraph_system/test_queue_max_concurrent_one"
    )
    xinggraph.config.data_root_directory(data_dir)
    xinggraph.config.system_root_directory(system_dir)
    ...
    with patch(GET_DATASET_QUEUE_SETTINGS) as mock_settings:
        ...
        # ---- add --------------------------------------------------------
        add_result = await xinggraph.add("John works for Apple.")
        data_id = add_result.data_ingestion_info[0]["data_id"]
        _assert_queue_empty(queue, "add")

        # ---- cognify ----------------------------------------------------
        cognify_result = await xinggraph.cognify()
