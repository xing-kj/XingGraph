...
async def main():
    # Import xinggraph first -- its __init__ calls dotenv.load_dotenv(override=True)
    # which overwrites os.environ from .env. We must set our overrides AFTER this.
    from xinggraph.tests.e2e.postgres.test_graphdb_shared import run_graph_db_test
    ...
    os.environ["DB_USERNAME"] = os.environ.get("DB_USERNAME", "xinggraph")
    os.environ["DB_PASSWORD"] = os.environ.get("DB_PASSWORD", "xinggraph")
    os.environ["DB_NAME"] = os.environ.get("DB_NAME", "xinggraph_db")
