...
async def run_graph_db_test(provider: str):
    ...
    system_dir = str((base / f".xinggraph_system/test_{provider}").resolve())
    ...
    try:
        xinggraph.config.set_graph_database_provider(provider)
        ...
        await xinggraph.add([nlp_path], dataset_name)
        ...
        await xinggraph.add([quantum_path], dataset_name)
        ...
        xinggraph.config.set_graph_database_provider(prev_provider)
        xinggraph.config.data_root_directory(prev_data_root)
        xinggraph.config.system_root_directory(prev_system_root)
