...
from xinggraph.infrastructure.databases.vector.embeddings.LiteLLMEmbeddingEngine import (
    LiteLLMEmbeddingEngine,
)
from xinggraph.infrastructure.llm import LLMGateway
from xinggraph.modules.engine.operations.setup import setup as setup_xinggraph
from xinggraph.modules.users.methods import get_default_user
from xinggraph.shared.data_models import Edge, KnowledgeGraph, Node, SummarizedContent
...
async def _setup(tmp_path):
    xinggraph.config.data_root_directory(str(tmp_path / "data"))
    xinggraph.config.system_root_directory(str(tmp_path / "system"))
    await xinggraph.prune.prune_data()
    await xinggraph.prune.prune_system(metadata=True)
    await setup_xinggraph()
...
async def _ingest_single_run():
    ...
    r1 = await xinggraph.add(DOC1)
    r2 = await xinggraph.add(DOC2)
    r3 = await xinggraph.add(DOC3)
