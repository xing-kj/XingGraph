...
from xinggraph.modules.cognify.rollback import cognify_rollback_handler
...
from xinggraph.modules.graph.models import Edge, Node
from xinggraph.modules.pipelines.models import PipelineContext, PipelineRun, PipelineRunStatus
...
from xinggraph.tasks.storage.add_data_points import add_data_points
...
async def _mock_structured_output(
    ...
    **_kwargs,
    ...
    from xinggraph.shared.data_models import Edge as KGEdge
    from xinggraph.shared.data_models import KnowledgeGraph, Node as KGNode, SummarizedContent
...
async def test_cognify_rollback_integration_keeps_preexisting_data_when_pipeline_fails_mid_execution(
    ...
    add_old = await xinggraph.add("Older baseline data", dataset_name=dataset_name, user=user)
    ...
    cognify_module = importlib.import_module("xinggraph.api.v1.cognify.cognify")
    ...
    cognify_result = await xinggraph.cognify(datasets=[dataset_name], user=user)
...
async def test_cognify_rollback_scope_isolated_across_users_and_datasets(clean_test_environment):
    ...
    add_a = await xinggraph.add("A dataset content", dataset_name=dataset_a.name, user=user_a)
    add_b = await xinggraph.add("B dataset content", dataset_name=dataset_b.name, user=user_b)
...
async def test_cognify_startup_recovery_rolls_back_stale_started_runs(clean_test_environment):
    ...
    add_result = await xinggraph.add("Recovery test data", dataset_name=dataset.name, user=user)
...
async def test_cognify_rollback_is_idempotent(clean_test_environment):
    ...
    add_result = await xinggraph.add("Idempotency test data", dataset_name=dataset.name, user=user)
    data_id = add_result.data_ingestion_info[0]["data_id"]
...
async def test_cognify_rollback_preserves_legacy_rows_without_pipeline_run_id(
    ...
    add_legacy = await xinggraph.add("Legacy data", dataset_name=dataset.name, user=user)
    add_new = await xinggraph.add("New data", dataset_name=dataset.name, user=user)
