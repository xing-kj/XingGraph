...
local stores (no LLM) and asserts a hard-deleted row is purged from xinggraph.
"""
...
async def clean_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XINGGRAPH_SKIP_CONNECTION_TEST", "true")  # no LLM/embedding ping
...
async def test_foreground_add_runs_deferred_orphan_cleanup(clean_env):
    ...
    # foreground path never awaited orphan_cleanup, so 'b' lingered in xinggraph.
    await xinggraph.add(_dlt_source([{"id": "b", "_deleted": True}]), dataset_name=DATASET, **kwargs)
