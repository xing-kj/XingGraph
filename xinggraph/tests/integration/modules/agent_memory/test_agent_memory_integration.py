"""Integration tests for the public xinggraph.agent_memory decorator behavior."""
...
from xinggraph.modules.agent_memory.runtime import get_current_agent_memory_context
...
async def agent_memory_integration_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XINGGRAPH_SKIP_CONNECTION_TEST", "true")
    ...
    xinggraph.config.set_vector_db_url(str(root / "system" / "databases" / "xinggraph.lancedb"))
    ...
    with patch(
        ...
        from xinggraph.infrastructure.databases.cache.fscache.FsCacheAdapter import FSCacheAdapter
...
async def test_agent_memory_retrieves_memory_from_shared_dataset_with_read_and_write_permissions(
    ...
    """Retrieve XingGraph memory through the public decorator path."""
    ...
    await xinggraph.add(
        [
            (
                "The private codename for the first xinggraph agent_memory release is Maple Panda. "
    ...
    await xinggraph.cognify([dataset_name], user=owner)
    ...
    @xinggraph.agent_memory(
        ...
        user=member,
        dataset_name=dataset_name,
        memory_query_fixed="What is the private codename for xinggraph agent_memory?",
    )
    ...
    assert "Relevant XingGraph Memory:" in memory_context
...
@pytest.mark.asyncio
async def test_agent_memory_persists_full_trace_payload_with_xinggraph_memory_integration(
    agent_memory_integration_env,
    monkeypatch,
):
    """Decorator path should persist the full trace payload after XingGraph-memory retrieval."""
    session_manager = agent_memory_integration_env["session_manager"]
    owner = await create_user(f"owner_{uuid4().hex[:8]}@example.com", "example")
    member = await create_user(f"member_{uuid4().hex[:8]}@example.com", "example")
    ...
    assert "Relevant XingGraph Memory:" in trace_entry.memory_context
    ...
    assert trace_entry.session_feedback == (
        "test_agent_memory_persists_full_trace_payload_with_xinggraph_memory_integration"
...
@pytest.mark.asyncio
async def test_agent_memory_combines_session_and_xinggraph_memory_integration(
    agent_memory_integration_env,
    monkeypatch,
):
    """Session memory and XingGraph search memory should both appear in deterministic order."""
    session_manager = agent_memory_integration_env["session_manager"]
    owner = await create_user(f"owner_{uuid4().hex[:8]}@example.com", "example")
    member = await create_user(f"member_{uuid4().hex[:8]}@example.com", "example")
