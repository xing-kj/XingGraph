...
from xinggraph.modules.data.models import Data, DatasetData
...
from xinggraph.tasks.cleanup.cleanup_unused_data import cleanup_unused_data
...
async def test_last_accessed_updates_on_search():
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent.parent, ".xinggraph_system/test_last_accessed_updates"
    )
    ...
    await xinggraph.add([doc_text], dataset_name=dataset_name, user=user)
...
async def test_cleanup_unused_data_dry_run():
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent.parent, ".xinggraph_system/test_cleanup_dry_run"
    )
    xinggraph.config.system_root_directory(xinggraph_directory_path)
    ...
    data_ids = []
    for doc in docs:
        add_result = await xinggraph.add([doc], dataset_name=dataset_name, user=user)
...
async def test_cleanup_actual_deletion():
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent.parent, ".xinggraph_system/test_cleanup_actual"
    )
    xinggraph.config.system_root_directory(xinggraph_directory_path)
