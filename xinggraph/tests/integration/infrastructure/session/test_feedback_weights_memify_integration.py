...
from xinggraph.tasks.memify.apply_feedback_weights import apply_feedback_weights
from xinggraph.tasks.memify.extract_feedback_qas import extract_feedback_qas
from xinggraph.tasks.memify.feedback_weights_constants import (
...
def session_manager_with_backend(request):
    ...
    if backend == "fs":
        ...
        with (
            ...
            from xinggraph.infrastructure.databases.cache.redis.RedisAdapter import RedisAdapter
...
async def test_feedback_weights_first_run_then_idempotent(session_manager_with_backend):
    ...
    with (
        patch("xinggraph.tasks.memify.extract_feedback_qas.session_user") as extract_user_ctx,
        patch("xinggraph.tasks.memify.apply_feedback_weights.session_user") as apply_user_ctx,
        patch("xinggraph.tasks.memify.extract_feedback_qas.get_session_manager", return_value=sm),
        patch("xinggraph.tasks.memify.apply_feedback_weights.get_session_manager", return_value=sm),
        patch("xinggraph.tasks.memify.apply_feedback_weights.get_graph_engine", return_value=graph),
    ):
