...
import xinggraph
from xinggraph.context_global_variables import graph_db_config, vector_db_config
from xinggraph.infrastructure.databases.graph import get_graph_engine
from xinggraph.infrastructure.session.session_manager import SessionManager
from xinggraph.low_level import setup as xinggraph_setup
from xinggraph.memify_pipelines.persist_agent_trace_feedbacks_in_knowledge_graph import (
    persist_agent_trace_feedbacks_in_knowledge_graph_pipeline,
)
from xinggraph.memify_pipelines.persist_sessions_in_knowledge_graph import (
...
def _reset_cache_backend_caches():
    from xinggraph.infrastructure.databases.cache.config import get_cache_config
    from xinggraph.infrastructure.databases.cache.get_cache_engine import create_cache_engine
...
async def session_persistence_env(event_loop):
    """Clean xinggraph env with one dataset (add + cognify); shared by all tests in this module."""
    with (
        patch.dict(
            os.environ,
            {"CACHE_BACKEND": "fs", "XINGGRAPH_SKIP_CONNECTION_TEST": "true"},
            clear=False,
        ),
        tempfile.TemporaryDirectory(prefix="xinggraph_session_persistence_system_") as system_path,
        tempfile.TemporaryDirectory(prefix="xinggraph_session_persistence_data_") as data_path,
    ):
        ...
        xinggraph.config.set_graph_database_provider("ladybug")
        ...
        xinggraph.config.set_relational_db_config({"db_provider": "sqlite"})
        xinggraph.config.set_migration_db_config({"migration_db_provider": "sqlite"})
        xinggraph.config.system_root_directory(system_path)
        xinggraph.config.data_root_directory(data_path)
        xinggraph.config.set_vector_db_url(
            str(pathlib.Path(system_path) / "databases" / "xinggraph.lancedb")
        )

        await _reset_engines_and_prune()
        await xinggraph_setup()

        dataset_name = "session_persist_integration"
        await xinggraph.add("XingGraph builds knowledge graphs from text.", dataset_name=dataset_name)
        await xinggraph.cognify(datasets=[dataset_name])
...
def session_manager_with_qa(request):
    ...
    if backend == "fs":
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch(
                "xinggraph.infrastructure.databases.cache.fscache.FsCacheAdapter.get_storage_config",
                return_value={"data_root_directory": tmpdir},
            ):
                from xinggraph.infrastructure.databases.cache.fscache.FsCacheAdapter import (
                    FSCacheAdapter,
                )
        ...
        patch_mod = "xinggraph.infrastructure.databases.cache.redis.RedisAdapter"
        with (
            patch(f"{patch_mod}.redis.Redis", return_value=MagicMock(ping=MagicMock())),
            patch(f"{patch_mod}.aioredis.Redis", return_value=store),
        ):
            from xinggraph.infrastructure.databases.cache.redis.RedisAdapter import (
                RedisAdapter,
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            from xinggraph.infrastructure.databases.cache.sql.SqlCacheAdapter import (
                SqlCacheAdapter,
            )
...
async def test_persist_sessions_in_knowledge_graph_uses_session_manager(
    ...
    await session_manager.add_qa(
        user_id=user_id,
        question="What does XingGraph do?",
        context="Intro to XingGraph",
        answer="XingGraph builds knowledge graphs from text.",
        session_id="integration_test_session",
    )

    extract_module = sys.modules["xinggraph.tasks.memify.extract_user_sessions"]
...
async def test_persist_agent_trace_feedbacks_in_knowledge_graph_uses_session_manager(
    ...
    extract_module = sys.modules["xinggraph.tasks.memify.extract_agent_trace_feedbacks"]
    with patch.object(extract_module, "get_session_manager", return_value=session_manager):
...
async def test_persist_sessions_watermark_ingests_everything_exactly_once(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch(
            ...
            extract_module = sys.modules["xinggraph.tasks.memify.extract_user_sessions"]
            cognify_module = sys.modules["xinggraph.tasks.memify.cognify_session"]
            with (
