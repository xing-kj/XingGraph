...
from xinggraph.infrastructure.databases.cache.sql.SqlCacheAdapter import SqlCacheAdapter
from xinggraph.infrastructure.databases.cache.sql.tables import cache_qa_entries
...
from xinggraph.infrastructure.session.session_turn import SessionTurnPreparation
...
def session_vector_mocks():
    with (
        patch(
            "xinggraph.infrastructure.session.session_manager.index_session_qa", new_callable=AsyncMock
        ),
        patch(
...
async def test_generate_completion_with_session_saves_qa(session_manager):
    ...
    mock_user.id = "u1"
    with (
        patch("xinggraph.infrastructure.session.session_manager.session_user") as mock_session_user,
