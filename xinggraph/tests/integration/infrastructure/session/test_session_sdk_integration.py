...
def _session_module():
    ...
    import xinggraph.api.v1.session  # noqa: F401

    return sys.modules["xinggraph.api.v1.session.session"]
...
async def test_sdk_get_session_returns_empty_when_no_data(sdk_uses_session_manager):
    """xinggraph.session.get_session returns [] when session has no QAs."""
    user = _user("u1")
    result = await xinggraph.session.get_session(session_id="s1", user=user)
    assert result == []


@pytest.mark.asyncio
async def test_sdk_get_session_returns_entries_after_add_qa(sdk_uses_session_manager):
    """Add QA via SessionManager, then retrieve via xinggraph.session.get_session."""
    user = _user("u1")
...
async def test_sdk_get_session_default_session_id(sdk_uses_session_manager):
    ...
    result = await xinggraph.session.get_session(user=user)
    assert len(result) == 1
...
async def test_sdk_get_session_last_n(sdk_uses_session_manager):
    ...
    result = await xinggraph.session.get_session(session_id="s1", user=user, last_n=2)
    assert len(result) == 2
    assert result[0].question == "Q1?"
...
async def test_sdk_add_feedback_roundtrip(sdk_uses_session_manager):
    ...
    ok = await xinggraph.session.add_feedback(
        session_id="s1",
        qa_id=qa_id,
    ...
    entries = await xinggraph.session.get_session(session_id="s1", user=user)
...
async def test_sdk_delete_feedback_roundtrip(sdk_uses_session_manager):
    ...
    ok = await xinggraph.session.delete_feedback(session_id="s1", qa_id=qa_id, user=user)
    assert ok is True
...
async def test_sdk_delete_feedback_returns_false_when_qa_not_found(sdk_uses_session_manager):
    ...
    user = _user("u1")
    ok = await xinggraph.session.delete_feedback(session_id="s1", qa_id="nonexistent-qa-id", user=user)
    assert ok is False
...
async def test_sdk_explicit_user_id_passed_to_cache(sdk_uses_session_manager):
    ...
    entries1 = await xinggraph.session.get_session(session_id="shared", user=user1)
    entries2 = await xinggraph.session.get_session(session_id="shared", user=user2)
