...
from xinggraph.infrastructure.session.feedback_models import FeedbackDetectionResult
from xinggraph.infrastructure.session.session_context_models import (
    CandidateContextUpdate,
    ServedContextRating,
    normalize_content,
)
from xinggraph.infrastructure.session.session_manager import SessionManager
from xinggraph.infrastructure.session.session_turn import apply_served_context_ratings
...
def _patches(completion_return, analysis_return=None):
    ...
    mock_user = patch("xinggraph.infrastructure.session.session_manager.session_user")
    mock_cfg = patch("xinggraph.infrastructure.session.session_manager.CacheConfig")
    mock_analyze = patch(
