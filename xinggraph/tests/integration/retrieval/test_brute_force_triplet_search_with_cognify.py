...
async def clean_environment():
    ...
    system_directory_path = str(base_dir / ".xinggraph_system/test_brute_force_triplet_search_e2e")
...
async def test_brute_force_triplet_search_end_to_end(clean_environment):
    ...
    text = """
        XingGraph is an open-source AI memory engine that structures data into searchable formats for use with AI agents.
        ...
        NLP systems can use XingGraph to store and retrieve structured information.
    """
    ...
    single_result = await brute_force_triplet_search(
        query="What can NLP systems use XingGraph for?",
        top_k=1,
    )
    ...
    batch_queries = ["What is XingGraph?", "What is the company's focus?"]
