...
from xinggraph.low_level import DataPoint
...
async def setup_test_environment_simple():
    ...
    system_directory_path = str(
        base_dir / ".xinggraph_system/test_graph_completion_decomposition_context_simple"
    )
    ...
    xinggraph.config.set_vector_db_config({"vector_db_provider": "lancedb"})
...
async def setup_test_environment_empty():
    ...
    base_dir = pathlib.Path(__file__).parent.parent.parent.parent
    system_directory_path = str(
        base_dir / ".xinggraph_system/test_graph_completion_decomposition_context_empty"
    )
    data_directory_path = str(
...
async def test_graph_completion_decomposition_combined_mode_context_coverage(
    ...
    with patch(
        "xinggraph.infrastructure.llm.LLMGateway.LLMGateway.acreate_structured_output",
...
async def test_graph_completion_decomposition_combined_mode_session_stores_only_final_qa(
    ...
    with (
        ...
        patch(
            "xinggraph.modules.retrieval.graph_completion_retriever.get_session_manager",
            return_value=session_manager,
        ),
        patch(
            "xinggraph.modules.retrieval.graph_completion_retriever.CacheConfig"
        ) as mock_retriever_cache_config,
        patch(
            "xinggraph.infrastructure.session.session_manager.CacheConfig"
        ) as mock_session_cache_config,
        patch(
            "xinggraph.modules.retrieval.graph_completion_retriever.session_user"
