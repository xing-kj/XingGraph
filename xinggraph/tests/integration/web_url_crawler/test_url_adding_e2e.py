...
from xinggraph.infrastructure.loaders.LoaderEngine import LoaderEngine
from xinggraph.infrastructure.loaders.external.beautiful_soup_loader import BeautifulSoupLoader
...
async def test_add_url():
    ...
    await xinggraph.add("http://example.com/")
...
async def test_beautiful_soup_loads_file_successfully():
    ...
    try:
        ...
        xinggraph_loaded_txt_path = await loader_engine.load_file(
            file_path=file_path, preferred_loaders=preferred_loaders
        )

        xinggraph_loaded_txt_path = get_data_file_path(xinggraph_loaded_txt_path)

        assert xinggraph_loaded_txt_path.endswith(".txt")

        extracted_file = Path(xinggraph_loaded_txt_path)
