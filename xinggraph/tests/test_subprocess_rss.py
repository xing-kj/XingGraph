...
Usage:
    python ./xinggraph/tests/test_subprocess_rss.py [options]
...
DATABASE_MAX_LRU_CACHE_SIZE must be set before xinggraph is imported — the
``@closing_lru_cache`` decorator captures it at import time — so argument
parsing and env-var setup happen at module top, before any ``import xinggraph``.
"""
...
# xinggraph. The LRU cache decorator reads it at class-definition time; setting
# the env var after ``import xinggraph`` has no effect.
ARGS = _parse_args()
...
from xinggraph_db_workers.harness import collect_garbage_in_all_workers  # noqa: E402
...
# per-repo-checkout.
LARGE_TEXT_CACHE_DIR = pathlib.Path(tempfile.gettempdir()) / "xinggraph_subprocess_rss_texts"
...
async def run_cycle(cycle_index: int, total_cycles: int, dataset_name: str, data) -> None:
    ...
    await xinggraph.add(data, dataset_name)
...
async def main() -> None:
    ...
    xinggraph.config.data_root_directory(str(base_dir / ".data_storage" / "test_subprocess_rss"))
    xinggraph.config.system_root_directory(str(base_dir / ".xinggraph_system" / "test_subprocess_rss"))
