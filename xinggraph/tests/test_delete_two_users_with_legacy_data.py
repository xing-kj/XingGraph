...
from xinggraph.infrastructure.databases.relational import get_relational_engine
...
from xinggraph.infrastructure.llm import LLMGateway
...
from xinggraph.modules.data.models import Data
...
from xinggraph.modules.graph.legacy.record_data_in_legacy_ledger import record_data_in_legacy_ledger
from xinggraph.modules.graph.utils.deduplicate_nodes_and_edges import deduplicate_nodes_and_edges
from xinggraph.modules.graph.utils.get_graph_from_model import get_graph_from_model
from xinggraph.modules.pipelines.models import DataItemStatus
from xinggraph.modules.users.methods import create_user, get_default_user
from xinggraph.shared.data_models import KnowledgeGraph, Node, Edge, SummarizedContent
from xinggraph.tasks.storage import index_data_points, index_graph_edges
from xinggraph.tests.utils.assert_edges_vector_index_not_present import (
    assert_edges_vector_index_not_present,
)
from xinggraph.tests.utils.assert_edges_vector_index_present import assert_edges_vector_index_present
from xinggraph.tests.utils.assert_graph_edges_not_present import assert_graph_edges_not_present
from xinggraph.tests.utils.assert_graph_edges_present import assert_graph_edges_present
from xinggraph.tests.utils.assert_graph_nodes_not_present import assert_graph_nodes_not_present
from xinggraph.tests.utils.assert_graph_nodes_present import assert_graph_nodes_present
from xinggraph.tests.utils.assert_nodes_vector_index_not_present import (
    assert_nodes_vector_index_not_present,
)
from xinggraph.tests.utils.assert_nodes_vector_index_present import assert_nodes_vector_index_present
from xinggraph.tests.utils.extract_entities import extract_entities
from xinggraph.tests.utils.extract_relationships import extract_relationships
from xinggraph.tests.utils.extract_summary import extract_summary
from xinggraph.tests.utils.filter_overlapping_entities import filter_overlapping_entities
from xinggraph.tests.utils.filter_overlapping_relationships import filter_overlapping_relationships
from xinggraph.tests.utils.get_contains_edge_text import get_contains_edge_text
...
async def main(mock_create_structured_output: AsyncMock):
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent,
        ".xinggraph_system/test_delete_two_users_with_legacy_data",
    )
    ...
    __, johns_legacy_data_points, johns_legacy_relationships = await create_mocked_data_points(john)

    johns_text = "John works for Apple. He is also affiliated with a non-profit organization called 'Food for Hungry'"
    add_john_result = await xinggraph.add(johns_text, user=john)
    johns_data_id = add_john_result.data_ingestion_info[0]["data_id"]

    johns_cognify_result: dict = await xinggraph.cognify(datasets=["main_dataset"], user=john)
    ...
    add_marie_result = await xinggraph.add(maries_text, user=marie)
    maries_data_id = add_marie_result.data_ingestion_info[0]["data_id"]

    maries_cognify_result: dict = await xinggraph.cognify(datasets=["main_dataset"], user=marie)
    ...
    await assert_graph_edges_present(johns_relationships + johns_legacy_relationships)
    ...
    await assert_graph_edges_present(maries_relationships + maries_legacy_relationships)
    ...
    # Assert data points presence in the graph, vector collections and nodes table
    await assert_graph_nodes_present(johns_legacy_data_points)
    await assert_nodes_vector_index_present(johns_legacy_data_points)
    ...
    await assert_graph_edges_present(johns_legacy_relationships)
    await assert_edges_vector_index_present(johns_legacy_relationships)
    ...
    # Assert data points presence in the graph, vector collections and nodes table
    await assert_graph_nodes_present(maries_legacy_data_points)
    await assert_nodes_vector_index_present(maries_legacy_data_points)

    await assert_graph_nodes_not_present(maries_data)
    await assert_nodes_vector_index_not_present(maries_data)

    # Assert relationships presence in the graph, vector collections and nodes table
    await assert_graph_edges_present(maries_legacy_relationships)
    await assert_edges_vector_index_present(maries_legacy_relationships)

    await assert_graph_edges_not_present(maries_relationships)
...
async def create_mocked_data_points(user):
    graph_engine = await get_graph_engine()
    legacy_data_points = create_legacy_data_points(user)
    ...
    dataset = await get_authorized_dataset_by_name("main_dataset", user, "write")
    if not dataset:
        dataset = await create_authorized_dataset(dataset_name="main_dataset", user=user)
...
def create_legacy_data_points(user):
    ...
    document = TextDocument(
        ...
        name=document_name,
        raw_data_location="git/xinggraph/examples/database_examples/data_storage/data/text_test.txt",
        external_metadata="{}",
