"""End-to-end tests for the public xinggraph.agent_memory feature."""
...
from xinggraph.modules.data.methods import create_authorized_dataset, get_datasets_by_name
...
async def test_agent_memory_e2e_requires_write_then_retrieves_shared_memory(agent_memory_e2e_env):
    ...
    await xinggraph.add(
        [f"The private codename for this xinggraph agent_memory e2e test is '{secret_codename}'."],
    ...
    @xinggraph.agent_memory(
        ...
        user=member,
        dataset_name=dataset_name,
        memory_query_fixed="What is the private codename for this xinggraph agent_memory e2e test?",
    ...
    async def shared_memory_agent() -> str:
        return await LLMGateway.acreate_structured_output(
            text_input="What is the private codename for this xinggraph agent_memory e2e test?",
            system_prompt=(
    ...
    with pytest.raises(XingGraphValidationError, match="both read and write permissions"):
