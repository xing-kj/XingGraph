...
async def test_delete_all_with_partial_permissions():
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent, ".xinggraph_system/test_delete_all_mixed_permissions"
    )
    ...
    # Add text data to all datasets using xinggraph.add()
    await xinggraph.add(["Document about Company A"], dataset_name="dataset_1", user=user_a)
    await xinggraph.add(["Document about Company B"], dataset_name="dataset_2", user=user_a)
    await xinggraph.add(["Document about Company C"], dataset_name="dataset_3", user=other_user)
    await xinggraph.add(["Document about Company D"], dataset_name="dataset_4", user=other_user)

    # Cognify all datasets to create graph data
    await xinggraph.cognify(["dataset_1"], user=user_a)
    await xinggraph.cognify(["dataset_2"], user=user_a)
    await xinggraph.cognify(["dataset_3"], user=other_user)
    await xinggraph.cognify(["dataset_4"], user=other_user)
...
async def test_delete_all_permission_error_handling():
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent, ".xinggraph_system/test_delete_all_no_permissions"
    )
    xinggraph.config.system_root_directory(xinggraph_directory_path)
    ...
    # Add data to dataset using xinggraph.add()
    await xinggraph.add(["Document about Company X"], dataset_name="dataset_x", user=owner_user)
    await xinggraph.cognify(["dataset_x"], user=owner_user)
