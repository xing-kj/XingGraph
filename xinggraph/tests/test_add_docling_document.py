...
async def main():
    ...
    await xinggraph.add(result.document)
    ...
    await xinggraph.add(file_path_pptx)

    await xinggraph.cognify()

    answer = await xinggraph.search("Tell me about Artificial Intelligence.")
    assert len(answer) != 0

    answer = await xinggraph.search("Do programmers change light bulbs?")
    assert len(answer) != 0
    lowercase_answer = answer[0]["search_result"][0].lower()
    assert ("no" in lowercase_answer) or ("none" in lowercase_answer)

    answer = await xinggraph.search("What colours are there in the presentation table?")
    assert len(answer) != 0
    lowercase_answer = answer[0]["search_result"][0].lower()
    assert (
