"""E2E: import a NON-xinggraph memory export (Zep/Graphiti graph shape) and
search it.

The cross-provider half of the COGX contract, driven through the public
``xinggraph.remember(ZepSource(...))`` path against real stores: external
entities get xinggraph's CURRENT class-namespaced ids (``Entity.id_for(name)``,
...
import xinggraph
from xinggraph.modules.migration.sources.zep import ZepSource
...
async def main():
    ...
    xinggraph.config.system_root_directory(
        str((base / ".xinggraph_system/test_external_source_import").resolve())
    )
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        result = await xinggraph.remember(source, dataset_name="zep_import")
    ...
    assert str(adas[0][0]) == str(Entity.id_for("Ada Lovelace")), (
        "External entities must use xinggraph's current class-namespaced ids."
    )
    assert any(
