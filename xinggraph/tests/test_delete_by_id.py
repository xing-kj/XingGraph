...
from xinggraph.modules.users.methods import get_default_user, create_user
...
from xinggraph.api.v1.exceptions import DocumentNotFoundError, DatasetNotFoundError
...
async def main():
    ...
    xinggraph_directory_path = str(
        pathlib.Path(
            os.path.join(pathlib.Path(__file__).parent, ".xinggraph_system/test_delete_by_id")
        ).resolve()
    )
    ...
    await xinggraph.add([text_1], dataset_name="tech_companies_1", user=default_user)
    ...
    await xinggraph.add([text_2], dataset_name="tech_companies_2", user=test_user)
    ...
    await xinggraph.add([text_3], dataset_name="tech_companies_3", user=isolation_user)

    tst = await xinggraph.cognify(["tech_companies_1"], user=default_user)
    tst2 = await xinggraph.cognify(["tech_companies_2"], user=test_user)
    tst3 = await xinggraph.cognify(["tech_companies_3"], user=isolation_user)
    print("tst", tst)
    print("tst2", tst2)
    print("tst3", tst3)
    ...
    try:
        result = await xinggraph.delete(data_id=data_to_delete_id, dataset_id=default_dataset.id)
    ...
    await xinggraph.add([text_1], dataset_name="another_dataset", user=default_user)
    await xinggraph.cognify(["another_dataset"], user=default_user)
    ...
    try:
        ...
        isolation_search_results = await xinggraph.search(
