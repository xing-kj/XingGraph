...
async def main():
    ...
    xinggraph_directory_path = str(
        pathlib.Path(
            os.path.join(pathlib.Path(__file__).parent, ".xinggraph_system/test_custom_model")
        ).resolve()
    )
    ...
    await xinggraph.cognify(graph_model=ProgrammingLanguage)
    ...
    await xinggraph.visualize_graph(graph_file_path)

    # Completion query that uses graph data to form context.
    completion = await xinggraph.search("What is python?", SearchType.GRAPH_COMPLETION)
    ...
    completion = await xinggraph.search("What is Python?", SearchType.RAG_COMPLETION)
    ...
    summaries = await xinggraph.search("Python", SearchType.SUMMARIES)
    assert len(summaries) != 0, "Summaries search didn't return any results."
    print("Summary results are:")
    for summary in summaries:
        print(summary)

    chunks = await xinggraph.search("Python", SearchType.CHUNKS)
