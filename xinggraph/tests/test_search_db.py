...
async def _seed_default_dataset(dataset_name: str) -> dict:
    ...
    await xinggraph.add(text_1, dataset_name)
    ...
    await xinggraph.add([explanation_file_path_quantum], dataset_name)
...
async def setup_test_environment():
    ...
    from xinggraph.memify_pipelines.create_triplet_embeddings import create_triplet_embeddings
...
async def e2e_state(_disable_session_turn_gating):
    ...
    completion_gk = await xinggraph.search(
        query_type=SearchType.GRAPH_COMPLETION,
        query_text="Where is germany located, next to which country?",
        verbose=True,
    )
    completion_decomposition_answer_per_subquery = await xinggraph.search(
        query_type=SearchType.GRAPH_COMPLETION_DECOMPOSITION,
        query_text="Where is germany located, next to which country?",
        verbose=True,
    )
    completion_decomposition_combined_triplets = await xinggraph.search(
        query_type=SearchType.GRAPH_COMPLETION_DECOMPOSITION,
        query_text="Where is germany located, next to which country?",
        verbose=True,
        retriever_specific_config={"decomposition_mode": "combined_triplets_context"},
    )
    completion_cot = await xinggraph.search(
        query_type=SearchType.GRAPH_COMPLETION_COT,
        query_text="What is the country next to germany??",
        verbose=True,
    )
    completion_ext = await xinggraph.search(
        query_type=SearchType.GRAPH_COMPLETION_CONTEXT_EXTENSION,
        query_text="What is the name of the country next to germany",
        verbose=True,
    )

    completion_sum = await xinggraph.search(
        query_type=SearchType.GRAPH_SUMMARY_COMPLETION,
        query_text="Next to which country is Germany located?",
        verbose=True,
    )
    completion_triplet = await xinggraph.search(
        query_type=SearchType.TRIPLET_COMPLETION,
        query_text="Next to which country is Germany located?",
        verbose=True,
    )
    completion_chunks = await xinggraph.search(
        query_type=SearchType.CHUNKS,
        query_text="Germany",
        verbose=True,
    )
    completion_summaries = await xinggraph.search(
        query_type=SearchType.SUMMARIES,
        query_text="Germany",
        verbose=True,
    )
    completion_rag = await xinggraph.search(
        query_type=SearchType.RAG_COMPLETION,
        query_text="Next to which country is Germany located?",
        verbose=True,
    )
    completion_temporal = await xinggraph.search(
