...
import xinggraph
from xinggraph.modules.engine.operations.setup import setup as setup_databases
from xinggraph.modules.pipelines.models.PipelineRunInfo import (
    PipelineRunCompleted,
    PipelineRunErrored,
)
from xinggraph.modules.pipelines.queues.pipeline_run_info_queues import get_from_queue
...
async def test_two_background_cognify_runs_are_isolated_per_file():
    ...
    xinggraph.config.data_root_directory(
        str((test_root / ".data_storage" / "test_concurrent_cognify").resolve())
    )
    xinggraph.config.system_root_directory(
        str((test_root / ".xinggraph_system" / "test_concurrent_cognify").resolve())
    )
    ...
    add_one = await xinggraph.add(_make_document("Alan Turing"), dataset_name)
    data_id_one = str(add_one.data_ingestion_info[0]["data_id"])
    started_one = await xinggraph.cognify([dataset_name], run_in_background=True)
    ...
    add_two = await xinggraph.add(_make_document("Ada Lovelace"), dataset_name)
    data_id_two = str(add_two.data_ingestion_info[0]["data_id"])
    started_two = await xinggraph.cognify([dataset_name], run_in_background=True)
