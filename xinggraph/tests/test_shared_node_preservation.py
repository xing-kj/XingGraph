...
async def test_shared_entity_preserved_across_documents(mock_create_structured_output: AsyncMock):
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent, ".xinggraph_system/test_shared_node_preservation"
    )
    ...
    add_bmw_result = await xinggraph.add(bmw_text)
    ...
    add_netherlands_result = await xinggraph.add(netherlands_text)
...
async def test_delete_permission_checks_delete_not_read():
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent, ".xinggraph_system/test_delete_permission_check"
    )
    xinggraph.config.system_root_directory(xinggraph_directory_path)
...
async def test_dataset_deletion_removes_files():
    ...
    xinggraph_directory_path = os.path.join(
        pathlib.Path(__file__).parent, ".xinggraph_system/test_file_cleanup"
    )
    xinggraph.config.system_root_directory(xinggraph_directory_path)
    ...
    add_result_1 = await xinggraph.add([text_1], dataset_name=dataset_name, user=user)
    data_1_id = add_result_1.data_ingestion_info[0]["data_id"]

    add_result_2 = await xinggraph.add([text_2], dataset_name=dataset_name, user=user)
    data_2_id = add_result_2.data_ingestion_info[0]["data_id"]

    # Cognify to create graph
    cognify_result = await xinggraph.cognify([dataset_name], user=user)
    dataset_id = list(cognify_result.keys())[0]

    # Check that files/data exist in storage
    from xinggraph.modules.data.methods import get_data
    ...
    from xinggraph.modules.data.methods import get_dataset
    ...
    # For text data added via xinggraph.add(), the files are stored in the data directory
