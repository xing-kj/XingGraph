...
async def apply_feedback_weights_pipeline(
    ...
    if (
        ...
        raise XingGraphValidationError(message="session_ids must be a non-empty list", log=False)

    if alpha <= 0 or alpha > 1:
        raise XingGraphValidationError(message="alpha must be in range (0, 1]", log=False)
    if not isinstance(batch_size, int) or batch_size < 1:
        raise XingGraphValidationError(message="batch_size must be a positive integer", log=False)
