...
def analyze_llm_model(
    ...
    reduction_milestones = {
        factor: queries_for_reduction(full_context_cost, xinggraph_cost, factor)
        for factor in args.reduction_factors
    }
    return assemble(
        chunk_measurements, average, full_context_cost, xinggraph_cost, reduction_milestones
    )
...
def assemble(
    ...
    return {
        "ingestion_multiplier": round(
            xinggraph_cost.ingestion_tokens / full_context_cost.corpus_tokens, 3
        ),
        "full_context": {
        ...
        "xinggraph": {
            "ingestion_tokens": round(xinggraph_cost.ingestion_tokens),
            "retrieved_context_tokens": xinggraph_cost.retrieved_context_tokens,
            "per_query_tokens": xinggraph_cost.retrieved_context_tokens,
        },
        "reduction_milestones": {
