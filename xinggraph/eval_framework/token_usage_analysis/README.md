...
Estimate the token cost of **xinggraph persistent memory** versus **full-context
...
xinggraph:        ingestion_tokens + queries × retrieved_context
```

- `ingestion_tokens` is the one-time cost of `xinggraph.remember()` — summarizing and
...
- `retrieved_context` is the (roughly constant) context `xinggraph.recall()` packs
  per query.
...
cumulative xinggraph cost, including xinggraph's one-time ingestion cost?
...
7× as many input/context tokens as xinggraph, so xinggraph uses 7× fewer input tokens
...
density, moves the numbers: xinggraph's `8191` default and `4095` (half). Values are
...
cd xinggraph/eval_framework/token_usage_analysis
```
...
The script chunks the input with xinggraph's `TextChunker`, samples a few chunks
(`--samples`, default 3), measures each through `xinggraph.remember()`'s summary and
...
| `--llm-models` | the `.env` model | comma list; runs each, switching xinggraph's config |
...
`--max-chunk-size` defaults to `4095`; pass `8191` for xinggraph's default.
...
- `corpus.py` — input → sampled chunks (xinggraph `TextChunker`). No LLM, no math.
- `measure.py` — the only LLM/IO surface: run a chunk through xinggraph and record
