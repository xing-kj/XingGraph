...
DESCRIPTION = "Estimate the token cost of xinggraph memory vs. full-context prompting."
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=_comma_list,
        default=None,
        help="comma list; default = the model xinggraph is configured with in .env",
    )
...
def _resolve_llm_models(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    """Default to the single llm_model xinggraph is configured with in .env."""
    if args.llm_models:
        return
