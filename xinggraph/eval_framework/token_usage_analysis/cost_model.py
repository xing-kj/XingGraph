"""Cost comparison between full-context prompting and xinggraph persistent memory.
...
@dataclass(frozen=True)
class XingGraphQueryCost:
    """Cost of answering queries via xinggraph: ingest once, then retrieve per query."""

    ingestion_tokens: float  # one-time xinggraph.remember() over the whole corpus
    retrieved_context_tokens: int  # xinggraph.recall() context per query (~constant at scale)
...
def queries_for_reduction(
    ...
    xinggraph_cost: XingGraphQueryCost,
    factor: float,
) -> float | None:
    """Queries at which full-context costs `factor`x as much as xinggraph.

    factor 1.0 is parity (the cross-over). Returns None when the factor is
    unreachable, i.e. xinggraph's per-query cost alone already exceeds the target.
    """
    full_per_query = full_context_cost.corpus_tokens + full_context_cost.query_overhead_tokens
    denominator = full_per_query - factor * xinggraph_cost.retrieved_context_tokens
    if denominator <= 0:
        return None
    return factor * xinggraph_cost.ingestion_tokens / denominator
