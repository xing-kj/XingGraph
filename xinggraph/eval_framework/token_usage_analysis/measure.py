"""Measure the real token cost of ingesting a chunk with xinggraph.

This is the only module that calls the LLM. Each sampled chunk is run through
xinggraph's summary and graph-extraction calls, and the real prompt/completion
token usage is read off each response (so instruction and schema overhead are
included). Multiple llm_models are run sequentially, switching xinggraph's config
between them.
"""
...
from xinggraph.infrastructure.llm.extraction import extract_content_graph, extract_summary
...
async def measure_chunk(chunk: str, llm_model: str) -> ChunkMeasurement:
    """Run xinggraph's two ingestion calls on one chunk and record real token usage."""
...
def _configure_llm_model(llm_model: str) -> None:
    """Switch xinggraph onto llm_model: set env, then clear the cached config."""
