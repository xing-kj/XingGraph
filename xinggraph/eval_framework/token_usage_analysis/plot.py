...
def _plot_crossover(name: str, llm_model: str, result: dict, out_dir: Path, plt) -> Path:
    ...
    ingestion = result["xinggraph"]["ingestion_tokens"]
    xinggraph_per_query = result["xinggraph"]["per_query_tokens"]
    ...
    xinggraph = [ingestion + q * xinggraph_per_query for q in queries]

    figure, axes = plt.subplots(figsize=(7, 4.5))
    axes.plot(queries, full_context, label="full-context")
    axes.plot(queries, xinggraph, label="xinggraph memory")
    ...
    axes.set_title(f"Full-context vs. xinggraph — {llm_model}")
    axes.legend()
