...
No LLM calls and no cost math here — just reading text and running xinggraph's
...
DEFAULT_MAX_CHUNK_SIZE = 4095  # half of xinggraph's 8191 default
...
def chunk_text(text: str, max_chunk_size: int = DEFAULT_MAX_CHUNK_SIZE) -> list[str]:
    """Split text into chunks using xinggraph's TextChunker."""
...
def _patch_chunker_tokenizer(tokenizer_model: str = CHUNKER_TOKENIZER_MODEL) -> None:
    """Point xinggraph's chunker at a local tokenizer so it needs no embedding model."""
    ...
    chunk_by_sentence = importlib.import_module("xinggraph.tasks.chunks.chunk_by_sentence")
