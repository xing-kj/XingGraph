...
DEFAULT_BEAM_RUBRIC_MAX_CONCURRENT_CRITERIA = 4
...
def _rubric_max_concurrent_criteria() -> int:
    raw_value = os.getenv("XINGGRAPH_BEAM_RUBRIC_MAX_CONCURRENT_CRITERIA")
    if raw_value is None:
        return DEFAULT_BEAM_RUBRIC_MAX_CONCURRENT_CRITERIA

    try:
        return max(1, int(raw_value))
    except ValueError:
        logger.warning(
            "Invalid XINGGRAPH_BEAM_RUBRIC_MAX_CONCURRENT_CRITERIA=%r; using default %s",
            raw_value,
            DEFAULT_BEAM_RUBRIC_MAX_CONCURRENT_CRITERIA,
        )
        return DEFAULT_BEAM_RUBRIC_MAX_CONCURRENT_CRITERIA
...
class BEAMRubricMetric:
    ...
    async def a_measure(self, test_case) -> float:
        ...
        semaphore = asyncio.Semaphore(_rubric_max_concurrent_criteria())
        ...
        self.score = sum(verdict["score"] for verdict in verdicts) / len(rubric)
