...
ANSWERING_STRATEGIES: dict[str, AnsweringStrategySpec] = {
    "xinggraph_graph_completion": AnsweringStrategySpec(
        name="xinggraph_graph_completion",
        mode="fixed_retriever",
        cls=GraphCompletionRetriever,
    ),
    "xinggraph_graph_completion_cot": AnsweringStrategySpec(
        name="xinggraph_graph_completion_cot",
        mode="fixed_retriever",
        cls=GraphCompletionCotRetriever,
    ),
    "xinggraph_graph_completion_context_extension": AnsweringStrategySpec(
        name="xinggraph_graph_completion_context_extension",
        mode="fixed_retriever",
        cls=GraphCompletionContextExtensionRetriever,
    ),
    "xinggraph_completion": AnsweringStrategySpec(
        name="xinggraph_completion",
        mode="fixed_retriever",
    ...
    "hybrid_completion": AnsweringStrategySpec(
        name="hybrid_completion",
        mode="fixed_retriever",
        cls=HybridRetriever,
    ),
...
def build_sweep_config(
    ...
    *,
    ...
    if qa_prompt_paths is not None:
        config["qa_prompt_paths"] = dict(qa_prompt_paths)
    if agentic_prompt_paths is not None:
        config["agentic_prompt_paths"] = dict(agentic_prompt_paths)
