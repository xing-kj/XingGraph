...
3. concurrently:
   - ``xinggraph.add`` that one JSON file and ``xinggraph.cognify`` it with ``JsonListChunker``,
...
from xinggraph.eval_framework.beam.session_io import (
...
# Keep XingGraph's normal logging quiet; this script prints its own progress.
os.environ["LOG_LEVEL"] = "ERROR"
os.environ["XINGGRAPH_LOG_FILE"] = "false"
...
def import_xinggraph_modules():
    if os.getenv("BEAM_INGEST_SHOW_XINGGRAPH_LOGS") == "1":
        import xinggraph
        from xinggraph.infrastructure.session.feedback_detection import (
            analyze_turn_for_session_context,
        )
        from xinggraph.infrastructure.session.get_session_manager import get_session_manager
        from xinggraph.infrastructure.session.session_turn import apply_session_turn_analysis
        from xinggraph.memify_pipelines.global_context_index import global_context_index_pipeline
        from xinggraph.modules.chunking.JsonListChunker import JsonListChunker
...
def configure_quiet_logging() -> None:
    ...
    for logger_name in ("xinggraph", "cognify", "global_context_index", "session_distillation"):
...
async def maybe_prune(args: argparse.Namespace) -> None:
    ...
    print_step("Pruning existing XingGraph data/system state before ingestion")
...
async def ensure_database_setup() -> None:
    ...
    print_step("Ensuring XingGraph database tables exist")
    await setup()
...
async def distill_session(
    *,
    ...
    result = await xinggraph.session.distill_session(
        session_id,
        dataset=dataset_name,
        user=user,
    )
