...
from xinggraph.cli import SupportsCliCommand, DEFAULT_DOCS_URL
from xinggraph.cli.config import CLI_DESCRIPTION
from xinggraph.cli import debug
...
# Debug functionality is now in xinggraph.cli.debug module


def _discover_commands() -> List[Type[SupportsCliCommand]]:
    """Discover all available CLI commands"""
    # Import commands dynamically to avoid early xinggraph initialization
    commands = []

    command_modules = [
        ("xinggraph.cli.commands.add_command", "AddCommand"),
        ("xinggraph.cli.commands.search_command", "SearchCommand"),
        ("xinggraph.cli.commands.cognify_command", "CognifyCommand"),
        ("xinggraph.cli.commands.delete_command", "DeleteCommand"),
        ("xinggraph.cli.commands.config_command", "ConfigCommand"),
        ("xinggraph.cli.commands.datasets_command", "DatasetsCommand"),
        ("xinggraph.cli.commands.agents_command", "AgentsCommand"),
        ("xinggraph.cli.commands.sessions_command", "SessionsCommand"),
        ("xinggraph.cli.commands.feedback_command", "FeedbackCommand"),
        ("xinggraph.cli.commands.memify_command", "MemifyCommand"),
        ("xinggraph.cli.commands.remember_command", "RememberCommand"),
        ("xinggraph.cli.commands.recall_command", "RecallCommand"),
        ("xinggraph.cli.commands.improve_command", "ImproveCommand"),
        ("xinggraph.cli.commands.forget_command", "ForgetCommand"),
        ("xinggraph.cli.commands.serve_command", "ServeCommand"),
        ("xinggraph.cli.commands.migrate_command", "UpgradeCommand"),
        ("xinggraph.cli.commands.migrate_command", "DowngradeCommand"),
        ("xinggraph.cli.commands.migrate_command", "HistoryCommand"),
        ("xinggraph.cli.commands.migrate_command", "CurrentCommand"),
        ("xinggraph.cli.commands.migrate_command", "StampCommand"),
        ("xinggraph.cli.commands.push_command", "PushCommand"),
    ]
...
def _create_parser() -> tuple[argparse.ArgumentParser, Dict[str, SupportsCliCommand]]:
    ...
    parser.add_argument("--version", action="version", version=f"xinggraph {version}")
    parser.add_argument(
        "--debug",
        action=DebugAction,
    ...
    parser.add_argument(
        "-ui",
        action=UiAction,
        help="Start the xinggraph web UI interface",
    )
    parser.add_argument(
        "--user-id",
        default=None,
    ...
    parser.add_argument(
        "--api-url",
        default=None,
        help="Delegate commands to a running XingGraph API server (e.g. http://localhost:8000). "
        "Required for multi-agent / concurrent usage with file-based databases.",
    )
    parser.add_argument(
        "--api-key",
        default=None,
        help="API key sent as X-Api-Key when --api-url is set. Falls back to $XINGGRAPH_API_KEY.",
    )
    parser.add_argument(
        "--api-token",
        default=None,
        help="Bearer token sent as Authorization: Bearer <token> when --api-url is set. "
        "Falls back to $XINGGRAPH_API_TOKEN. Ignored if --api-key is also provided.",
    )
...
def main() -> int:
    ...
    if hasattr(args, "start_ui") and args.start_ui:
        ...
        try:
            from xinggraph import start_ui

            fmt.echo("Starting xinggraph UI...")
    ...
    from xinggraph.cli.api_dispatch import can_dispatch, dispatch as api_dispatch
