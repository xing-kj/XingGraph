...
# Tiny constants-only import; the xinggraph package is already loaded by the CLI.
from xinggraph.modules.migration.sources.base import IMPORT_MODES


class PushCommand(SupportsCliCommand):
    command_string = "push"
    help_string = "Upload a local dataset's knowledge graph to XingGraph Cloud"
...
Authentication reuses the serve credentials: run `xinggraph serve` once to log
in, then push any time. Alternatively pass --url/--api-key or set
XINGGRAPH_SERVICE_URL and XINGGRAPH_API_KEY.
...
Examples:
  xinggraph push                                  # push main_dataset
  xinggraph push my_dataset
  xinggraph push my_dataset --target-dataset prod_dataset
  xinggraph push my_dataset --mode hybrid
  xinggraph push --url https://my.xinggraph.ai --api-key ck_...
    """
    ...
    def configure_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "XINGGRAPH_SERVICE_URL, or saved serve credentials)"
            ),
        )
        parser.add_argument(
            "--api-key",
    ...
    def execute(self, args: argparse.Namespace) -> None:
        try:
            ...
            fmt.echo(f"Pushing dataset '{args.dataset}' to XingGraph Cloud...")

            async def run_push():
                try:
                    from xinggraph.api.v1.push.push import _resolve_client
                    from xinggraph.cli.user_resolution import resolve_cli_user
