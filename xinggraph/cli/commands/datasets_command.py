...
"""
    ...
    def _list(self, args: argparse.Namespace) -> None:
        async def run():
            ...
            ds = await xinggraph.datasets.list_datasets(user=user)
            if not ds:
    ...
    def _create(self, args: argparse.Namespace) -> None:
        async def run():
            from xinggraph.cli.user_resolution import resolve_cli_user
            from xinggraph.modules.data.methods import create_dataset, get_datasets_by_name
    ...
    def _data(self, args: argparse.Namespace) -> None:
        async def run():
            ...
            items = await xinggraph.datasets.list_data(dataset_id, user=user)
            if not items:
    ...
    def _status(self, args: argparse.Namespace) -> None:
        async def run():
            ...
            statuses = await xinggraph.datasets.get_status(ids, pipeline_names=args.pipelines)
            if not statuses:
    ...
    def _delete(self, args: argparse.Namespace) -> None:
        ...
        async def run():
            ...
            user = await resolve_cli_user(getattr(args, "user_id", None))
            await xinggraph.datasets.empty_dataset(dataset_id, user=user)
