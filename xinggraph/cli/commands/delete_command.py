...
from xinggraph.modules.data.methods.get_deletion_counts import get_deletion_counts


class DeleteCommand(SupportsCliCommand):
    command_string = "delete"
    help_string = "Delete data from xinggraph knowledge base"
    docs_url = DEFAULT_DOCS_URL
    description = """
The `xinggraph delete` command removes data from your knowledge base.
    ...
    def execute(self, args: argparse.Namespace) -> None:
        try:
            ...
            if getattr(args, "all", False):
                confirm_msg = "Delete ALL data from xinggraph?"
                operation = "all data"
            else:
            ...
            async def run_delete():
                try:
                    ...
                    if getattr(args, "all", False):
                        await xinggraph_datasets.delete_all(user=user)
                        ...
                        await xinggraph_datasets.empty_dataset(dataset_id=datasets[0].id, user=user)
