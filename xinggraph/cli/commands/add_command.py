...
class AddCommand(SupportsCliCommand):
    command_string = "add"
    help_string = "Add data to XingGraph for knowledge graph processing"
...
After adding data, use `xinggraph cognify` to process it into knowledge graphs.
    """
        ...
        try:
            ...
            async def run_add():
                try:
                    ...
                    # Pass all data items as a list to xinggraph.add if multiple items
                    if len(args.data) == 1:
                    ...
                    await xinggraph.add(data=data_to_add, dataset_name=args.dataset_name, user=user)
