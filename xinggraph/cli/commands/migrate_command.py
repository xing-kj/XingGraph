"""Alembic-style CLI commands for graph/vector database migrations.
    ...
    xinggraph upgrade [revision]     apply migrations up to a revision (default: head)
    xinggraph downgrade <revision>   revert migrations down to a revision ('base' = all)
    xinggraph history                show the migration chains (newest first)
    xinggraph current                show each database's stamped revision
...
def _bookkeeping_guard(error: Exception) -> None:
    ...
    if isinstance(error, (OperationalError, ProgrammingError)):
        fmt.error(
            ...
            "database — run `xinggraph-cli upgrade` first."
        )
...
Examples:
  xinggraph upgrade
  xinggraph upgrade head
  xinggraph upgrade namespace_entity_type_node_ids
"""
    ...
    def configure_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            "deployments; defaults to xinggraph's packaged alembic dir or $XINGGRAPH_ALEMBIC_PATH",
        )
    ...
    def execute(self, args: argparse.Namespace) -> None:
        ...
        async def run():
            from xinggraph.modules.migrations.startup import apply_all_migrations
...
Examples:
  xinggraph downgrade base
  xinggraph downgrade namespace_entity_type_node_ids
  xinggraph downgrade base --alembic base          # full rollback: data + schema
"""
    ...
    def configure_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            "xinggraph's packaged alembic dir or $XINGGRAPH_ALEMBIC_PATH",
        )
        parser.add_argument(
            "--dataset",
            action="append",
    ...
    def execute(self, args: argparse.Namespace) -> None:
        ...
        async def run():
            from xinggraph.modules.migrations.startup import revert_all_migrations
...
"""
    ...
    def execute(self, args: argparse.Namespace) -> None:
        from xinggraph.modules.migrations.migration import order_migrations
        from xinggraph.modules.migrations.registry import MIGRATIONS
        ...
        for index, migration in enumerate(reversed(ordered)):
            ...
            fmt.echo(
                ...
                f"xinggraph {migration.xinggraph_version}, {reversible}"
            )
...
"""
    ...
    def execute(self, args: argparse.Namespace) -> None:
        async def run():
            from xinggraph.context_global_variables import backend_access_control_enabled
            ...
            from xinggraph.modules.migrations.migration import head_revision
            ...
            if backend_access_control_enabled():
                ...
                if record is None:
                    fmt.note("No global_database_version row yet — run `xinggraph upgrade`.")
                    return
...
`xinggraph upgrade` re-runs the idempotent chain against it), or data you have
verified by hand. REVISION is 'head', 'base', or a migration slug.
Never touches data.

Examples:
  xinggraph stamp base --dataset 7df514cd-...   # re-arm migrations for one dataset
  xinggraph stamp head                          # mark everything migrated (dangerous)
"""
    ...
    def execute(self, args: argparse.Namespace) -> None:
        ...
        async def run():
            from xinggraph.modules.migrations.runner import stamp_revisions
