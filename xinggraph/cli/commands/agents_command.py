...
class AgentsCommand(SupportsCliCommand):
    ...
    description = """
Manage XingGraph agents and their connections.
...
"""
    ...
    def _create(self, args: argparse.Namespace) -> None:
        async def run():
            ...
            result = await xinggraph.agents.create(args.name, datasets=args.datasets, user=user)
    ...
    def _list(self, args: argparse.Namespace) -> None:
        async def run():
            ...
            user = await resolve_cli_user(getattr(args, "user_id", None), strict=True)
            agents = await xinggraph.agents.list(user=user)
            if not agents:
    ...
    def _get(self, args: argparse.Namespace) -> None:
        async def run():
            ...
            user = await resolve_cli_user(getattr(args, "user_id", None), strict=True)
            info = await xinggraph.agents.get(args.agent_id, user=user)
    ...
    def _delete(self, args: argparse.Namespace) -> None:
        ...
        async def run():
            ...
            user = await resolve_cli_user(getattr(args, "user_id", None), strict=True)
            await xinggraph.agents.delete(agent_id, user=user)
    ...
    def _register(self, args: argparse.Namespace) -> None:
        async def run():
            ...
            user = await resolve_cli_user(getattr(args, "user_id", None), strict=True)
            connection = await xinggraph.agents.register(
    ...
    def _unregister(self, args: argparse.Namespace) -> None:
        async def run():
            ...
            user = await resolve_cli_user(getattr(args, "user_id", None), strict=True)
            count = await xinggraph.agents.unregister(args.agent_session_name, user=user)
    ...
    def _connections(self, args: argparse.Namespace) -> None:
        async def run():
            ...
            response = await xinggraph.agents.list_connections(
                user=user,
                agent_id=agent_id,
