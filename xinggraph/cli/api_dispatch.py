"""Dispatch CLI commands via the XingGraph HTTP API instead of in-process.
...
from xinggraph.cli.api_client import XingGraphApiClient
...
def dispatch(args: argparse.Namespace) -> None:
    ...
    api_key = getattr(args, "api_key", None) or os.environ.get("XINGGRAPH_API_KEY")
    api_token = getattr(args, "api_token", None) or os.environ.get("XINGGRAPH_API_TOKEN")
    if api_key:
        headers["X-Api-Key"] = api_key
    elif api_token:
        headers["Authorization"] = f"Bearer {api_token}"

    with XingGraphApiClient(args.api_url, headers=headers) as client:
        # Health probe — fail fast with a clear message
        try:
            client.health()
        except Exception:
            raise RuntimeError(
                f"Cannot connect to XingGraph API at {args.api_url}.  "
                f"Is the server running?  Start it with:  "
                f"uvicorn xinggraph.api.client:app --port 8000"
            )
...
def _dispatch_add(client: XingGraphApiClient, args: argparse.Namespace) -> None:
...
def _dispatch_cognify(client: XingGraphApiClient, args: argparse.Namespace) -> None:
...
def _dispatch_search(client: XingGraphApiClient, args: argparse.Namespace) -> None:
...
def _dispatch_memify(client: XingGraphApiClient, args: argparse.Namespace) -> None:
...
def _dispatch_datasets(client: XingGraphApiClient, args: argparse.Namespace) -> None:
...
def _dispatch_delete(client: XingGraphApiClient, args: argparse.Namespace) -> None:
...
def _dispatch_remember(client: XingGraphApiClient, args: argparse.Namespace) -> None:
...
def _dispatch_recall(client: XingGraphApiClient, args: argparse.Namespace) -> None:
...
def _dispatch_improve(client: XingGraphApiClient, args: argparse.Namespace) -> None:
...
def _dispatch_forget(client: XingGraphApiClient, args: argparse.Namespace) -> None:
