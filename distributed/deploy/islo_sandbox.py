"""
XingGraph on Islo — deploy XingGraph API inside an Islo cloud sandbox.
...
Python SDK to create a sandbox, install XingGraph, start the API server, and print
...
Note:
    The sandbox name is fixed ("xinggraph-api"). Re-running the script while a
    previous deployment still exists fails with a name conflict — delete the
    old sandbox first:
        python -c "from islo import Islo; Islo().sandboxes.delete_sandbox('xinggraph-api')"
"""
...
# environment as PEP 668 "externally managed", so XingGraph is installed into a
# dedicated virtualenv (bootstrapped via apt) rather than the system interpreter.
VENV_PATH = "/opt/xinggraph-venv"
...
_INSTALL_SCRIPT = (
    ...
    f"{VENV_PATH}/bin/pip install 'xinggraph[api]'\n"
)
...
def wait_for_server_health(
    ...
    """Poll the XingGraph API /health endpoint inside the sandbox until it responds."""
    ...
    run_command(
        client,
        sandbox_name,
        ["bash", "-lc", "tail -n 30 /tmp/xinggraph-server.log"],
        timeout_secs=30,
        label="server-log",
    )
...
def deploy_xinggraph():
    """Create an Islo sandbox, start the XingGraph API server, and print a share URL."""
    ...
    print("Creating Islo sandbox for XingGraph...")
    try:
    ...
    if install.status != "completed" or install.exit_code not in (0, None):
        raise RuntimeError(
            f"Failed to install xinggraph (status '{install.status}', exit {install.exit_code}): "
            f"{install.stderr}"
        )
    print()

    print("=== Starting XingGraph API server ===")
    server_start = run_command(
        ...
        [
            "bash",
            "-lc",
            f"nohup {VENV_PYTHON} -m uvicorn xinggraph.api.client:app --host 0.0.0.0 --port {API_PORT} "
            "> /tmp/xinggraph-server.log 2>&1 & echo started",
        ],
        timeout_secs=30,
        label="server",
    )
    if server_start.status != "completed" or server_start.exit_code not in (0, None):
        raise RuntimeError(
            f"Failed to start xinggraph API server (status '{server_start.status}', "
    ...
    if not wait_for_server_health(client, SANDBOX_NAME):
        raise RuntimeError(
            "XingGraph API server did not become healthy; see the server log printed above"
        )

    share = client.shares.create_share(SANDBOX_NAME, port=API_PORT, ttl_seconds=86400)

    print("\nXingGraph is running!")
