# XingGraph Deployment

1-click deployment configurations for hosting XingGraph as a service.
...
Simplest path to a hosted XingGraph API. Native Postgres add-on with pgvector support.
...
- XingGraph API service (from Dockerfile)
...
The script handles app creation, secrets, volume provisioning, and deployment. Your API will be at `https://xinggraph.fly.dev`.
...
Daytona provides secure, isolated cloud sandboxes. XingGraph runs inside a sandbox with persistent storage.
...
daytona create
# Inside the sandbox:
pip install 'xinggraph[api]'
python -m uvicorn xinggraph.api.client:app --host 0.0.0.0 --port 8000
```
...
Islo provides isolated cloud sandbox VMs for autonomous agents, built by the Incredibuild team. XingGraph runs inside a sandbox and is exposed through a temporary public share URL. Docs: https://docs.islo.dev

```bash
curl -fsSL https://islo.dev/install.sh | bash
islo login
islo api-key create xinggraph-deploy --expires 90 --show
...
The CLI is used only to create the access key. The deployment itself uses the official Python SDK to create the sandbox, install `xinggraph[api]`, start the API server, verify `/health`, and create a 24-hour share URL. Stop or delete the sandbox via the SDK:

```python
from islo import Islo

client = Islo()  # reads ISLO_API_KEY from the environment
client.sandboxes.stop_sandbox("xinggraph-api")
client.sandboxes.delete_sandbox("xinggraph-api")
```

The sandbox name is fixed (`xinggraph-api`), so re-running the script while a previous deployment still exists fails with a name conflict — delete the old sandbox first (see above), then re-run.
...
### GitHub Codespaces
```bash
gh codespace create --repo xing-kj/xinggraph
```
...
5. **Trace**: Enable OpenTelemetry tracing with `XINGGRAPH_TRACING_ENABLED=true` and an OTLP endpoint. Install with `pip install xinggraph[tracing]`.
