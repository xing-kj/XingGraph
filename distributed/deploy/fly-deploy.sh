#!/usr/bin/env bash
# XingGraph 1-click deployment to Fly.io
#
...
APP_NAME="${FLY_APP_NAME:-xinggraph}"
REGION="${FLY_REGION:-iad}"
VOLUME_SIZE="${FLY_VOLUME_SIZE:-10}"

echo "Deploying XingGraph to Fly.io..."
echo "  App:    $APP_NAME"
echo "  Region: $REGION"
...
if ! fly volumes list -a "$APP_NAME" --json | grep -q "xinggraph_data"; then
    echo "Creating persistent volume..."
    fly volumes create xinggraph_data \
        --size "$VOLUME_SIZE" \
        --region "$REGION" \
        -a "$APP_NAME" \
        --yes
...
echo ""
echo "XingGraph deployed successfully!"
