"""
XingGraph on Daytona — deploy XingGraph API inside a Daytona sandbox.

Daytona provides secure, isolated cloud sandboxes with persistent volumes.
This script creates a sandbox, installs XingGraph, and starts the API server,
...
def deploy_xinggraph():
    """Create a Daytona sandbox and start the XingGraph API server."""
    ...
    print("Creating Daytona sandbox for XingGraph...")
    sandbox = daytona.create(
        CreateSandboxFromImageParams(
            ...
            labels={"app": "xinggraph", "service": "api"},
        ),
    )
    print(f"Sandbox created: {sandbox.id}\n")

    # Create a session for streamed output
    session_id = "xinggraph-deploy"
    sandbox.process.create_session(session_id)

    # Step 1: Install XingGraph (streamed)
    print("=== Installing XingGraph ===")
    _run_streamed(sandbox, session_id, "pip install 'xinggraph[api]'", label="install")
    print("\n")
    ...
    sandbox.process.exec(
        "nohup python -m uvicorn xinggraph.api.client:app "
        "--host 0.0.0.0 --port 8000 > /tmp/xinggraph-server.log 2>&1 &",
        timeout=10,
    )
    ...
    for i in range(30):
        ...
        try:
            log = sandbox.process.exec("tail -30 /tmp/xinggraph-server.log", timeout=5)
            print(f"\nServer log:\n{log.result}")
        except Exception:
            pass
    ...
    print(
        ...
        f"s=d.get('{sandbox.id}'); print(s.process.exec('cat /tmp/xinggraph-server.log').result)\""
    )
    print("\nTo stop:")
