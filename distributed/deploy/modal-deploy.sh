#!/usr/bin/env bash
# XingGraph 1-click deployment to Modal
#
...
echo "Deploying XingGraph to Modal..."

# Create or update the secret group
echo "Setting up secrets..."
modal secret create xinggraph-secrets \
    LLM_API_KEY="$LLM_API_KEY" \
    LLM_MODEL="${LLM_MODEL:-openai/gpt-4o-mini}" \
    LLM_PROVIDER="${LLM_PROVIDER:-openai}" \
    DB_PROVIDER="${DB_PROVIDER:-sqlite}" \
    2>/dev/null || \
modal secret update xinggraph-secrets \
    LLM_API_KEY="$LLM_API_KEY" \
    LLM_MODEL="${LLM_MODEL:-openai/gpt-4o-mini}" \
    LLM_PROVIDER="${LLM_PROVIDER:-openai}" \
    DB_PROVIDER="${DB_PROVIDER:-sqlite}"
...
echo ""
echo "XingGraph deployed successfully to Modal!"
