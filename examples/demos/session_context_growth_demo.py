...
from xinggraph.infrastructure.session.session_manager import SessionManager, SessionTurnPreparation
...
async def run_demo() -> list[dict]:
    ...
    try:
        turns = demo_turns()
        with (
            patch("xinggraph.infrastructure.session.session_manager.CacheConfig", demo_cache_config),
            patch(
                "xinggraph.infrastructure.session.session_manager.analyze_turn_for_session_context",
                new_callable=AsyncMock,
                side_effect=[turn.simulated_analysis for turn in turns],
            ),
            patch(
                "xinggraph.infrastructure.session.session_manager.uuid.uuid4",
                side_effect=["qa-1", "feedback-1", "qa-2", "feedback-2", "qa-3"],
            ),
            patch(
                "xinggraph.infrastructure.session.session_context_builder.uuid4",
