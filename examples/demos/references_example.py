...
Demonstrates the `include_references` flag added to `xinggraph.search(...)`:
...
_DATA_DIR = tempfile.mkdtemp(prefix="xinggraph_references_example_")
...
import xinggraph  # noqa: E402
from xinggraph.modules.search.types import SearchType  # noqa: E402

# Force the default local stack via the config API. xinggraph loads .env with
...
xinggraph.config.set_graph_database_provider("kuzu")  # Ladybug (local, embedded)
xinggraph.config.set_vector_db_provider("lancedb")
xinggraph.config.data_root_directory(str(Path(_DATA_DIR) / "data"))
xinggraph.config.system_root_directory(str(Path(_DATA_DIR) / "system"))
...
async def main() -> None:
    ...
    banner("ADD + COGNIFY")
    await xinggraph.add(str(doc_path), dataset_name="references_demo")
    await xinggraph.cognify(datasets=["references_demo"])
