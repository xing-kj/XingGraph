...
# Configure cache/session behavior before importing xinggraph internals.
...
DEMO_SYSTEM_DIR = BASE_DIR / ".xinggraph_system"
...
async def _latest_qa_for_session(session_id: str):
    ...
    entries = await xinggraph.session.get_session(session_id=session_id, last_n=1, user=user)
    if not entries:
        return None
    return entries[-1]
...
async def get_session_content(
    ...
    user = await get_default_user()
    entries = await xinggraph.session.get_session(
...
async def init_demo():
    ...
    try:
        ...
        xinggraph.config.data_root_directory(str(DEMO_DATA_DIR))
        xinggraph.config.system_root_directory(str(DEMO_SYSTEM_DIR))
