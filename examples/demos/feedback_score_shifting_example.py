...
if not cache_config.caching or cache_config.cache_backend != "fs":
    raise XingGraphConfigurationError(
...
async def main():
    ...
    await xinggraph.remember([TEXT_1, TEXT_2], self_improvement=False)
    ...
    qa_cars = (await xinggraph.session.get_session(session_id=session_id, user=user, last_n=1))[0]
    ...
    qa_companies = (await xinggraph.session.get_session(session_id=session_id, user=user, last_n=1))[0]
    await xinggraph.session.add_feedback(
        session_id=session_id,
