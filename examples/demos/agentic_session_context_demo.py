...
import xinggraph
from xinggraph.memory import TraceEntry
...
async def run_distillation_act(user) -> dict:
    ...
    from xinggraph.infrastructure.session.agent_context_extraction import extract_pending_agent_context
...
async def run_recall_act(user) -> dict:
    ...
    agent_ctx = await xinggraph.recall(
    ...
    raw_traces = await xinggraph.recall(
        "dotenv",
        scope=["trace"],
        session_id=SESSION_ID,
        only_context=True,
...
async def setup_demo_data():
    ...
    await xinggraph.add(["Agentic session-context demo dataset."], dataset_name=DATASET_NAME)
