...
2. After the session, ``xinggraph.session.distill_session`` gates the learned guidance,
...
async def setup_demo_data():
    ...
    await xinggraph.add(DOCUMENTS, dataset_name=DATASET_NAME)
...
async def distill_and_verify(user):
    ...
    result = await xinggraph.session.distill_session(SESSION_ID, dataset=DATASET_NAME, user=user)
