"""Step-by-step trace of the xinggraph 1.0 memory loop, driven by remember() + recall().
...
move through the system using only the two top-level verbs, `xinggraph.remember` and `xinggraph.recall`.

It demonstrates that session DISTILLATION is now covered by remember(): distillation is wired
into improve() (see xinggraph/api/v1/improve/improve.py, stage 2c), and remember(session,
...
# Load the repo .env BEFORE importing xinggraph, with override=True so the real keys in
...
# Pin a gpt-4o-family model for the OpenAI path. xinggraph's current default (openai/gpt-5-mini)
# forces instructor's json_schema_mode, which calls response_model.model_json_schema() and
# therefore breaks on xinggraph's str-returning paths (preflight + recall answer generation):
...
# Disable xinggraph's 30s LLM preflight via env var. The preflight calls the model with
...
import xinggraph  # noqa: E402  (env must be configured before importing xinggraph)
from xinggraph import SearchType  # noqa: E402
from xinggraph.infrastructure.session.get_session_manager import get_session_manager  # noqa: E402
from xinggraph.modules.users.methods import get_default_user  # noqa: E402
...
async def seed_permanent_memory() -> None:
    ...
    result = await xinggraph.remember(DOCUMENTS, dataset_name=DATASET_NAME)
    step(
...
async def auto_distill_checkpoint(user, turn_no: int) -> None:
    ...
    await xinggraph.improve(dataset=DATASET_NAME, session_ids=[SESSION_ID], user=user)
...
async def visualize(user) -> None:
    ...
    html = await xinggraph.visualize_graph(
