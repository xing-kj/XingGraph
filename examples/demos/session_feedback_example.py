...
# Set os.environ before importing XingGraph: XingGraph reads env-backed settings at import time, so values
# assigned later may not override defaults or `.env`. See https://docs.xinggraph.ai/setup-configuration/overview#using-os-environ
...
async def main():
    ...
    texts = [
        "XingGraph builds knowledge graphs from text and provides session-based feedback APIs. "
    ...
    await xinggraph.remember(texts, self_improvement=False)
    ...
    all_qas = await xinggraph.session.get_session(session_id=session_id, user=user)
    print(f"  get_session({session_id!r}) → {len(all_qas)} Q&A(s)\n")

    # Show only the last 2 interactions
    recent = await xinggraph.session.get_session(session_id=session_id, user=user, last_n=2)
    ...
    results_default = await xinggraph.recall(
        query_text="How are sessions related to XingGraph?",
        query_type=SearchType.GRAPH_COMPLETION,
        user=user,
    )
    print("  Q: How are sessions related to XingGraph?")
    print(f"  A: {results_default[0] if results_default else '(no answer)'}\n")

    default_qas = await xinggraph.session.get_session(session_id="default_session", user=user)
    print(f"  get_session('default_session') → {len(default_qas)} Q&A(s)")
    latest_default = default_qas[-1]
    await xinggraph.session.add_feedback(
    ...
    after_add = await xinggraph.session.get_session(session_id=session_id, user=user)
    ...
    deleted = await xinggraph.session.delete_feedback(
        session_id=session_id, qa_id=latest.qa_id, user=user
    )
    print(f"  delete_feedback(...) → {deleted}")

    after_del = await xinggraph.session.get_session(session_id=session_id, user=user)
