...
Run from the `xinggraph/` directory:
...
def _configure_demo_storage() -> None:
    ...
    xinggraph.config.system_root_directory(str(repo_root / ".xinggraph_system/hybrid_context_demo"))
    xinggraph.config.data_root_directory(str(repo_root / ".data_storage/hybrid_context_demo"))
...
async def _ingest_demo_data() -> None:
    ...
    await xinggraph.add(DOCUMENTS, dataset_name=DATASET)

    print("Cognifying demo dataset...")
    await xinggraph.cognify(DATASET)
