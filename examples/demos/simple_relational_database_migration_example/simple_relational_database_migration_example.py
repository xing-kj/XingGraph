...
os.environ.setdefault("MIGRATION_DB_NAME", "xinggraph_migration")
os.environ.setdefault("MIGRATION_DB_USERNAME", "xinggraph")
os.environ.setdefault("MIGRATION_DB_PASSWORD", "xinggraph")
...
from xinggraph.run_migrations import (
    run_migrations,
...
async def main(ontology_path: str = None):
    ...
    # Ensure the local XingGraph DB exists.
    await create_relational_db_and_tables()
    # Update a reused local XingGraph DB so its tables match the current models.
    ...
    if ontology_path:
        ...
        await xinggraph.remember(db_texts, dataset_name=dataset_name, self_improvement=False)
        graph_visualization_path = os.path.join(
    ...
    await xinggraph.visualize_graph(graph_visualization_path)
