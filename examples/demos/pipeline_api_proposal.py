...
from xinggraph.infrastructure.engine.models import DataPoint
from xinggraph.infrastructure.engine.models.FieldAnnotations import Embeddable, Dedup, LLMContext
...
from xinggraph.modules.pipelines.tasks.task import Task, task  # noqa: E402
from xinggraph.pipelines.types import Drop  # noqa: E402
...
async def run_simple_pipeline(datasets, user):
    from xinggraph.modules.pipelines.operations.run_tasks import run_tasks
...
#
#   DELETE xinggraph/pipelines/flow.py      → semaphore goes into run_tasks.py
#   DELETE xinggraph/pipelines/step.py      → @task decorator already exists in task.py
#   DELETE xinggraph/pipelines/builder.py   → Pipeline builder adds complexity without value
#   DELETE xinggraph/pipelines/context.py   → xinggraph_pipeline() duplicates existing setup
#   DELETE xinggraph/pipelines/types.py     → Drop already in task.py, Pipe[T]/Ctx[T] unused
#
# KEEP:
#   xinggraph/infrastructure/engine/models/FieldAnnotations.py  → genuinely useful
#   xinggraph/pipelines/__init__.py  → legacy compat re-exports (simplified)
...
async def full_example():
    ...
    from xinggraph.modules.data.methods import load_or_create_datasets
    from xinggraph.modules.users.methods import get_default_user
    from xinggraph.modules.pipelines.operations.run_tasks import run_tasks
    from xinggraph.modules.pipelines.tasks.task import Task, task
    from xinggraph.pipelines.types import Drop
