...
async def main():
    ...
    await xinggraph.remember(PERMANENT_TEXT, dataset_name=DATASET)
    ...
    await xinggraph.remember(SESSION_TEXT_1, session_id=SESSION)
    print("  Stored in session cache.")

    print("\n--- Step 4: remember(session_id) -- session memory (entry 2) ---")
    await xinggraph.remember(SESSION_TEXT_2, session_id=SESSION)
    print("  Stored in session cache.")
    ...
    result = await xinggraph.forget(everything=True)
    print(f"  {result}")
