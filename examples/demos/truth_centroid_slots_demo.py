...
This demo uses real xinggraph ingestion, cognification, vector search, graph state,
...
os.environ.setdefault("XINGGRAPH_CLI_MODE", "true")
os.environ.setdefault("XINGGRAPH_LOG_FILE", "false")
...
from xinggraph.infrastructure.databases.vector.embeddings.get_embedding_engine import (
...
from xinggraph.modules.retrieval.hybrid.results import payload
from xinggraph.modules.retrieval.hybrid_retriever import HybridRetriever
from xinggraph.modules.truth_subspace import align
from xinggraph.modules.truth_subspace.build import build_truth_subspace
from xinggraph.modules.truth_subspace.centroids import load_centroids
...
def configure_demo_storage() -> None:
    ...
    xinggraph.config.set_graph_database_provider("kuzu")
    xinggraph.config.set_vector_db_provider("lancedb")
    xinggraph.config.system_root_directory(str(repo_root / ".xinggraph_system/truth_slots_demo"))
    xinggraph.config.data_root_directory(str(repo_root / ".data_storage/truth_slots_demo"))
...
async def main() -> None:
    ...
    await xinggraph.add(DOCUMENTS, dataset_name=DATASET, node_set=CORPUS_NODE_SET)
    ...
    for title, learnings in LEARNING_BATCHES:
        ...
        await xinggraph.add(learnings, dataset_name=DATASET, node_set=LEARNINGS_NODE_SET)
        await xinggraph.cognify(datasets=[DATASET])
