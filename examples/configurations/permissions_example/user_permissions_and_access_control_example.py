...
async def main():
    ...
    # Reference the .env.tempalte file for available option and how to change LLM provider: https://github.com/xing-kj/xinggraph/blob/main/.env.template
    ...
    ai_remember_result = await xinggraph.remember(
    ...
    quantum_remember_result = await xinggraph.remember(
        [text],
    ...
    print("User 2 is creating XingGraphLab tenant/organization")
    tenant_id = await create_tenant("XingGraphLab", user_2.id)

    print("User 2 is selecting XingGraphLab tenant/organization as active tenant")
    ...
    print("\nOperation started as user_2 to add user_3 to XingGraphLab tenant/organization")
    ...
    print("\nOperation as user_3 to select XingGraphLab tenant/organization as active tenant")
    await select_tenant(user_id=user_3.id, tenant_id=tenant_id)

    print(
        "\nOperation started as user_2, with XingGraphLab as its active tenant, to give read permission to Researcher role for the dataset QUANTUM owned by user_2"
    )
    # Even though the dataset owner is user_2, the dataset doesn't belong to the tenant/organization XingGraphLab.
    # So we can't assign permissions to it when we're acting in the XingGraphLab tenant.
    try:
        await authorized_give_permission_on_datasets(
            role_id,
            [quantum_dataset_id],
        ...
        print(
            "User 2 could not give permission to the role as the QUANTUM dataset is not part of the XingGraphLab tenant"
        )

    print(
        "We will now create a new QUANTUM dataset with the QUANTUM_XINGGRAPH_LAB name in the XingGraphLab tenant so that permissions can be assigned to the Researcher role inside the tenant/organization"
    )
    # We can re-create the QUANTUM dataset in the XingGraphLab tenant. The old QUANTUM dataset is still owned by user_2 personally
    ...
    print(
        "\nOperation started as user_2, with XingGraphLab as its active tenant, to give read permission to Researcher role for the dataset QUANTUM owned by the XingGraphLab tenant"
    )
    await authorized_give_permission_on_datasets(
        role_id,
    ...
    print("\nRecall result as user_3 on the QUANTUM dataset owned by the XingGraphLab organization:")
    recall_results = await xinggraph.recall(
        query_type=SearchType.GRAPH_COMPLETION,
        query_text="What is in the document?",
        user=user_3,
