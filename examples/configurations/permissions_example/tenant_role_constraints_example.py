...
async def tenant_and_role_constraints_example():
    ...
    print(
        "\nOperation started as user_1, with XingGraphLab as its active tenant, to give read permission to Researcher role for the dataset QUANTUM owned by user_1"
    )
    # Even though the dataset owner is user_1, the dataset doesn't belong to the tenant/organization XingGraphLab.
    ...
    try:
        ...
        print(
            "User 1 could not give permission to the role as the QUANTUM dataset is not part of the XingGraphLab tenant"
        )

    # We can re-create the QUANTUM dataset in the XingGraphLab tenant. The old QUANTUM dataset is still owned by user_1 personally
