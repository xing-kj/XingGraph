...
# In case environment variables are not set use the example database from the XingGraph repo.
MIGRATION_DB_PROVIDER = os.environ.get("MIGRATION_DB_PROVIDER", "sqlite")
MIGRATION_DB_PATH = os.environ.get(
    "MIGRATION_DB_PATH",
    os.path.join(Path(__file__).resolve().parent.parent.parent, "xinggraph/tests/test_data"),
)
...
from xinggraph.infrastructure.databases.relational import (
    create_db_and_tables as create_relational_db_and_tables,
)
from xinggraph.infrastructure.databases.relational import (
    get_migration_relational_engine,
)
from xinggraph.infrastructure.databases.relational.config import get_migration_config
from xinggraph.infrastructure.databases.vector.pgvector import (
...
# 3. Fill all relevant MIGRATION_DB information for the database you want to migrate to graph / XingGraph

# NOTE: If you don't have a DB you want to migrate you can try it out with our
#       test database at the following location:
#           MIGRATION_DB_PATH="/{path_to_your_local_xinggraph}/xinggraph/tests/test_data"
#           MIGRATION_DB_NAME="migration_database.sqlite"
#           MIGRATION_DB_PROVIDER="sqlite"


async def main():
    # Clean all data stored in XingGraph
    await xinggraph.forget(everything=True)

    # Needed to create appropriate database tables only on the XingGraph side
    ...
    from xinggraph.tasks.ingestion import migrate_relational_database
