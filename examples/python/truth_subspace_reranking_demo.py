...
not touch your other xinggraph data. Re-running is safe (adds are content-addressed,
...
# Run against THIS checkout of xinggraph even if a different copy is pip-installed,
...
from xinggraph.modules.retrieval.hybrid.results import payload, result_id
...
async def main():
    ...
    await xinggraph.add(CORPUS, dataset_name=DATASET, node_set=CORPUS_NODE_SET)
    ...
    await xinggraph.add(LESSONS, dataset_name=DATASET, node_set=LESSONS_NODE_SET)
    await xinggraph.cognify(datasets=[DATASET])
