...
- Provides `disambiguate_entities_pipeline(...)`, a pipeline equivalent to `xinggraph.cognify()` but
  with the graph extraction task swapped for the POC entity-disambiguation extractor.

Parameters (differences vs `xinggraph.cognify()`):
...
- End-to-end runnable example that:
  - Prunes existing XingGraph data.
  - Reads a small example file line-by-line from `disambiguation/data/`.
  - Adds each line as a separate datapoint.
  - Runs either the POC pipeline (`use_poc=True`) or the standard `xinggraph.cognify()`.
