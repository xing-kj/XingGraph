...
// XingGraph's graph HTML loads d3 via <script src="https://d3js.org/d3.v7.min.js">.
...
//
// Brittleness flag: the regex below hard-codes xinggraph's exact CDN script tag
// from xinggraph/modules/visualization/xinggraph_network_visualization.py. If xinggraph
...
// d3 inside xinggraph's template upstream so consumers don't need this patch.
//
...
// ── XingGraph mark (5 pillars) ─────────────────────────────────────

function XingGraphMark({ size = 16 }: { size?: number }) {
  ...
  return (
    <div className="graph-wrap">
      ...
      <div className="gstage">
        {srcDoc ? (
          <iframe
            title="XingGraph Knowledge Graph"
            srcDoc={srcDoc}
...
function WorkspaceApp() {
  ...
  const { app, error } = useApp({
    appInfo: { name: "XingGraph Workspace", version: "0.2.0" },
    capabilities: {},
    onAppCreated: (app) => {
