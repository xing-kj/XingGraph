...
MCP_ROOT = Path(__file__).resolve().parents[1]  # xinggraph-mcp/
if str(MCP_ROOT) not in sys.path:
    sys.path.insert(0, str(MCP_ROOT))

XingGraphClient = importlib.import_module("src.xinggraph_client").XingGraphClient
...
@pytest.mark.asyncio
async def test_xinggraph_client_api_add_uses_content_addressed_filename():
    ...
    client = XingGraphClient(api_url="http://xinggraph.local")
...
@pytest.mark.asyncio
async def test_xinggraph_client_api_remember_sends_session_id():
    requests: list[httpx.Request] = []
    ...
    client = XingGraphClient(api_url="http://xinggraph.local", api_token="token")
    await client.client.aclose()
    client.client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
...
@pytest.mark.asyncio
async def test_xinggraph_client_api_recall_sends_session_id_and_null_search_type():
    requests: list[httpx.Request] = []
...
@pytest.mark.asyncio
async def test_xinggraph_client_api_delete_uses_mode_aware_endpoint():
    requests: list[httpx.Request] = []
...
async def test_cognify_tool_batches_add_calls(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "xinggraph_client", fake_client)
...
async def test_document_retrieval_tools_format_json(monkeypatch):
    ...
    monkeypatch.setattr(server, "xinggraph_client", FakeClient())
