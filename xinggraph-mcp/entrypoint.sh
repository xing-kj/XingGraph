...
if [ -n "$EXTRAS" ]; then
    ...
    # Get the xinggraph version that's currently installed
    XINGGRAPH_VERSION=$(uv pip show xinggraph | grep "Version:" | awk '{print $2}')
    echo "Current xinggraph version: $XINGGRAPH_VERSION"

    # Build the extras list for xinggraph
    ...
    echo "Installing xinggraph with extras: $ALL_EXTRAS"
    echo "Running: uv pip install 'xinggraph[$ALL_EXTRAS]==$XINGGRAPH_VERSION'"
    uv pip install "xinggraph[$ALL_EXTRAS]==$XINGGRAPH_VERSION"
...
echo "Starting XingGraph MCP Server with transport mode: $TRANSPORT_MODE"
...
if [ -n "$API_URL" ]; then
    ...
    echo "Direct mode: Using local xinggraph instance"
fi

echo "calling xinggraph-mcp" "${ARGS[@]}"
...
if [ "$DEBUG" = "true" ] && { [ "$ENV" = "dev" ] || [ "$ENV" = "local" ]; }; then
    ...
    exec python -m debugpy --wait-for-client --listen 0.0.0.0:"$DEBUG_PORT" "$(command -v xinggraph-mcp)" "${ARGS[@]}"
else
    exec xinggraph-mcp "${ARGS[@]}"
fi
