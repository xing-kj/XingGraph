...
from xinggraph.modules.data.methods.get_datasets_by_name import get_datasets_by_name
from xinggraph.modules.data.methods.get_last_added_data import get_last_added_data
from xinggraph.modules.users.methods import get_default_user
from xinggraph.shared.logging_utils import get_logger, setup_logging, get_log_file_location
...
from xinggraph.modules.storage.utils import JSONEncoder
...
try:
    from .xinggraph_client import XingGraphClient
except ImportError:
    from xinggraph_client import XingGraphClient
...
try:
    from xinggraph.tasks.codingagents.coding_rule_associations import (
...
mcp = FastMCP("XingGraph")

logger = get_logger()

xinggraph_client: Optional[XingGraphClient] = None
...
async def cognify(
    ...
    This is the core processing step in XingGraph that converts raw text and documents
    ...
    graph_model_file : str, optional
        ...
        Default is None, which uses XingGraph's built-in KnowledgeGraph model.
    ...
    Next Steps:
        ...
        insights = await xinggraph.search(
            "What are the main themes?",
            query_type=SearchType.GRAPH_COMPLETION
        )

        # 2. Get entity relationships and connections
        relationships = await xinggraph.search(
            "connections between concepts",
            query_type=SearchType.GRAPH_COMPLETION
        )

        # 3. Find relevant document chunks
        chunks = await xinggraph.search(
            "specific topic",
    ...
    async def cognify_task(
        ...
        with redirect_stdout(sys.stderr):
            ...
            for data_item in data_items:
                await xinggraph_client.add(data_item, dataset_name=dataset_name)

            try:
                await xinggraph_client.cognify(
...
async def save_interaction(data: str) -> list:
    ...
    async def save_user_agent_interaction(data: str) -> None:
        ...
        with redirect_stdout(sys.stderr):
            ...
            await xinggraph_client.add(data, node_set=["user_agent_interaction"])
            ...
            try:
                ...
                if not xinggraph_client.use_api:
...
async def search(
    ...
    This is the final step in the XingGraph workflow that retrieves information from the
    ...
    Search Prerequisites:
        ...
        - **Data Added**: Must have data previously added via `xinggraph.add()`
        - **Knowledge Graph Built**: Must have processed data via `xinggraph.cognify()`
    ...
    search_type : str
        ...
        - "GRAPH_COMPLETION": Returns an LLM response based on the search query and XingGraph's memory
    ...
    - The function handles the conversion between XingGraph's internal result format and MCP's output format
    ...
    async def search_task(
        ...
        #       going to stdout ( like the print function ) to stderr.
        with redirect_stdout(sys.stderr):
            search_results = await xinggraph_client.search(
            ...
            if not xinggraph_client.use_api and search_type == "INSIGHTS":
...
async def get_document(
    ...
    with redirect_stdout(sys.stderr):
        try:
            result = await xinggraph_client.get_document(
...
async def get_chunk_neighbors(
    ...
    with redirect_stdout(sys.stderr):
        try:
            result = await xinggraph_client.get_chunk_neighbors(
                chunk_id=chunk_id,
    ...
    with redirect_stdout(sys.stderr):
        try:
            ...
            if dataset_id:
                ...
                from xinggraph.modules.users.methods import get_default_user
                from xinggraph.modules.data.methods import get_dataset, get_dataset_data
                ...
                datasets = await xinggraph_client.list_datasets()
...
async def delete_dataset(dataset_name: str) -> list:
    ...
    with redirect_stdout(sys.stderr):
        try:
            ...
            from xinggraph.modules.users.methods import get_default_user
            from xinggraph.modules.data.methods import delete_dataset as _delete_dataset
            from xinggraph.modules.data.methods import get_datasets
...
async def delete(data_id: str, dataset_id: str, mode: str = "soft") -> list:
    """
    Delete specific data from a dataset in the XingGraph knowledge graph.
    ...
    with redirect_stdout(sys.stderr):
        try:
            ...
            # Call the xinggraph delete function via client
            result = await xinggraph_client.delete(
...
async def prune():
    """
    Reset the XingGraph knowledge graph by removing all stored information.

    This function performs a complete reset of both the data layer and system layer
    of the XingGraph knowledge graph, removing all nodes, edges, and associated metadata.
    ...
    with redirect_stdout(sys.stderr):
        try:
            await xinggraph_client.prune_data()
            await xinggraph_client.prune_system(metadata=True)
...
async def remember(
    ...
    with redirect_stdout(sys.stderr):
        try:
            result = await xinggraph_client.remember(
                data=data,
                dataset_name=dataset_name,
...
async def recall(
    ...
    with redirect_stdout(sys.stderr):
        try:
            ...
            results = await xinggraph_client.recall(
                query_text=query,
...
async def forget(
    ...
    with redirect_stdout(sys.stderr):
        try:
            ...
            result = await xinggraph_client.forget(dataset=dataset, everything=everything)
...
async def improve(
    ...
    with redirect_stdout(sys.stderr):
        try:
            ...
            result = await xinggraph_client.improve(
                dataset_name=dataset_name,
...
async def cognify_status(
    ...
    with redirect_stdout(sys.stderr):
        try:
            if xinggraph_client.use_api:
                # API mode: resolve the dataset id over HTTP (no local xinggraph
                ...
                from xinggraph.modules.data.methods.get_unique_dataset_id import get_unique_dataset_id
                from xinggraph.modules.users.methods import get_default_user
            ...
            if len(requested_pipelines) == 1:
                status = await xinggraph_client.get_pipeline_status(
                    [dataset_id], requested_pipelines[0]
                )
            else:
                status: dict[str, dict] = {str(dataset_id): {}}
                for pipeline_name in requested_pipelines:
                    pipeline_status = await xinggraph_client.get_pipeline_status(
...
_VISUALIZE_APP_URI = "ui://xinggraph-visualize/graph.html"


@mcp.resource(
    _VISUALIZE_APP_URI,
    name="XingGraph Graph Visualization UI",
    description="Interactive MCP App UI that renders a XingGraph knowledge graph.",
...
def _visualize_graph_ui_resource() -> str:
    ...
    # dev (running `python src/server.py` directly), it's xinggraph-mcp/src/
    ...
    if not bundle.is_file():
        raise FileNotFoundError(
            ...
            "Build it with: cd xinggraph-mcp/apps-src && npm install && npm run build"
        )
    return bundle.read_text(encoding="utf-8")


# CSS overrides appended to xinggraph's graph HTML so it fits the MCP App
...
#
# Note: d3 is loaded from a CDN by xinggraph's HTML, which the MCP App iframe
...
@mcp.tool(
    ...
    description=(
        "Open the XingGraph workspace UI and render the current knowledge graph. "
...
async def visualize_graph_ui(dataset_name: str = None) -> types.CallToolResult:
    """Render the XingGraph graph for a specific dataset.
    ...
    if explicit_dataset and xinggraph_client.use_api:
            ...
            content=[
                types.TextContent(
                    ...
                    text=(
                        ...
                        "Drop the dataset_name argument or run xinggraph-mcp without --api-url."
                    ),
                )
            ],
        )
    ...
    with redirect_stdout(sys.stderr):
        html: str | None = None
        if dataset_name and not xinggraph_client.use_api:
            from xinggraph.api.v1.visualize.visualize import visualize_multi_user_graph
    ...
    return types.CallToolResult(
        content=[types.TextContent(type="text", text="XingGraph knowledge graph rendered.")],
...
@mcp.tool(
    name="upload_file_ui",
    description=(
        "Open the XingGraph workspace UI so the user can upload files to memory. "
...
async def upload_file_ui() -> types.CallToolResult:
    return types.CallToolResult(
        content=[types.TextContent(type="text", text="XingGraph workspace opened.")],
    )


@mcp.tool(
    name="open_xinggraph_workspace",
    description=(
        "Open the XingGraph workspace UI. Use for generic intents like "
        "'run the xinggraph UI', 'show the xinggraph app', 'open xinggraph'. "
        "The UI provides dataset management, file upload, text ingestion, "
        "search, and graph visualization."
    ),
    meta={"ui": {"resourceUri": _VISUALIZE_APP_URI}},
)
@log_usage(function_name="MCP open_xinggraph_workspace", log_type="mcp_tool")
async def open_xinggraph_workspace() -> types.CallToolResult:
    return types.CallToolResult(
        content=[types.TextContent(type="text", text="XingGraph workspace opened.")],
    )
...
@mcp.tool(
    name="cognify_file",
    description=(
        "Ingest an uploaded file into XingGraph memory. Accepts the file as base64. "
...
async def cognify_file(
    ...
    # name so xinggraph's Data.name comes out as e.g. "alice_notes" rather than
    ...
    with redirect_stdout(sys.stderr):
        tmp_dir = tempfile.mkdtemp(prefix="xinggraph_upload_")
        tmp_path = os.path.join(tmp_dir, safe_name)
        try:
            with open(tmp_path, "wb") as f:
                f.write(data)
            await xinggraph_client.add(tmp_path, dataset_name=dataset_name)
        finally:
            try:
                os.unlink(tmp_path)
            except OSError:
    ...
    async def _cognify_bg():
        with redirect_stdout(sys.stderr):
            try:
                await xinggraph_client.cognify(datasets=[dataset_name])
...
@mcp.tool(
    ...
    description=(
        "List datasets as structured JSON for the XingGraph workspace UI. "
...
async def list_datasets_json() -> types.CallToolResult:
    with redirect_stdout(sys.stderr):
        raw = await xinggraph_client.list_datasets()
...
@mcp.tool(
    ...
    description=(
        "List data items in a dataset as structured JSON for the XingGraph workspace UI. "
...
def _is_agent_scoping_enabled() -> bool:
    ...
    Controlled by XINGGRAPH_MCP_AGENT_SCOPED env var (default: 'true').
    When 'false', tools fall back to 'main_dataset' as the default,
    matching the pre-agent-scoping behavior.
    """
    return os.getenv("XINGGRAPH_MCP_AGENT_SCOPED", "true").strip().lower() != "false"
...
def _agent_scoped_default_dataset() -> str:
    ...
    'cursor_vscode_memory'). Falls back to 'main_dataset' when:
      - agent scoping is disabled via XINGGRAPH_MCP_AGENT_SCOPED=false, or
...
async def get_client_info_json() -> types.CallToolResult:
    ...
    if agent_scoped:
        ...
        if not xinggraph_client.use_api:
            with redirect_stdout(sys.stderr):
                from xinggraph.modules.data.methods.create_authorized_dataset import (
...
async def main():
    ...
    # XingGraph API connection options
    parser.add_argument(
        "--api-url",
        default=None,
        help="Base URL of a running XingGraph FastAPI server (e.g., http://localhost:8000). "
        "If provided, the MCP server will connect to the API instead of using xinggraph directly.",
    )
    ...
    # XingGraph Cloud connection options
    parser.add_argument(
        "--serve-url",
        default=None,
        help="XingGraph Cloud or remote instance URL (e.g., https://your-instance.xinggraph.ai). "
        "Calls xinggraph.serve() at startup so all SDK operations route to the cloud. "
        "Can also be set via XINGGRAPH_SERVICE_URL env var.",
    )

    parser.add_argument(
        "--serve-api-key",
        default=None,
        help="API key for the XingGraph Cloud instance. Can also be set via XINGGRAPH_API_KEY env var.",
    )

    args = parser.parse_args()

    # Initialize the global XingGraphClient
    xinggraph_client = XingGraphClient(api_url=args.api_url, api_token=args.api_token)
    ...
    serve_url = args.serve_url or os.environ.get("XINGGRAPH_SERVICE_URL", "")
    serve_api_key = args.serve_api_key or os.environ.get("XINGGRAPH_API_KEY", "")

    # Connect to XingGraph Cloud if configured (before migrations — cloud handles its own DB)
    if serve_url and not args.api_url:
        ...
        await xinggraph.serve(**serve_kwargs)
        logger.info(f"Connected to XingGraph Cloud: {serve_url}")
    ...
    if not args.no_migration and not is_remote:
        ...
        from xinggraph.run_migrations import run_migrations
        ...
        # applies around its xinggraph calls.
        with redirect_stdout(sys.stderr):
            await setup()
        ...
        if _background_tasks:
            ...
            try:
                ...
                for t in _background_tasks:
                    t.cancel()
        if xinggraph_client is not None:
            await xinggraph_client.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(f"Error initializing XingGraph MCP server: {str(e)}")
        raise
