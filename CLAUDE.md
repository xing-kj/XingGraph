...
XingGraph is an open-source AI memory platform that transforms raw data into persistent knowledge graphs for AI agents. It replaces traditional RAG (Retrieval-Augmented Generation) with an ECL (Extract, Cognify, Load) pipeline combining vector search, graph databases, and LLM-powered entity extraction.
...
# Run with coverage
pytest --cov=xinggraph --cov-report=html

# Run specific test file
pytest xinggraph/tests/test_custom_model.py

# Run specific test function
pytest xinggraph/tests/test_custom_model.py::test_function_name

# Run async tests
pytest -v xinggraph/tests/integration/

# Run unit tests only
pytest xinggraph/tests/unit/

# Run integration tests only
pytest xinggraph/tests/integration/
```
...
# Using CLI
xinggraph-cli add "Your text here"
xinggraph-cli cognify
xinggraph-cli search "Your query"
xinggraph-cli delete --all
...
All data flows through task-based pipelines (`xinggraph/modules/pipelines/`). Tasks are composable units that can run sequentially or in parallel. Example pipeline tasks: `classify_documents`, `extract_graph_from_data`, `add_data_points`.
...
Key files:
- `xinggraph/infrastructure/databases/graph/graph_db_interface.py`
- `xinggraph/infrastructure/databases/vector/vector_db_interface.py`
...
```
API Layer (xinggraph/api/v1/)
    ↓
Main Functions (add, cognify, search, memify)
    ↓
Pipeline Orchestrator (xinggraph/modules/pipelines/)
    ↓
Task Execution Layer (xinggraph/tasks/)
    ↓
...
Key files: `xinggraph/api/v1/add/add.py`, `xinggraph/tasks/ingestion/ingest_data.py`
...
Key files:
- `xinggraph/api/v1/cognify/cognify.py`
- `xinggraph/tasks/graph/extract_graph_from_data.py`
- `xinggraph/tasks/storage/add_data_points.py`

#### SEARCH: Retrieval
`search(query_text, query_type)` → route to retriever type → filter by permissions → return results

Available search types (from `xinggraph/modules/search/types/SearchType.py`):
...
Key files:
- `xinggraph/api/v1/search/search.py`
- `xinggraph/modules/retrieval/context_providers/TripletSearchContextProvider.py`
- `xinggraph/modules/search/types/SearchType.py`

### Core Data Models

#### Engine Models (`xinggraph/infrastructure/engine/models/`)
...
#### Graph Models (`xinggraph/shared/data_models.py`)
...
#### LLM Gateway (`xinggraph/infrastructure/llm/LLMGateway.py`)
...
#### Embedding Engines
Factory pattern for embeddings: `xinggraph/infrastructure/databases/vector/embeddings/get_embedding_engine.py`

#### Document Loaders
Support for PDF, DOCX, CSV, images, audio, code files in `xinggraph/infrastructure/files/`
...
```bash
# PostgreSQL (requires postgres extra: pip install xinggraph[postgres])
...
VECTOR_DB_URL=postgresql://xinggraph:xinggraph@localhost:5432/xinggraph_db
```

#### Graph Databases
Supported: ladybug (default), neo4j, neptune, ladybug-remote, postgres
```bash
# Neo4j (requires neo4j extra: pip install xinggraph[neo4j])
...
# Postgres (requires postgres extra: pip install xinggraph[postgres])
# Does not support raw Cypher queries, natural language search, or Graphiti.
GRAPH_DATABASE_PROVIDER=postgres
GRAPH_DATABASE_URL=postgresql+asyncpg://xinggraph:xinggraph@localhost:5432/xinggraph_db
```
...
CACHE_DB_URL=postgresql+asyncpg://xinggraph:xinggraph@localhost:5432/xinggraph_db
```
...
# Or use BAML (requires baml extra: pip install xinggraph[baml])
...
# S3 (requires aws extra: pip install xinggraph[aws])
STORAGE_BACKEND="s3"
STORAGE_BUCKET_NAME="your-bucket-name"
AWS_REGION="us-east-1"
AWS_ACCESS_KEY_ID="your_access_key"
AWS_SECRET_ACCESS_KEY="your_secret_key"
DATA_ROOT_DIRECTORY="s3://your-bucket/xinggraph/data"
SYSTEM_ROOT_DIRECTORY="s3://your-bucket/xinggraph/system"
```
...
1. **New Task Type**: Create task function in `xinggraph/tasks/`, return Task object, register in pipeline
2. **New Database Backend**: Implement `GraphDBInterface` or `VectorDBInterface` in `xinggraph/infrastructure/databases/`
3. **New LLM Provider**: Add configuration in LLM config (uses litellm)
4. **New Document Processor**: Extend loaders in `xinggraph/modules/data/processing/`
5. **New Search Type**: Add to `SearchType` enum and implement retriever in `xinggraph/modules/retrieval/`
6. **Custom Graph Models**: Define Pydantic models extending `DataPoint` in your code

### Working with Ontologies
XingGraph supports ontology-based entity extraction to ground knowledge graphs in standardized semantic frameworks (e.g., OWL ontologies).
...
Implementation: `xinggraph/modules/ontology/`
...
Tests are organized in `xinggraph/tests/`:
...
FastAPI application with versioned routes under `xinggraph/api/v1/`:
- `/add` - Data ingestion
...
Main functions exported from `xinggraph/__init__.py`:
...
- `ALLOW_HTTP_REQUESTS` - Allow HTTP requests from XingGraph (default: True)
...
### Using LLM Gateway
```python
from xinggraph.infrastructure.llm.get_llm_client import get_llm_client
...
await xinggraph.add(data, dataset_name="my_project")
await xinggraph.cognify(datasets=["my_project"])
```
...
```bash
# Via CLI
xinggraph-cli -ui  # Launches full stack with UI at http://localhost:3000

# Via Python
from xinggraph.api.v1.visualize import start_visualization_server
...
- Use `debugpy` optional dependency for debugging: `pip install xinggraph[debug]`
...
- [Documentation](https://docs.xinggraph.ai/)
- [Discord Community](https://discord.gg/NQPKmU5CCg)
- [GitHub Issues](https://github.com/xing-kj/xinggraph/issues)
