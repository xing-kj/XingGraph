...
function pipelineLabel(name: string): string {
  if (name.includes("cognify")) return "xinggraph.cognify";
  if (name.includes("add")) return "xinggraph.add";
  if (name.includes("search")) return "xinggraph.search";
  if (name.includes("recall")) return "xinggraph.recall";
  return name;
}

export function ownerDisplayName(email: string | null): string {
  if (!email) return "System";
  if (email.endsWith("@xinggraph.agent")) {
...
export function AgentActivityTerminal({
  ...
  // Two trigger points:
  //   - Step 3 of onboarding ("Ask xinggraph anything"), where the variant is
  ...
  const [demoEligible] = useState(() => {
    try {
      if (localStorage.getItem("xinggraph-terminal-demo-shown") === "1") return false;
      if (variant === "onboarding") return true;
      return !!localStorage.getItem("xinggraph-onboarding-complete");
    } catch { return false; }
  });
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    try { localStorage.setItem("xinggraph-terminal-demo-shown", "1"); } catch {}
