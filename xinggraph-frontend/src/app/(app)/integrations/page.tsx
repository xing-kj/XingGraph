...
const VSCODE_MCP_CONFIG = `{
  "inputs": [
    {
      "type": "promptString",
      "id": "xinggraph-api-key",
      "description": "XingGraph API key",
      "password": true
    }
  ],
  "servers": {
    "xinggraph": {
      ...
      "env": {
        ...
        "XINGGRAPH_API_KEY": "\${input:xinggraph-api-key}"
      }
    }
  }
}`;
...
function credStep(baseUrl: string, apiKey: string, loading: boolean): StepDef {
  return {
    ...
    description: "Open a terminal and run these commands to configure your XingGraph endpoint and key.",
    code: `export XINGGRAPH_BASE_URL="${baseUrl}"`,
    codeToCopy: `export XINGGRAPH_BASE_URL="${baseUrl}"\nexport XINGGRAPH_API_KEY="${apiKey}"`,
    loading,
  };
}
function mcpConfigStep(displayPath: string, app: string, dir: string, file: string, baseUrl: string, apiKey: string, loading: boolean): StepDef {
  return {
    title: `Configure ${app}`,
    description: `Run this command to write the config file (an existing one is backed up to .bak first), then restart ${app}. XingGraph runs via uvx — no separate install needed (requires uv). If you already use other MCP servers in ${app}, merge the "xinggraph" entry into your existing config instead.`,
    code: displayPath,
    codeToCopy: `mkdir -p ${dir} && [ -f ${file} ] && cp ${file} ${file}.bak; cat > ${file} << 'XINGGRAPH_EOF'\n${fillTemplate(MCP_STDIO_CONFIG, baseUrl, apiKey)}\nXINGGRAPH_EOF`,
    loading,
  };
}
...
const AGENT_CARDS: AgentCfg[] = [
  {
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      ...
      {
        title: "Install the XingGraph plugin",
        description: "Run these in your terminal one at a time — register the XingGraph marketplace, then install the memory plugin.",
        codeBlocks: [
      ...
      {
        title: "Upload something to XingGraph",
        description: "Pick one and paste it into Claude Code — it stores the content in your XingGraph memory so you can recall it next.",
        codeBlocks: [
      ...
      {
        title: "Recall it from XingGraph",
        description: "First run /exit to close the session — that syncs it into XingGraph Cloud — then reopen Claude Code and ask the question below. Answering from a fresh session proves it's recalling from your cloud memory.",
        codeBlocks: [
          { code: "/exit" },
      ...
      },
      {
        ...
        description: "The XingGraph plugin hooks into Claude Code's lifecycle — no curl or manual API calls — and captures your session as you work. When a session ends (e.g. /exit), it consolidates that session into your XingGraph Cloud knowledge graph, and every new session automatically recalls it back. Sessions are disposable; your memory isn't.",
      },
    ],
  },
  {
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      ...
      {
        title: "Install the XingGraph plugin",
        description: "Run these in your terminal one at a time — enable Codex hooks, register the XingGraph marketplace, then install the memory plugin.",
        codeBlocks: [
      ...
      {
        title: "Upload something to XingGraph",
        description: "Pick one and paste it into Codex — it stores the content in your XingGraph memory so you can recall it next.",
        codeBlocks: [
          { label: "Option A · Your existing memory", code: UPLOAD_MEMORY_PROMPT },
          { label: "Option B · Try it with a sample", code: UPLOAD_SAMPLE_PROMPT },
        ],
      ...
      {
        title: "Recall it from XingGraph",
        description: "First run /exit to close the session — that syncs it into XingGraph Cloud — then reopen Codex and ask the question below. Answering from a fresh session proves it's recalling from your cloud memory.",
        codeBlocks: [
          { code: "/exit" },
          { code: RECALL_SAMPLE_PROMPT },
        ],
      ...
      {
        title: "You're all set",
        description: "The XingGraph plugin hooks into Codex's lifecycle — no curl or manual API calls — and captures your session as you work. When a session ends (e.g. /exit), it consolidates that session into your XingGraph Cloud knowledge graph, and every new session automatically recalls it back. Sessions are disposable; your memory isn't.",
      },
    ],
  },
  {
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      ...
      { title: "Create the workspace AGENTS.md", description: "Run this command to add the XingGraph memory instructions to OpenClaw's workspace. An existing AGENTS.md is backed up to AGENTS.md.bak — merge it manually afterwards.", code: "~/.openclaw/workspace/AGENTS.md", codeToCopy: `mkdir -p ~/.openclaw/workspace && [ -f ~/.openclaw/workspace/AGENTS.md ] && cp ~/.openclaw/workspace/AGENTS.md ~/.openclaw/workspace/AGENTS.md.bak; cat > ~/.openclaw/workspace/AGENTS.md << 'XINGGRAPH_EOF'\n${OPENCLAW_PROMPT}\nXINGGRAPH_EOF` },
      { title: "Test the connection", description: `Open OpenClaw and ask: "What do you know from xinggraph?" — if it responds with knowledge from your brain, you're connected.` },
    ],
  },
  {
    key: "claude-desktop",
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      ...
      { title: "Test the connection", description: "Restart Claude Desktop. Open a new conversation and ask: \"What do you know from xinggraph?\" — XingGraph's memory will respond." },
    ],
  },
  {
    key: "cursor",
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      ...
      { title: "Test the connection", description: "Restart Cursor. Open a project and ask the agent: \"What do you know from xinggraph?\" — you should get a memory-grounded response." },
    ],
  },
  {
    key: "hermes",
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      { title: "Configure Hermes Agent", description: "Hermes reads YAML — add this block under mcp_servers in ~/.hermes/config.yaml and restart the agent. XingGraph runs via uvx (requires uv) — no separate install.", code: "~/.hermes/config.yaml", codeToCopy: fillTemplate(HERMES_MCP_CONFIG, baseUrl, apiKey), loading },
      { title: "Test the connection", description: "Ask Hermes: \"What do you know from xinggraph?\" — it should use the XingGraph memory tool and return a response from your brain." },
    ],
  },
  {
    key: "vscode",
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      { title: "Configure VS Code", description: "From your project root, run this command to register the XingGraph MCP server for VS Code (Copilot agent mode), then reload the window. XingGraph runs via uvx (requires uv) — no separate install. VS Code prompts for your API key on first use (kept out of the committable file); an existing mcp.json is backed up to .bak.", code: ".vscode/mcp.json", codeToCopy: `mkdir -p .vscode && [ -f .vscode/mcp.json ] && cp .vscode/mcp.json .vscode/mcp.json.bak; cat > .vscode/mcp.json << 'XINGGRAPH_EOF'\n${fillTemplate(VSCODE_MCP_CONFIG, baseUrl, apiKey)}\nXINGGRAPH_EOF`, loading },
      { title: "Test the connection", description: "Open Copilot Chat in agent mode and ask: \"What do you know from xinggraph?\" — the XingGraph memory tools should respond." },
    ],
  },
  {
    key: "gemini-cli",
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      ...
      { title: "Test the connection", description: "Run `gemini` in your terminal and ask: \"What do you know from xinggraph?\" — XingGraph's memory tool will respond." },
    ],
  },
  {
    key: "cline",
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      { title: "Configure Cline", description: "In VS Code, open the Cline sidebar → MCP Servers → Configure MCP Servers — paste this JSON block and save. XingGraph runs via uvx (requires uv) — no separate install.", code: '{ "mcpServers": { "xinggraph": … } }', codeToCopy: fillTemplate(MCP_STDIO_CONFIG, baseUrl, apiKey), loading },
      { title: "Test the connection", description: "Ask Cline: \"What do you know from xinggraph?\" — it should use the XingGraph memory tool and return a response from your brain." },
    ],
  },
  {
    key: "https-api",
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      credStep(baseUrl, apiKey, loading),
      {
        ...
        description: "Send a recall query to your XingGraph endpoint from any HTTP client or language.",
      ...
      {
        title: "Or install the XingGraph skill",
        description: "Prefer skills? Run this command from your project root to create the skill file, then point your agent at it (skills directory, instructions file, or system prompt). The skill teaches your agent to call the XingGraph API using the credentials from step 1.",
        code: "skills/xinggraph/SKILL.md",
        codeToCopy: GENERIC_SKILL_INSTALL,
      },
      { title: "Test the connection", description: "Ask your agent: \"What do you know from xinggraph?\" — XingGraph's memory tool should respond with knowledge from your brain." },
    ],
  },
];
...
const AUTOMATION_CARDS: AgentCfg[] = [
  {
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      { title: "Install the XingGraph community node", description: "In n8n, open Settings → Community Nodes → Install and enter the package name.", code: "n8n-nodes-xinggraph", codeToCopy: "n8n-nodes-xinggraph" },
      { title: "Create the XingGraph API credential", description: "Add a \"XingGraph API\" credential, paste your Base URL and API key, and click Test — it verifies the connection against your tenant.", code: baseUrl, codeToCopy: `${baseUrl}\n${apiKey}`, loading },
      { title: "Test the connection", description: "Add a XingGraph node to a workflow (or attach it to an AI Agent) and run it — it should answer from your brain." },
    ],
  },
  {
    key: "dify",
    ...
    buildSteps: (baseUrl, apiKey, loading) => [
      { title: "Install the XingGraph plugin", description: "In your Dify workspace, open the Marketplace, search for \"XingGraph\" (by xing-kj), and click Install.", code: "marketplace.dify.ai/plugin/xing-kj/xinggraph", codeToCopy: "https://marketplace.dify.ai/plugin/xing-kj/xinggraph" },
      // The Dify plugin needs the /api suffix — without it, validation passes (root /health) but every tool call 404s.
      { title: "Configure the plugin", description: "Open the plugin's authorization settings and enter your XingGraph Base URL (including the /api suffix) and API key.", code: `${baseUrl}/api`, codeToCopy: `${baseUrl}/api\n${apiKey}`, loading },
      { title: "Add XingGraph tools to your app", description: "In an Agent or Workflow app, add the XingGraph tools — create a dataset, ingest text or files, run Cognify, then search your memory." },
      { title: "Test the connection", description: "Run the app and ask: \"What do you know from xinggraph?\" — the XingGraph search tool should answer from your brain." },
    ],
  },
];
...
function AgentSection({ cards }: { cards: AgentCfg[] }) {
  ...
  const baseUrl = serviceUrl || "https://your-tenant.aws.xinggraph.ai";
      ...
      <div style={{ padding: "24px 32px 40px", display: "flex", flexDirection: "column", gap: 40 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "rgba(237,236,234,0.55)", margin: 0 }}>Connect your AI agents and coding tools to XingGraph for persistent memory.</p>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "rgba(237,236,234,0.55)", margin: 0 }}>Give your automation workflows access to XingGraph memory via MCP.</p>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <p style={{ fontSize: 13, color: "rgba(237,236,234,0.35)", textAlign: "center", margin: 0 }}>
            ...
            <a href="mailto:support@xinggraph.ai?subject=Integration%20request" style={{ color: "#6510F4", textDecoration: "underline" }}>Let us know</a> what to prioritize.
          </p>
        </div>
