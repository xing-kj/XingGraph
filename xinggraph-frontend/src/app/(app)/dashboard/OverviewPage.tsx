...
const AWAITING_DATASET_KEY = "xinggraph-awaiting-dataset";
...
// (see xing-kj/xinggraph-integrations: claude-code → "cc", codex → "codex"), so
...
// forwards the host session id (no XingGraph prefix) and API/MCP is user-defined, so
...
export default function OverviewPage() {
  ...
  const [graphNodes, setGraphNodes] = useState<number | null>(() => {
    try { const v = sessionStorage.getItem("xinggraph-graph-nodes"); return v !== null ? Number(v) : null; } catch { return null; }
  });
  ...
  const [creditsBannerDismissed, setCreditsBannerDismissed] = useState<boolean>(() => {
    try { return sessionStorage.getItem("xinggraph-credits-banner-dismissed") === "1"; } catch { return false; }
  });
  ...
  useEffect(() => {
    if (!tenantId) return;
    const storeKey = `xinggraph-connected-integrations-${tenantId}`;
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      try { sessionStorage.setItem("xinggraph-graph-nodes", String(totalNodes)); } catch {}
    }).catch(() => {});
    return () => { cancelled = true; };
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const localSkipped = sessionStorage.getItem("xinggraph-onboarding-skipped");
      if (localSkipped) {
      ...
      try {
        ...
        if (appState?.onboarding_complete) {
          ...
          const localComplete = localStorage.getItem("xinggraph-onboarding-complete");
          if (localComplete) {
            setOnboardingDecided(true);
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100%" }}>
        <div
          style={{
            ...
            animation: "xinggraph-spin 0.8s linear infinite",
          }}
        />
        <style>{"@keyframes xinggraph-spin { to { transform: rotate(360deg); } }"}</style>
      </div>
    );
  }
      ...
      {showLlmKeyModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.35)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => { if (!llmKeySaving) setShowLlmKeyModal(false); }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "rgba(15,15,15,0.92)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 24, width: 420, maxWidth: "calc(100vw - 32px)", display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
            ...
            <p style={{ fontSize: 13, color: "rgba(237,236,234,0.65)", margin: 0 }}>
              XingGraph uses <strong style={{ color: "#EDECEA" }}>{llmSettings?.provider || "openai"}</strong> ({llmSettings?.model || "gpt-5-mini"}) to build your knowledge graph. Paste your API key — it&apos;s applied to the running backend right away.
            </p>
            <input
              type="password"
              value={llmKeyInput}
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 13, color: "#FDE047" }}>
              No LLM API key configured — XingGraph can&apos;t process uploads until you add one.
            </span>
          </div>
          <button
      ...
      {showCreditPctBanner && (
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            ...
            <button
              onClick={() => {
                try { sessionStorage.setItem("xinggraph-credits-banner-dismissed", "1"); } catch {}
...
function AgentConnectionSection({
  ...
  // means the user's agent successfully connected to XingGraph Cloud — we then mark
  ...
  // XingGraph Cloud standardizes on XINGGRAPH_BASE_URL everywhere user-facing — the
  // plugins/skills (claude-code, codex, …) and the REST flows all read it.
  const CREDS_CODE = `export XINGGRAPH_BASE_URL="${baseUrl}"\nexport XINGGRAPH_API_KEY="${resolvedKey}"`;
  ...
  const CARDS_CFG: { key: AciAgentKey; name: string; description: string }[] = [
    ...
    { key: "codex",       name: "Codex",       description: "Connect OpenAI Codex to your knowledge graph via the XingGraph plugin" },
  ...
  function getSteps(key: AciAgentKey): AciStepDef[] {
    ...
    if (key === "claude-code") return [
      ...
      },
      {
        ...
        description: "Pick one and paste it into Claude — it stores the content in your XingGraph memory so you can recall it in the next step.",
        codeBlocks: [
      ...
      {
        title: connectVerified ? "Connected — session detected ✓" : "Recall it from XingGraph",
        description: connectVerified
          ? "We detected your new session in XingGraph Cloud — you're connected."
          : "First run /exit to close the session — that syncs it into XingGraph Cloud — then reopen Claude Code and ask the question below. Answering from a fresh session proves it's recalling from your cloud memory.",
        codeBlocks: [
          { code: "/exit" },
    ...
    if (key === "codex") return [
      ...
      {
        title: "Upload something to XingGraph",
        description: "Pick one and paste it into Codex — it stores the content in your XingGraph memory so you can recall it in the next step.",
        codeBlocks: [
          { label: "Option A · Your existing memory", code: UPLOAD_MEMORY_PROMPT },
          { label: "Option B · Try it with a sample", code: UPLOAD_SAMPLE_PROMPT },
        ],
    ...
    if (key === "openclaw") return [
      credStep,
      {
        title: "Install the XingGraph skill",
        ...
        skillPath: "~/.openclaw/skills/xinggraph/SKILL.md",
        skillContent: OPENCLAW_SKILL_INSTALL,
      },
      {
        title: "Test the connection",
        description: `Open Openclaw in your project and ask: "What do you know from xinggraph?" — if it responds with knowledge from your brain, you're connected.`,
      },
    ];
    if (key === "api-mcp") return [
      ...
      {
        title: "Test the connection",
        description: `Ask your agent: "What do you know from xinggraph?" — XingGraph's memory should respond with knowledge from your brain.`,
      },
    ];
    return [];
  }
