...
export default function KnowledgeGraphPage() {
  ...
  const loadIframe = async () => {
    if (!datasetId || !cogniInstance) return;
    setLoading(true);
    setIframeSrc(null);
    setError(null);
    setVizReady(false);
    ...
    try {
      const r = await cogniInstance.fetch(`/v1/visualize?dataset_id=${datasetId}`);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 32px", flexShrink: 0, gap: 12, flexWrap: "wrap", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        <button
          onClick={() => { setPollKey((k) => k + 1); loadIframe(); }}
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
        {(loading || (!iframeSrc && datasetStatus === "running")) && (
