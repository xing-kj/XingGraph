...
// Default extraction prompt from xinggraph OSS (generate_graph_prompt.txt)
...
# 5. Product Model Codes
A "product model" is a concrete variant/SKU code of a product family (e.g. "DW-86L959W", "HCP-168", "HYR-111"). It is NOT a generic noun ("freezer", "model", "产品") and NOT a whole sentence/description.
Rules for product model codes:
  - Extract the code as its OWN node; keep it verbatim (letter case as written in the text).
  - Set the node type to "PRODUCT_MODEL".
  - Do NOT merge the code into the product family's name or description.
  - Only tag PRODUCT_MODEL when the token is clearly a concrete model/SKU designator.
A "product category" is the generic product family the model belongs to (e.g. "Haier Biomedical medical low-temperature preservation box").
  - Extract it as its OWN node with type "Product".
  - The category and each of its model codes must be separate nodes.

# 6. is_product Relationship
When a product category clearly is the family that a PRODUCT_MODEL code belongs to, add an edge between them:
  - source_node_id: the product category node
  - target_node_id: the PRODUCT_MODEL code node
  - relationship_name: "is_product" (exactly this, snake_case, do not paraphrase)
    ...
    >
            ...
            {inferring ? (
              ...
              <>
                <p style={{ fontSize: 13, color: "rgba(237,236,234,0.55)", margin: 0, lineHeight: "20px" }}>
                  Would you like to infer a graph model from your existing data? XingGraph will analyze your files and suggest entity types and relationships.
                </p>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => !inferringPrompt && setShowCreatePromptModal(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "rgba(15,15,15,0.92)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 24, width: 440, display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 16px 48px rgba(0,0,0,0.12)" }}>
            ...
            {inferringPrompt ? (
              ...
              <>
                <p style={{ fontSize: 13, color: "rgba(237,236,234,0.55)", margin: 0, lineHeight: "20px" }}>
                  A custom prompt guides how XingGraph extracts entities and relationships from your data.
                </p>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <button
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: "rgba(15,15,15,0.92)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 24, width: 600, maxHeight: "80vh", display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 16px 48px rgba(0,0,0,0.12)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1, minHeight: 0 }}>
              ...
              <textarea
                ...
                placeholder="Write your extraction prompt here. This prompt will be used by XingGraph when extracting entities and relationships from your data.&#10;&#10;Example: Extract all companies, people, and their relationships from the text. Focus on ownership, employment, and partnership relations."
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(237,236,234,0.55)", letterSpacing: 0.2, display: "flex", alignItems: "center", gap: 4 }}>Prompt <Tooltip label="Custom instructions that guide how XingGraph extracts entities and relationships from your data." withArrow multiline w={240} position="top"><svg width="12" height="12" viewBox="0 0 16 16" fill="none" style={{ flexShrink: 0 }}><circle cx="8" cy="8" r="7" stroke="#A1A1AA" strokeWidth="1.5" /><text x="8" y="12" textAnchor="middle" fontSize="10" fontWeight="700" fill="#A1A1AA">i</text></svg></Tooltip></span>
      ...
      {showUploadOntologyModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowUploadOntologyModal(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "rgba(15,15,15,0.92)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 24, width: 440, display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 16px 48px rgba(0,0,0,0.12)" }}>
            ...
            <p style={{ fontSize: 13, color: "rgba(237,236,234,0.55)", margin: 0, lineHeight: "20px" }}>
              Upload an OWL ontology file to guide how XingGraph structures your knowledge graph.
            </p>
