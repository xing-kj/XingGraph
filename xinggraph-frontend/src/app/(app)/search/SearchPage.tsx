...
type SearchType = "HYBRID_COMPLETION" | "GRAPH_COMPLETION" | "WIKI_COMPLETION" | "RAG_COMPLETION";
...
function RelatedEntitiesCard({ entities }: { entities?: RelevantEntity[] }) {
  const list = Array.isArray(entities) ? entities : [];
  if (list.length === 0) return null;

  const models = list.filter((e) => e.is_model);
  const others = list.filter((e) => !e.is_model);

  const row = (e: RelevantEntity, tag?: string) => (
    <div
      key={e.name}
      style={{
        display: "flex",
    ...
    >
      ...
      <span style={{ fontWeight: 600, color: "#EDECEA", whiteSpace: "nowrap" }}>{e.name}</span>
      {e.description ? (
        <span style={{ color: "rgba(237,236,234,0.55)", minWidth: 0 }}>{e.description}</span>
      ) : null}
    </div>
  );
  ...
  return (
    <div style={{
      marginTop: 10,
      border: "1px solid rgba(188,155,255,0.18)",
      borderRadius: 10,
      background: "rgba(188,155,255,0.06)",
      padding: "12px 14px",
    }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: "#BC9BFF", marginBottom: 8 }}>相关实体 ({list.length})</span>
      </div>
      {models.length > 0 && (
        <div style={{ marginBottom: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", color: "rgba(237,236,234,0.45)", marginBottom: 4 }}>
            型号 ({models.length})
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {models.map((e) => row(e, "型号"))}
          </div>
        </div>
      )}
      {others.length > 0 && (
        <details>
          <summary style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", color: "rgba(237,236,234,0.45)", cursor: "pointer", marginBottom: 4 }}>
            其他实体 ({others.length})
          </summary>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {others.map((e) => row(e))}
...
type RetrievalTrace = Record<string, any>;
...
function RetrievalTracePanel({ trace, context }: { trace?: RetrievalTrace; context?: string }) {
  const [open, setOpen] = useState(false);
  if (!trace) return null;

  const s1: any[] = Array.isArray(trace.step1_entities) ? trace.step1_entities : [];
  const s0: any = trace.step0_llm_entities || {};
  const s2: any = trace.step2_traversal || {};
  const s3: any = trace.step3_wiki_expansion || {};
  const s4: any = trace.step4_screening || {};
  const mode: string = trace.step5_answer_mode || "unknown";
  const sufficient = Array.isArray(s4.sufficient) ? s4.sufficient : [];
  const screened = Array.isArray(s4.wikis) ? s4.wikis : [];
  const accepted = screened.filter((w: any) => sufficient.includes(w.id));
  ...
  const stepTitle: React.CSSProperties = {
    ...
    textTransform: "uppercase", color: "#BC9BFF",
  };
  const shortId = (id?: string) => (id ? id.slice(0, 8) : "?");
  const ENTITY_CAP = 8;

  // Step 1: client-side sort (score desc) + dedupe by name (keep best score).
  const seen = new Set<string>();
  const s1Sorted: any[] = [];
  for (const e of s1) {
    const key = e && e.name ? String(e.name) : e && e.id ? String(e.id) : "?";
  ...
  const s1Shown = s1Sorted.slice(0, ENTITY_CAP);
  const s1Extra = s1Sorted.length - ENTITY_CAP;
  const entityText = s1Shown.length
    ? s1Shown.map((e) => `${e && (e.name || e.id) ? e.name || shortId(e.id) : "?"}${e && e.related_subject ? `→${e.related_subject}` : ""}${typeof e.score === "number" ? `  ·  ${e.score.toFixed(2)}` : ""}`).join("　")
    : "(空)";

  const s0Subj: string[] = Array.isArray(s0.subjects) ? s0.subjects : [];
  const s0Attr: any[] = Array.isArray(s0.attributes) ? s0.attributes : [];
  const dedup: any = trace.step1_dedup || {};
  const droppedList: any[] = Array.isArray(dedup.dropped) ? dedup.dropped : [];

  const paths: any[] = Array.isArray(s2.paths) ? s2.paths : [];;

  const full = { ...trace, context };

  return (
    <div style={{ marginTop: 10, borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 10 }}>
      <button
        onClick={() => setOpen((o) => !o)}
        style={{ background: "none", border: "none", color: "#BC9BFF", cursor: "pointer", fontSize: 12, fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6, padding: 0 }}
      >
        <span style={{ display: "inline-block", transform: open ? "rotate(90deg)" : "none", transition: "transform 150ms", fontSize: 10 }}>▶</span>
        查看检索链路{open ? "" : `  (${mode})`}
      </button>
      ...
      {open && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 8 }}>
          <div style={stepStyle}>
            <span style={stepTitle}>Step 0 · LLM 拆解 · 主体 {s0Subj.length} · 属性 {s0Attr.length}</span>
            <span>
              主体: {s0Subj.length ? s0Subj.join("、") : "(整句回退)"}
            </span>
            <span>
              属性: {s0Attr.length
                ? s0Attr.map((a) => `${a.term}${a.subject ? `→${a.subject}` : ""}`).join("　")
                : "(无)"}
            </span>
            {typeof dedup.raw === "number" ? (
              <span style={{ fontSize: 11, color: "rgba(237,236,234,0.4)" }}>
                原始命中 {dedup.raw} · 保留 {dedup.kept?.length ?? 0} · 丢弃 {droppedList.length} · 去重后 {dedup.unique}
              </span>
            ) : null}
          </div>
          <div style={stepStyle}>
            <span style={stepTitle}>Step 1 · 实体召回 · 去重后 {s1Sorted.length}</span>
            <span>{entityText}{s1Extra > 0 ? `　(+${s1Extra})` : ""}</span>
            {droppedList.length ? (
              <div style={{ fontSize: 11, color: "rgba(252,165,165,0.75)" }}>
                丢弃(不同源文档): {droppedList.map((d) => d.name || "?").join("、")}
              </div>
            ) : null}
          </div>
          <div style={stepStyle}>
            <span style={stepTitle}>Step 2 · 图遍历 · {s2.max_hops || 2}跳</span>
            <span>Entity {s2.entity_nodes ?? 0} · Chunk {s2.chunk_nodes ?? 0} · Wiki {s2.wiki_nodes ?? 0} · 簇对 {s2.chunk_wiki_pairs ?? 0}</span>
            {paths.length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 5, marginTop: 4 }}>
                <span style={{ fontSize: 11, color: "rgba(237,236,234,0.45)" }}>路径示例:</span>
                {paths.map((p: any, i: number) => {
                  const ents = Array.isArray(p.entities) ? p.entities.join("、") : "";
                  const wikiText = Array.isArray(p.wikis)
                    ? p.wikis.map((w: any) => w.summary || shortId(w.id)).join(" / ")
                    : "";
                  return (
                    <div key={i} style={{ display: "flex", flexDirection: "column", gap: 1, fontSize: 11, lineHeight: "16px" }}>
                      <span style={{ color: "rgba(237,236,234,0.85)" }}>
                        <span style={{ color: "#BC9BFF" }}>{i + 1}.</span>{" "}
                        {ents ? `实体[${ents}] → ` : ""}
                        块[{p.chunk_snippet || shortId(p.chunk_id)}]
                        {wikiText ? ` → Wiki[${wikiText}]` : ""}
                      </span>
                      {p.document_name ? (
                        <span style={{ color: "rgba(237,236,234,0.35)", paddingLeft: 16 }}>来源: {p.document_name}</span>
                      ) : null}
                    </div>
                  );
                })}
          ...
          <div style={stepStyle}>
            <span style={stepTitle}>Step 3 · Wiki 向量补充</span>
            <span>{s3.enabled ? `新增 ${s3.added ?? 0} 条` : "(未启用 / 无实体)"}</span>
          </div>
          <div style={stepStyle}>
            <span style={stepTitle}>Step 4 · LLM 筛选 · {s4.outcome || "unknown"}</span>
            {accepted.length > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {accepted.map((w: any, i: number) => (
                  <div key={i} style={{ borderLeft: "2px solid #BC9BFF", paddingLeft: 8 }}>
                    <span style={{ color: "rgba(237,236,234,0.85)" }}>{w.summary}</span>
                    <span style={{ color: "rgba(237,236,234,0.4)", fontSize: 11 }}>
                      {" "}· Topics: {Array.isArray(w.key_topics) ? w.key_topics.join(", ") : ""}
                    </span>
                  </div>
                ))}
              </div>
            ) : (
              <span>无足够 wiki 采纳（{s4.considered ?? 0} 个候选）</span>
            )}
          </div>
          <div style={stepStyle}>
            <span style={stepTitle}>Step 5 · 答案模式</span>
            <span style={{ color: mode === "fulltext_fallback" ? "#FCA5A5" : "#7EE0A3", fontWeight: 600 }}>{mode}</span>
          </div>
          {context ? (
            <details style={stepStyle}>
              <summary style={{ ...stepTitle, cursor: "pointer" }}>上下文给 LLM 的真实内容</summary>
              <div style={{ marginTop: 8 }}>
                {context.includes('============================================================') ? (
                  context.split('============================================================').map((section, idx) => {
                    const trimmed = section.trim();
                    if (!trimmed) return null;
                    const lines = trimmed.split('\n');
                    let title = '相关实体列表';
                    let body = trimmed;
                    if (lines[0]?.startsWith('ENTITY ')) {
                      title = lines[0].trim();
                      body = lines.slice(1).join('\n').trim();
                    } else if (lines[0]?.includes('GENERAL CONTEXT')) {
                      title = '通用信息';
                      body = lines.slice(1).join('\n').trim();
                    }
                    const icon = title.startsWith('ENTITY') ? '📦' : '📋';
                    return (
                      <div key={idx} style={{
                        ...
                        <div style={{ fontWeight: 600, marginBottom: 6, color: '#BC9BFF' }}>
                          {icon} {title}
                        </div>
                        <pre style={{
                          whiteSpace: 'pre-wrap',
          ...
          <div style={{ display: "flex", gap: 6 }}>
            ...
            <button
              onClick={() => {
                ...
                a.download = "retrieval_trace.json";
                a.click();
              }}
              style={TRACE_BTN}
            >
...
export default function SearchPage() {
  ...
  const [searchType, setSearchType] = useState<SearchType>("HYBRID_COMPLETION");
  ...
  const handleSend = async (q: string) => {
    ...
    try {
      ...
      const tracedItem = resultData.find((it) => it.trace) || resultData[0];
      const trace = tracedItem?.trace;
      const context = tracedItem?.context;
      const clarification = resultData.find((it) => it.clarification_request)?.clarification_request || undefined;
      const datasetLabel = resultData[0]?.dataset_name || selectedDataset?.name;

      setSessionConvos((prev) =>
        prev.map((c) => {
          if (c.id !== finalConvoId) return c;
          return { ...c, messages: c.messages.map((m) => m.id === loadingMsg.id ? { ...m, content, dataset: datasetLabel, trace, context, loading: false, clarification_request: clarification } : m) };
        })
      );
    } catch (err) {
      setSessionConvos((prev) =>
                      ...
                      <>
                        ...
                        {msg.clarification_request ? (
                          <div style={{
                            ...
                            💬 {stripThinkBlocks(msg.clarification_request)}
                          </div>
                        ) : (
                          <MarkdownContent text={stripThinkBlocks(msg.content)} />
                        )}
                        {msg.trace?.relevant_entities && (
                          <RelatedEntitiesCard entities={msg.trace.relevant_entities as RelevantEntity[]} />
                        )}
                        {msg.trace && <RetrievalTracePanel trace={msg.trace} context={msg.context} />}
                      </>
                    )}
                  </div>
                </div>
          ...
          <div style={{ maxWidth: 800, marginInline: "auto", display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                {(["HYBRID_COMPLETION", "GRAPH_COMPLETION", "WIKI_COMPLETION", "RAG_COMPLETION"] as SearchType[]).map((t) => {
                  const active = searchType === t;
                  const label = t.replace("_COMPLETION", "").replace("_", " ");
                  return (
                    <button
                      key={t}
                      type="button"
