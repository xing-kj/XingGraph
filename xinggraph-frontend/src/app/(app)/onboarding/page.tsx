...
function SkipLink({ label = "Skip onboarding and go to dashboard", compact = false }: { label?: string; compact?: boolean } = {}) {
  ...
  return (
    <button onClick={() => { trackEvent({ pageName: "Onboarding", eventName: "onboarding_skipped" }); sessionStorage.setItem("xinggraph-onboarding-skipped", "1"); markOnboardingCompleteLocally(); markOnboardingComplete().catch(() => {}); router.push("/dashboard"); }} className="cursor-pointer" style={{ background: "none", border: "none", color: "rgba(237,236,234,0.65)", fontSize: 13, paddingTop: compact ? 12 : 32, paddingBottom: compact ? 0 : 24 }}>
      {label}
    </button>
  );
}
    ...
    <div style={{
      ...
      <div style={{ alignItems: "center", display: "flex", flexDirection: "column", paddingBottom: 40, paddingLeft: "clamp(16px, 5vw, 80px)", paddingRight: "clamp(16px, 5vw, 80px)", paddingTop: 48, width: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{ alignItems: "center", display: "flex", flexDirection: "column", gap: 8, paddingBottom: 40 }}>
          ...
          <div style={{ color: "rgba(237,236,234,0.65)", fontSize: 15, lineHeight: "22px", textAlign: "center", maxWidth: 440 }}>Choose how to get your data into XingGraph. You can always add or change sources later.</div>
        </div>
...
function Step2({ files, datasetId, onNext, cogniInstance }: {
  ...
  async function runPipeline() {
    ...
    try {
      ...
      try { sessionStorage.setItem("xinggraph-awaiting-dataset", currentDsId!); } catch { /* ignore */ }
      stopUpload();
      ...
      try { sessionStorage.removeItem("xinggraph-awaiting-dataset"); } catch { /* ignore */ }
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 15, color: "rgba(237,236,234,0.65)", margin: 0, textAlign: "center", maxWidth: 480, lineHeight: "22px" }}>
        XingGraph is extracting entities, building relationships, and generating embeddings.
      </p>
...
function Step3({ datasetId, cogniInstance, demoEntries }: {
  ...
  return (
    <div style={{
      ...
      <h1 style={{ fontSize: 28, fontWeight: 300, color: "#EDECEA", margin: 0, fontFamily: '"TWKLausanne", sans-serif', letterSpacing: "-0.02em" }}>Ask xinggraph anything</h1>
...
// "Connected" once a new session is detected in XingGraph Local.
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5 }}>
      ...
      <span style={{ color: verified ? "#22C55E" : "rgba(237,236,234,0.5)" }}>
        {verified ? "Connected — activity detected in XingGraph Local" : "Waiting for a connection…"}
      </span>
    </div>
  );
}
...
function AgentOnboarding({ agent, serviceUrl, apiKey, cogniInstance, onRestart }: {
  ...
  const baseUrl = serviceUrl || (isCloud ? "https://your-tenant.aws.xinggraph.ai" : "http://localhost:8000");
  const resolvedKey = apiKey || "<your-api-key>";
  // API key + base url are all the plugin/skill needs.
  const credsCode = `export XINGGRAPH_BASE_URL="${baseUrl}"\nexport XINGGRAPH_API_KEY="${resolvedKey}"`;
  ...
  const credsCard = {
    ...
    description: isCloud
      ? "Open a terminal and run these to point your agent at your XingGraph memory."
      : "Create an API key on the API Keys page, then run these in a terminal to point your agent at your local XingGraph instance.",
    node: (
      <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%" }}>
        <OnboardingInlineCode code={`export XINGGRAPH_BASE_URL="${baseUrl}"`} toCopy={credsCode} loading={!credsReady} placeholder="Preparing your credentials…" />
        {!isCloud && (
          <Link
            href="/api-keys"
            target="_blank"
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, width: "100%" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: 14, borderRadius: 10, background: "rgba(237,236,234,0.04)", border: "1px solid rgba(237,236,234,0.10)" }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 18, fontSize: 13, lineHeight: "19px", color: "rgba(237,236,234,0.6)" }}>
            <li>The XingGraph plugin hooks into your agent&apos;s lifecycle — no curl or manual API calls — and captures your session as you work.</li>
            <li>When a session ends (e.g. you <strong style={{ color: "#EDECEA" }}>exit</strong>), it consolidates that session into your XingGraph Local knowledge graph.</li>
  ...
  const cards: { title: string; description: string; node?: React.ReactNode }[] =
    agent === "claude-code"
      ? [
          ...
          {
            title: connectVerified ? "Connected — activity detected" : "Recall it from XingGraph",
            description: connectVerified
              ? "We detected your new session in XingGraph Local — you're connected. You're all set."
              : "Now ask Claude a question about what you just uploaded — it should answer from XingGraph Local. (For the sample, use the question below.) This step completes on its own once your session shows up.",
            node: (
      ...
      : [
          ...
          {
            title: "Upload something to XingGraph",
            description: `Pick one and paste it into ${name} — it stores the content in your XingGraph memory so you can recall it in the next step.`,
            node: (
              <div style={{ display: "flex", flexDirection: "column", gap: 14, width: "100%" }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#EDECEA", marginBottom: 6 }}>Option A · Your existing memory</div>
          ...
          },
          {
            ...
            description: connectVerified
              ? "We detected your new session in XingGraph Local — you're connected. You're all set."
              : `Now ask ${name} a question about what you just uploaded — it should answer from XingGraph Local. (For the sample, use the question below.) This step completes on its own once your session shows up.`,
            node: (
              <div style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%" }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#EDECEA", marginBottom: 6 }}>First, run this to start a fresh session</div>
...
function skipToDashboard(router: ReturnType<typeof useRouter>) {
  try {
    markOnboardingCompleteLocally();
    sessionStorage.setItem("xinggraph-onboarding-skipped", "1");
  } catch { /* ignore */ }
...
export default function OnboardingPage() {
  ...
  // Top-level branch: path selection → either the Claude/Codex card
  // flow ("agent") or the existing Company Brain upload flow ("company").
  // The old Step0 welcome screen is gone — the /welcome page owns that moment
  // (including the tutorial video), so onboarding starts at path selection.
  // A ?step= deep link jumps straight into the Company Brain sub-steps.
  const [view, setView] = useState<"select" | "company" | "agent">(initialStep >= 1 ? "company" : "select");
  ...
  if (isServeMode) {
    // Local-mode onboarding requires a working xinggraph-cli backend connection.
    if (!cogniInstance) {
      return (
        <><TrackPageView page="Onboarding" /><div className="flex items-center justify-center h-screen" style={darkPage}>
          <span style={{ fontSize: 14, color: "rgba(237,236,234,0.65)" }}>Connecting...</span>
