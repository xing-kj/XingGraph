  ...
  return (
    <div className="flex flex-col items-center gap-8 flex-1" style={{ paddingTop: 48, paddingBottom: 48, paddingInline: 80 }}>
      <div className="flex flex-col items-center gap-2">
        <StepBadge step={1} />
        <h1 style={{ fontSize: 28, fontWeight: 300, color: "#EDECEA", margin: 0, fontFamily: '"TWKLausanne", sans-serif', letterSpacing: "-0.02em" }}>Connected to XingGraph Cloud</h1>
        <p style={{ fontSize: 15, color: "rgba(237,236,234,0.55)", margin: 0, textAlign: "center", lineHeight: "22px", maxWidth: 480 }}>
          Your local SDK is connected to your XingGraph Cloud instance via <code style={{ background: "rgba(255,255,255,0.1)", color: "#EDECEA", padding: "1px 6px", borderRadius: 4, fontSize: 13 }}>xinggraph.serve()</code>.
    ...
    <div className="flex flex-col items-center gap-6 flex-1" style={{ paddingTop: 48, paddingBottom: 48, paddingInline: 80 }}>
      ...
      <div className="flex flex-col gap-4 w-full items-center">
        ...
        <div style={{ width: "100%", maxWidth: 520 }}>
          ...
          <CodeBlock code={`results = await xinggraph.recall(\n    "What did Einstein develop?",\n    datasets=["scientists"]\n)\nprint(results)`} />
        </div>
        ...
        <div style={{ width: "100%", maxWidth: 520 }}>
          ...
          <CodeBlock code={`await xinggraph.visualize("graph.html")`} />
        </div>
      </div>
...
function ServeStep3({ onNext, cogniInstance }: {
  ...
  const handleSearch = async (q: string) => {
    ...
    try {
      ...
      setResults(["Search failed. Make sure you've added data via xinggraph.remember()."]);
    } finally {
