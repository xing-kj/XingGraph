...
function colorizeLine(line: string): React.ReactNode {
  ...
  let remaining = line;
  let key = 0;
  const pattern = /(import |from |await |async |def |for |if |return |export |pip install |curl |xinggraph\.\w+|"[^"]*"|'[^']*')/g;
  let match;
  let lastIndex = 0;
  while ((match = pattern.exec(remaining)) !== null) {
    ...
    } else if (token.startsWith("xinggraph.")) {
...
// ── Connect Local XingGraph SDK ──

export function LocalXingGraphStep({ onBack, onSkip, standalone }: { onBack: () => void; onSkip: () => void; standalone?: boolean }) {
    ...
    <div style={{ alignItems: "center", display: "flex", flexDirection: "column", flexGrow: 1, gap: standalone ? 16 : 32, paddingBlock: standalone ? 16 : 48, paddingInline: standalone ? 16 : 40, fontFamily: '"Inter", system-ui, sans-serif', overflowY: "auto" }}>
      {!standalone && (
        <div style={{ alignItems: "center", display: "flex", flexDirection: "column", gap: 8 }}>
          <StepBadge step={1} />
          <h1 style={{ color: "#18181B", fontSize: 28, fontWeight: 600, lineHeight: "34px", margin: 0 }}>Connect your local XingGraph SDK</h1>
          <p style={{ color: "#71717A", fontSize: 15, lineHeight: "22px", margin: 0, textAlign: "center", maxWidth: 540 }}>
            Use <code style={{ background: "#F4F4F5", padding: "1px 6px", borderRadius: 4, fontSize: 13 }}>xinggraph.serve()</code> to link your local Python SDK to a remote XingGraph instance. All operations (remember, recall, forget, improve) will route to the connected instance.
          </p>
        </div>
      )}
      ...
      <div style={{ backgroundColor: "#FFFFFF", borderColor: standalone ? "transparent" : "#E4E4E7", borderRadius: standalone ? 0 : 12, borderStyle: "solid", borderWidth: standalone ? 0 : 1, display: "flex", flexDirection: "column", gap: 20, paddingBlock: 24, paddingInline: 24, width: "100%" }}>
        ...
        <div style={{ display: "flex", gap: 0, borderRadius: 8, border: "1px solid #E4E4E7", overflow: "hidden" }}>
          <button onClick={() => { trackEvent({ pageName: "Onboarding - Local XingGraph", eventName: "local_xinggraph_mode_switched", additionalProperties: { mode: "cloud" } }); setConnectMode("cloud"); }} className="cursor-pointer" style={{ flex: 1, padding: "10px 16px", background: connectMode === "cloud" ? "#F0EDFF" : "#fff", border: "none", fontSize: 13, fontWeight: 500, color: connectMode === "cloud" ? "#6510F4" : "#71717A", fontFamily: "inherit", borderRight: "1px solid #E4E4E7" }}>
            Connect to XingGraph Cloud
          </button>
          <button onClick={() => { trackEvent({ pageName: "Onboarding - Local XingGraph", eventName: "local_xinggraph_mode_switched", additionalProperties: { mode: "direct" } }); setConnectMode("direct"); }} className="cursor-pointer" style={{ flex: 1, padding: "10px 16px", background: connectMode === "direct" ? "#F0EDFF" : "#fff", border: "none", fontSize: 13, fontWeight: 500, color: connectMode === "direct" ? "#6510F4" : "#71717A", fontFamily: "inherit" }}>
        ...
        {connectMode === "cloud" ? (
          <>
            ...
            <div style={{ display: "flex", gap: 10, background: "#F0EDFF", border: "1px solid #DDD6FE", borderRadius: 8, padding: "10px 14px" }}>
              <span style={{ fontSize: 12, color: "#52525B", lineHeight: "18px" }}>
                Connect your local XingGraph SDK to your cloud tenant using the API Base URL and key from the <a href="/api-keys" style={{ color: "#6510F4", textDecoration: "underline" }}>API Keys</a> page. All operations (remember, recall) will route to cloud.
              </span>
            </div>

            <div style={{ alignItems: "center", display: "flex", gap: 12 }}>
              <NumberCircle n={1} />
              <span style={{ color: "#18181B", fontSize: 14, fontWeight: 500 }}>Install XingGraph</span>
            </div>
            <CodeBlock step="Local XingGraph">pip install xinggraph</CodeBlock>
            ...
            <Link
              href="/api-keys"
              onClick={() => trackEvent({ pageName: "Onboarding - Local XingGraph", eventName: "local_go_to_api_keys" })}
            ...
            <div style={{ alignItems: "center", display: "flex", gap: 12 }}>
              <NumberCircle n={3} />
              <span style={{ color: "#18181B", fontSize: 14, fontWeight: 500 }}>Connect and use XingGraph</span>
            </div>
            <MultiLineCode step="Local XingGraph" lines={[
              "import asyncio",
              "import xinggraph",
              "",
              "async def main():",
              '    await xinggraph.serve(',
              '        url="<your-api-base-url>",',
              '        api_key="<your-api-key>"',
              "    )",
              "",
              '    await xinggraph.remember("Einstein developed general relativity in 1915.", datasets=["default_dataset"])',
              "",
              '    results = await xinggraph.recall("What did Einstein develop?", datasets=["default_dataset"])',
              "    print(results)",
              "",
              "asyncio.run(main())",
            ]} />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <span style={{ color: "#A1A1AA", fontSize: 12, lineHeight: "18px" }}>
                Credentials are saved at <code style={{ fontSize: 11 }}>~/.xinggraph/cloud_credentials.json</code>. Subsequent <code style={{ fontSize: 11 }}>xinggraph.serve()</code> calls reconnect automatically.
              </span>
            </div>
          </>
        ) : (
          <>
            ...
            <div style={{ display: "flex", gap: 10, background: "#F0EDFF", border: "1px solid #DDD6FE", borderRadius: 8, padding: "10px 14px" }}>
              <span style={{ fontSize: 12, color: "#52525B", lineHeight: "18px" }}>
                Connect to any XingGraph instance — self-hosted, another team member's machine, or a staging environment. Just provide the URL and an optional API key.
              </span>
            </div>
            ...
            <MultiLineCode step="Local XingGraph" lines={[
              ...
              '    await xinggraph.serve(',
              '        url="https://your-instance.xinggraph.ai",',
              '        api_key="your-api-key"',
              "    )",
              "",
              "    # Add and process data on the remote instance",
              '    await xinggraph.remember("Your data here", datasets=["default_dataset"])',
              "",
              "    # Query the remote knowledge graph",
              '    results = await xinggraph.recall("What do we know about X?", datasets=["default_dataset"])',
              "    print(results)",
              "",
              "asyncio.run(main())",
            ]} />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <span style={{ color: "#A1A1AA", fontSize: 12, lineHeight: "18px" }}>
                To connect to a XingGraph backend running on the same machine or your local network:
              </span>
              <MultiLineCode step="Local XingGraph" lines={[
                "# Inside an async function:",
                '# Same machine',
                'await xinggraph.serve(url="http://localhost:8000")',
                "",
                "# Local network",
                'await xinggraph.serve(url="http://192.168.1.50:8000")',
              ]} />
            </div>
          </>
        )}
      ...
      {!standalone && <NavButtons onBack={onBack} continueDisabled={false} onContinue={onSkip} pageName="Onboarding - Local XingGraph" />}
    </div>
  );
}
  ...
  return (
    <>
      ...
      {mcpMode === "local" ? (
        <>
          <span style={{ color: "#71717A", fontSize: 12, lineHeight: "18px" }}>
            The MCP server starts automatically with <code style={{ fontSize: 11 }}>xinggraph -ui</code>, or run it standalone:
          </span>
          <CodeBlock step="Agent">xinggraph-mcp --transport sse --port 8001</CodeBlock>
          <div style={{ display: "flex", gap: 8, background: "#FEF3C7", border: "1px solid #FDE68A", borderRadius: 8, padding: "10px 14px", alignItems: "flex-start" }}>
            ...
            <span style={{ fontSize: 12, color: "#92400E", lineHeight: "18px" }}>
              ...
              <code style={{ fontSize: 11, background: "#FDE68A", padding: "1px 4px", borderRadius: 3 }}>LLM_API_KEY=sk-... xinggraph-mcp --transport sse --port 8001</code>
            </span>
          </div>
        </>
      ) : (
        <>
          <span style={{ color: "#71717A", fontSize: 12, lineHeight: "18px" }}>
            Connect the MCP server to XingGraph Cloud. Use your API Base URL and API key from the <Link href="/api-keys" style={{ color: "#6510F4", textDecoration: "underline" }} onClick={() => trackEvent({ pageName: "Onboarding - Agent", eventName: "mcp_go_to_api_keys" })}>API Keys</Link> page.
          </span>
          <MultiLineCode step="Agent" lines={[
            'xinggraph-mcp --transport sse --port 8001 \\',
            '  --serve-url https://your-instance.xinggraph.ai \\',
      ...
      <span style={{ color: "#71717A", fontSize: 12, lineHeight: "18px" }}>
        Add XingGraph as a tool server in your MCP client&apos;s configuration:
      </span>
      <MultiLineCode step="Agent" lines={[
        '{',
        '  "mcpServers": {',
      ...
      {!standalone && (
        <div style={{ alignItems: "center", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <p style={{ color: "#71717A", fontSize: 15, lineHeight: "22px", margin: 0, textAlign: "center", maxWidth: 520 }}>
            Give any agent memory by connecting it to XingGraph. Choose how your agent should integrate.
          </p>
        </div>
      )}
      ...
      <div style={{ backgroundColor: "#FFFFFF", borderColor: standalone ? "transparent" : "#E4E4E7", borderRadius: standalone ? 0 : 12, borderStyle: "solid", borderWidth: standalone ? 0 : 1, display: "flex", flexDirection: "column", gap: 20, paddingBlock: 24, paddingInline: 24, width: "100%" }}>
        ...
        {method === "sdk" && (
          <>
            ...
            <CodeBlock step="Agent">pip install xinggraph</CodeBlock>
            ...
            <MultiLineCode step="Agent" lines={[
              ...
              '    await xinggraph.remember("User prefers dark mode and concise answers.", datasets=["default_dataset"])',
              "",
              "    # Retrieve relevant context for your agent",
              '    results = await xinggraph.recall("What are the user preferences?", datasets=["default_dataset"])',
              "    for item in results:",
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <span style={{ color: "#A1A1AA", fontSize: 12, lineHeight: "18px" }}>
                The SDK is framework-agnostic. Call <code style={{ fontSize: 11 }}>xinggraph.remember()</code> and <code style={{ fontSize: 11 }}>xinggraph.recall()</code> from within your agent's tool functions, hooks, or middleware — regardless of whether you use LangChain, LlamaIndex, raw OpenAI, or a custom framework.
              </span>
            </div>
          </>
        )}
...
const SAAS_SOURCES: { name: string; install: string; code: string[]; extracts: string }[] = [
  {
    ...
    code: [
      ...
      ")",
      "",
      'await xinggraph.add(source, dataset_name="slack_data")',
    ],
  ...
  },
  {
    ...
    code: [
      ...
      "",
      'await xinggraph.add(source, dataset_name="notion_data")',
    ],
  ...
  },
  {
    ...
    code: [
      ...
      "",
      'await xinggraph.add(source, dataset_name="github_data")',
    ],
  ...
  },
  {
    ...
    code: [
      ...
      "source = hubspot()",
      "",
      'await xinggraph.add(source, dataset_name="hubspot_data")',
    ],
  ...
  },
  {
    ...
    code: [
      ...
      ")",
      "",
      'await xinggraph.add(source, dataset_name="sheets_data")',
    ],
  ...
  },
  {
    ...
    code: [
      ...
      "source = jira()",
      "",
      'await xinggraph.add(source, dataset_name="jira_data")',
    ],
  ...
  },
  {
    ...
    code: [
      ...
      "})",
      "",
      'await xinggraph.add(source, dataset_name="api_data")',
    ],
      ...
      {!standalone && (
        <div style={{ alignItems: "center", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <p style={{ color: "#71717A", fontSize: 15, lineHeight: "22px", margin: 0, textAlign: "center", maxWidth: 560 }}>
            XingGraph uses <a href="https://dlthub.com" target="_blank" rel="noopener noreferrer" style={{ color: "#6510F4", textDecoration: "underline" }} onClick={() => trackEvent({ pageName: "Onboarding - Database", eventName: "click_out", additionalProperties: { target_url: "https://dlthub.com" } })}>dlt (data load tool)</a> to connect to databases, SaaS platforms, and APIs. Data is extracted, loaded into a knowledge graph with schema and relationships preserved, and made searchable.
          </p>
        </div>
      )}
      ...
      <div style={{ backgroundColor: "#FFFFFF", borderColor: standalone ? "transparent" : "#E4E4E7", borderRadius: standalone ? 0 : 12, borderStyle: "solid", borderWidth: standalone ? 0 : 1, display: "flex", flexDirection: "column", gap: 20, paddingBlock: 24, paddingInline: 24, width: "100%" }}>
        ...
        {mode === "cloud" && (
          <div style={{ display: "flex", gap: 10, background: "#FEF3C7", border: "1px solid #FDE68A", borderRadius: 8, padding: "10px 14px", alignItems: "flex-start" }}>
            ...
            <span style={{ fontSize: 12, color: "#92400E", lineHeight: "18px" }}>
              For cloud, the XingGraph SDK runs locally to access your data sources, then pushes extracted data to your cloud tenant. Call <code style={{ background: "#FEF3C7", fontSize: 11 }}>xinggraph.serve(url, api_key)</code> first to connect.
            </span>
          </div>
        )}
        ...
        {category === "database" && (
          <>
            ...
            <div style={{ display: "flex", gap: 10, background: "#F0EDFF", border: "1px solid #DDD6FE", borderRadius: 8, padding: "10px 14px" }}>
              <span style={{ fontSize: 12, color: "#52525B", lineHeight: "18px" }}>
                Pass your connection string to <code style={{ background: "#E4DEFF", padding: "1px 4px", borderRadius: 3, fontSize: 11 }}>xinggraph.add()</code>. XingGraph auto-discovers tables, extracts schema and foreign key relationships, and builds a searchable knowledge graph.
              </span>
            </div>
            ...
            <MultiLineCode step="Database" lines={[
              ...
              ...(mode === "cloud" ? [
                ...
                '    await xinggraph.serve(',
                '        url="https://api.aws.xinggraph.ai",',
                '        api_key="your-api-key"',
                "    )",
                "",
              ] : []),
              `    await xinggraph.remember("${dbConfig.example}", datasets=["default_dataset"])`,
              "",
              "asyncio.run(main())",
            ]} />
        ...
        {category === "saas" && (
          <>
            ...
            <div style={{ display: "flex", gap: 10, background: "#F0EDFF", border: "1px solid #DDD6FE", borderRadius: 8, padding: "10px 14px" }}>
              <span style={{ fontSize: 12, color: "#52525B", lineHeight: "18px" }}>
                <strong>{saasConfig.name}</strong> extracts: {saasConfig.extracts}. XingGraph converts the extracted data into a knowledge graph with entities, relationships, and embeddings.
              </span>
            </div>
            ...
            <MultiLineCode step="Database" lines={[
              ...
              '    await xinggraph.cognify()  # Build knowledge graph',
              "",
              "asyncio.run(main())",
            ]} />
          </>
        ...
        {category === "files" && (
          <>
            <div style={{ display: "flex", gap: 10, background: "#F0EDFF", border: "1px solid #DDD6FE", borderRadius: 8, padding: "10px 14px" }}>
              <span style={{ fontSize: 12, color: "#52525B", lineHeight: "18px" }}>
                Upload files directly, pass file paths, or point to CSV files. XingGraph auto-detects CSV and treats it as structured data using DLT. Other files (PDF, DOCX, TXT, MD) are processed as documents.
              </span>
            </div>
            ...
            <MultiLineCode step="Database" lines={[
              ...
              '    # Single file',
              '    await xinggraph.remember("/path/to/report.pdf", datasets=["default_dataset"])',
              "",
              "    # Multiple files",
            ...
            <MultiLineCode step="Database" lines={[
              "# Inside an async function:",
              'await xinggraph.remember("/path/to/data.csv", datasets=["default_dataset"])',
            ]} />
            ...
            <MultiLineCode step="Database" lines={[
              "# Inside an async function:",
              'await xinggraph.remember("s3://my-bucket/documents/report.pdf", datasets=["default_dataset"])',
            ]} />
          </>
        )}
      </div>
