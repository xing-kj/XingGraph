    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", paddingInline: 32, paddingBottom: 32 }}>
      <div style={{ flex: 1, background: "rgba(255,255,255,0.06)", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, padding: 48 }}>
        ...
        <p style={{ fontSize: 14, color: "rgba(237,236,234,0.35)", margin: 0, maxWidth: 360, textAlign: "center" }}>
          Upload documents to a brain and let xinggraph process them. Once a knowledge graph is built, its type structure will appear here.
        </p>
...
export default function SchemaPage() {
  ...
  useEffect(() => {
    ...
    loadGraphModelsConfig(cogniInstance).then((cfg) => {
      ...
      setSelectedChunker(findChunkerForDataset(cfg.chunkerAssignments ?? {}, datasetId));
    }).catch(() => {});
...
useEffect(() => {
  ...
  function getCognifyOptions() {
    const opts: { graphModel?: object; customPrompt?: string; ontologyKey?: string[]; chunker?: string } = {};
    if (selectedModelId) {
    ...
    if (selectedChunker) opts.chunker = selectedChunker;
    return opts;
  }
  ...
  function handleSelectChunker(value: string | null) {
    setSelectedChunker(value);
    setChunkerDropdownOpen(false);
    const label = value && value !== "automatic"
      ? (CHUNKER_OPTIONS.find((o) => o.value === value)?.label ?? value)
      : "Automatic";
    notifications.show({ title: "Chunker updated", message: `"${datasetName}" will use "${label}" on next run.`, color: "green", autoClose: 4000 });
    if (cogniInstance && datasetId) assignChunkerToDataset(cogniInstance, datasetId, value).catch(() => {});
  }
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <div style={{ padding: "24px 32px 16px" }}>
          ...
          <p style={{ fontSize: 14, color: "rgba(237,236,234,0.55)", margin: 0 }}>Define how XingGraph extracts knowledge from your brain — set the extraction model, prompt, and ontology, then re-process to build memory.</p>
        </div>
      ...
      {datasetId && (
        <div style={{ padding: "10px 32px 12px", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", borderBottom: "1px solid rgba(255,255,255,0.08)", flexShrink: 0 }}>
          ...
          <div ref={chunkerDropdownRef} style={{ position: "relative" }}>
            <button onClick={() => setChunkerDropdownOpen((v) => !v)} style={BTN}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(237,236,234,0.6)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v20M17 7l-5-5-5 5" transform="rotate(-180 12 12)" /><rect x="4" y="2" width="16" height="20" rx="2" /></svg>
              Chunker: {selectedChunker && selectedChunker !== "automatic"
                ? (CHUNKER_OPTIONS.find((o) => o.value === selectedChunker)?.label ?? selectedChunker)
                : "Automatic"}
              <svg width="9" height="9" viewBox="0 0 10 10" fill="none"><path d="M2.5 4L5 6.5L7.5 4" stroke="rgba(237,236,234,0.4)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </button>
            {chunkerDropdownOpen && (
              <div style={PANEL}>
                <div style={{ padding: "6px 10px 4px", fontSize: 10, fontWeight: 700, color: "rgba(237,236,234,0.35)", letterSpacing: 0.4, textTransform: "uppercase" }}>Chunker</div>
                {CHUNKER_OPTIONS.map((option) => (
                  <button key={option.value} onClick={() => handleSelectChunker(option.value)} style={{ width: "100%", background: "none", border: "none", padding: "8px 12px", fontSize: 13, color: "#EDECEA", display: "flex", alignItems: "center", gap: 8, textAlign: "left", fontFamily: "inherit", cursor: "pointer" }}>
                    <span style={{ width: 16, color: "#BC9BFF" }}>
                      {selectedChunker && selectedChunker === option.value ? "✓" : !selectedChunker && option.value === "automatic" ? "✓" : ""}
                    </span>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => !inferringPrompt && setShowCreatePromptModal(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "rgba(15,15,15,0.92)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 24, width: 440, display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 16px 48px rgba(0,0,0,0.5)" }}>
            ...
            {inferringPrompt ? (
              ...
              <>
                <p style={{ fontSize: 13, color: "rgba(237,236,234,0.55)", margin: 0, lineHeight: "20px" }}>A custom prompt guides how XingGraph extracts entities and relationships.</p>
      ...
      {showUploadOntologyModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowUploadOntologyModal(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "rgba(15,15,15,0.92)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 24, width: 440, display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 16px 48px rgba(0,0,0,0.5)" }}>
            ...
            <p style={{ fontSize: 13, color: "rgba(237,236,234,0.55)", margin: 0, lineHeight: "20px" }}>Upload an OWL ontology file to guide how XingGraph structures your memory schema.</p>
