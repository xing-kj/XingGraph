...
const fetchMessages = (instance: XingGraphInstance) => {
  return instance.fetch("/v1/search/")
    .then(response => response.json());
};

const sendMessage = (message: string, searchType: string, topK: number, instance: XingGraphInstance) => {
  return instance.fetch("/v1/search/", {
    method: "POST",
    headers: {
...
export default function useChat(dataset: Dataset, instance: XingGraphInstance) {
