﻿<div align="center">
  ...
  XingGraph - 정확하고 지속적인 AI 메모리

  <p align="center">
  <a href="https://www.youtube.com/watch?v=1bezuvLwJmw&t=2s">데모</a>
  .
  <a href="https://docs.xinggraph.ai/">문서</a>
  .
  <a href="https://xinggraph.ai">더 알아보기</a>
  ·
  <a href="https://discord.gg/NQPKmU5CCg">Discord 참여</a>
  ·
  <a href="https://www.reddit.com/r/AIMemory/">r/AIMemory 참여</a>
  .
  <a href="https://github.com/xing-kj/xinggraph-community">커뮤니티 플러그인 & 애드온</a>
  </p>
...
<p>
  <a href="https://www.producthunt.com/posts/xinggraph?embed=true&utm_source=badge-top-post-badge&utm_medium=badge&utm_souce=badge-xinggraph" target="_blank" style="display:inline-block; margin-right:10px;">
    <img src="https://api.producthunt.com/widgets/embed-image/v1/top-post-badge.svg?post_id=946346&theme=light&period=daily&t=1744472480704" alt="xinggraph - Memory&#0032;for&#0032;AI&#0032;Agents&#0032;&#0032;in&#0032;5&#0032;lines&#0032;of&#0032;code | Product Hunt" width="250" height="54" />
  </a>
...
데이터를 사용하여 AI 에이전트를 위한 개인화되고 동적인 메모리를 구축하세요. XingGraph를 사용하면 RAG를 확장 가능하고 모듈화된 ECL(추출[Extract], 인지화[Cognify], 로드[Load]) 파이프라인으로 대체할 수 있습니다.
...
XingGraph는 원시 데이터를 에이전트를 위한 지속적이고 동적인 AI 메모리로 변환하는 오픈 소스 도구이자 플랫폼입니다. 벡터 검색과 그래프 데이터베이스를 결합하여 문서를 의미적으로 검색 가능하게 하고 관계별로 연결합니다.

XingGraph는 두 가지 방식으로 사용할 수 있습니다.

1. [XingGraph 오픈 소스 (셀프 호스팅)](https://docs.xinggraph.ai/getting-started/installation): 기본적으로 모든 데이터를 로컬에 저장합니다.
2. [XingGraph Cloud (관리형)](https://platform.xinggraph.ai/): 관리형 인프라에서 동일한 OSS 스택을 사용하여 더 쉽게 개발하고 프로덕션화할 수 있습니다.

### XingGraph 오픈 소스 (셀프 호스팅):
...
단 몇 줄의 코드로 XingGraph를 사용해 보세요. 자세한 설정 및 구성은 [XingGraph 문서](https://docs.xinggraph.ai/getting-started/installation#environment-configuration)를 참조하세요.
...
**pip**, **poetry**, **uv** 또는 선호하는 Python 패키지 관리자를 사용하여 XingGraph를 설치할 수 있습니다.
...
```
또는 [템플릿](https://github.com/xing-kj/xinggraph/blob/main/.env.template)을 사용하여 `.env` 파일을 생성하세요.

다른 LLM 공급자를 통합하려면 [LLM 공급자 문서](https://docs.xinggraph.ai/setup-configuration/llm-providers)를 참조하세요.

### 3단계: 파이프라인 실행

XingGraph는 문서를 가져와 지식 그래프를 생성한 다음 결합된 관계를 기반으로 그래프를 쿼리합니다.
...
async def main():
    # XingGraph에 텍스트 추가
    await xinggraph.add("XingGraph turns documents into AI memory.")
    ...
    # 지식 그래프 쿼리
    results = await xinggraph.search("What does XingGraph do?")
...
보시다시피, 출력은 이전에 XingGraph에 저장한 문서에서 생성됩니다.

```bash
  XingGraph turns documents into AI memory.
```
...
```bash
xinggraph-cli add "XingGraph turns documents into AI memory."

xinggraph-cli cognify

xinggraph-cli search "What does XingGraph do?"
...
[LangGraph 에이전트를 위한 XingGraph 메모리](https://github.com/user-attachments/assets/e113b628-7212-4a2b-b288-0be39a93a1c3)
...
### 기여하기
여러분들의 기여를 환영합니다! 여러분의 의견은 XingGraph를 더 좋게 만드는 데 큰 도움이 됩니다. 시작하려면 [`CONTRIBUTING.md`](CONTRIBUTING.md)를 참조하세요.

### 규칙

우리는 포용적이고 존중하는 커뮤니티를 만들기 위해 노력하고 있습니다. 규칙은 [규칙 문서](https://github.com/xing-kj/xinggraph/blob/main/CODE_OF_CONDUCT.md)를 확인해주세요.
...
@misc{xinggraph2025optimizinginterfaceknowledgegraphs,
      ...
      author={XingGraph Contributors and XingGraph Contributor and XingGraph Contributor and XingGraph Contributor},
      year={2025},
      eprint={2505.24478},
      archivePrefix={arXiv},
      primaryClass={cs.AI},
