...
import asyncio
import pathlib
from xinggraph import config, add, cognify, search, SearchType, prune, visualize_graph
...
async def main():
    ...
    # Set up the data directory. XingGraph will store files here.
    config.data_root_directory(data_directory_path)

    xinggraph_directory_path = str(
        pathlib.Path(os.path.join(pathlib.Path(__file__).parent, ".xinggraph_system")).resolve()
    )
    # Set up the XingGraph system directory. XingGraph will store system files and databases here.
    config.system_root_directory(xinggraph_directory_path)
    ...
    # Add the text data to XingGraph.
    await add(text)
