...
``harness`` or ``xinggraph``.

Keep this module stdlib-only (apart from a lazy ``import ladybug`` inside
the function body). It's imported by both the xinggraph adapter (which runs in
the parent process with xinggraph available) and by
``xinggraph_db_workers.kuzu_worker`` (which runs in a spawned subprocess that
must NOT pull xinggraph in). Adding a top-level xinggraph import here would
silently regress that invariant — the subprocess would re-import xinggraph's
full ~200 MB dependency graph at start. The ``test_worker_import_hygiene.py``
test enforces the no-xinggraph rule, but keeping it documented at the source
...
def install_json_extension_local(
    ...
    pattern as
    ``xinggraph/infrastructure/databases/graph/ladybug/ladybug_migrate.py``.
    """
    import ladybug
