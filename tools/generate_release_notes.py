...
Uses litellm + instructor directly to avoid xinggraph's dotenv.load_dotenv(override=True)
...
async def generate_release_notes_with_llm(
    ...
    Uses litellm + instructor directly instead of xinggraph's LLM client
    to avoid xinggraph's dotenv.load_dotenv(override=True) overwriting
    ...
    system_prompt = """You are a technical writer creating release notes for XingGraph, an AI memory platform.
...
- Assume the reader has never seen this feature before and does not know XingGraph's internal vocabulary.
...
"""
    ...
    text_input = f"""Generate release notes for XingGraph version {version or "TBD"}.
...
def generate_fallback_notes(
    ...
    notes.summary = f"XingGraph v{version} includes {len(notes.features)} new features, {len(notes.bug_fixes)} bug fixes, and {len(notes.improvements)} improvements."
...
def format_release_notes(
    ...
    # Footer
    md += "---\n\n"
    md += f"*— The XingGraph Team · {date_str}*\n"
