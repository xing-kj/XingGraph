...
def score_source_file(changed_file: str, notes: dict[str, Any], assessment: dict[str, Any]) -> int:
    ...
    if lower_path.startswith("xinggraph/api/"):
        score += 60
    if lower_path.startswith("xinggraph/cli/"):
        score += 55
    ...
    if "/models/" in lower_path:
        score += 30
    if lower_path.startswith("xinggraph/modules/"):
        score += 25
    if lower_path.startswith("xinggraph/infrastructure/"):
        score += 20
