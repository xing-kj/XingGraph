"""Simulate an agent connecting to a XingGraph instance.
...
Usage:
    ...
    python tools/simulate_agent.py --type ResearchAgent --port 8000 --search "What is XingGraph?"
"""
...
def main():
    parser = argparse.ArgumentParser(description="Simulate an agent connecting to XingGraph")
    parser.add_argument("--type", required=True, help="Agent type name (e.g. SecFilingAgent)")
    parser.add_argument("--port", type=int, default=8000, help="XingGraph backend port")
    ...
    email = f"{args.type}-{short_id}@xinggraph.agent"
