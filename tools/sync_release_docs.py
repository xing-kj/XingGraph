#!/usr/bin/env python3
"""
Sync release docs artifacts into a checked-out xinggraph-docs repository.

This script is intended for CI usage from the core `xinggraph` repository:
...
title: "Changelog"
description: "Recent XingGraph releases"
icon: "scroll-text"
---

XingGraph releases with highlights and links to the full release notes on GitHub.
"""


def generate_openapi_spec(output_path: Path) -> None:
    """
    Generate OpenAPI schema from xinggraph FastAPI app and write it to output_path.
    """
    try:
        # Avoid prod-only initialization behavior for CI schema generation.
        os.environ.setdefault("ENV", "dev")
        from xinggraph.api.client import app  # pylint: disable=import-outside-toplevel
    except Exception as exc:  # pragma: no cover - runtime import environment specific
        raise RuntimeError(f"Failed to import xinggraph API app: {exc}") from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sync release docs artifacts into xinggraph-docs repo"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--openapi-output",
        default="xinggraph_openapi_spec.json",
        type=Path,
